...
5. ✅ Tests: service (búsqueda con artículo real, min 2 chars, gating endpoints, **promos especiales** NxM-lista + combo-grupo) + smoke del modal. Traducciones es/en/pt. Pint verde.
6. ✅ Promos: el consultor muestra promos de los DOS sistemas (normales vía `obtenerPromocionesActivas` + **especiales** NxM/combo/menú vía las 5 vías de participación en `PromocionEspecial`).
7. ✅ Rediseño orientado a SCANNER (pedido del usuario): input **oculto** siempre enfocado (el escáner = teclado, termina en Enter; fallback por silencio de 120ms), frase **idle configurable** (`mensaje_idle`), al escanear muestra nombre + precio (gigante) + "Promociones activas" centrado, y vuelve a idle tras `duracion_resultado` segundos (configurable). Estado "No se encontró el artículo" si no matchea. Sin buscador visible ni lista.
