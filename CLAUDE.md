...
sudo systemctl reload apache2      # SAPI web real: mod_php 8.3 (verificado 2026-07-30) — ver regla de abajo
```
...
- **El SAPI web es `mod_php` 8.3, NO php-fpm** (verificado en el server 2026-07-30: Apache carga `php_module`, `proxy_fcgi` deshabilitado, ningún vhost referencia FPM). `opcache.validate_timestamps` está en su default (1): OPcache revalida por mtime y los deploys toman efecto solos — el `reload apache2` es cinturón de seguridad. **⚠️ NUNCA setear `validate_timestamps=0` sin migrar antes el SAPI a FPM y el paso de reload junto con él**: con vt=0 bajo mod_php y un reload que apunta a un servicio FPM que no atiende tráfico, los deploys dejan de tomar cambios en silencio.
