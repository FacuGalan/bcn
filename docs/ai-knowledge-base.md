...
**Login del CORE en movil + traducciones de auth (RF-C1, spec `tienda-sesion-persistente`, 2026-08-06)**: `resources/views/layouts/guest.blade.php` (compartido por las 5 vistas de auth: login, forgot-password, reset-password, confirm-password, verify-email) redisenia el layout SOLO en movil -- el conjunto logo+tarjeta pasa a quedar centrado verticalmente en el viewport (contenedor raiz con `justify-center`, logo compacto `h-16` pegado a la tarjeta) en vez del layout previo donde el logo flotaba solo en un bloque `flex-1` gigante y el formulario quedaba al fondo de la pantalla. Desktop (`sm:` en adelante) queda igual, sin cambios. Se agregan `lang/{es,pt}/auth.php`, `passwords.php` y `validation.php` (no existian: los mensajes de framework -"These credentials do not match our records.", "We have emailed your password reset link!", "The password field is required."- salian en ingles aunque el resto del sistema ya estuviera traducido); `en.json`/`pt.json` suman las claves literales de las vistas que faltaban (en `es.json` la clave es el propio texto). Sin cambios de logica de autenticacion del core (el modal de limite de sesiones sigue igual, solo hereda el layout nuevo en movil).
...
**Panel tactil — precio efectivo, no el fallback global (RF-05, 2026-07-24)**: `cargarCatalogoTactil()` arma un snapshot de articulos por categoria que Alpine consume del lado cliente (seed unico via `@js`, no se re-renderiza solo). El precio de cada articulo se resuelve con `obtenerPrecioConLista($a)['precio']` — la MISMA cadena que aplica `seleccionarArticulo()` al agregar (override por sucursal + lista activa) — en vez del `articulos.precio_base` global crudo que se usaba antes (bug: en sucursales con precio propio o con lista condicionada activa, el precio que se veia en la grilla no coincidia con el que terminaba agregandose). `cargarCatalogoTactil()` se llama al FINAL de `mount()`, despues de resolver lista de precios/forma de venta (incluida la lista del pedido en edicion) — antes se llamaba antes, con el contexto de precios todavia sin definir. Mismo fix, mismo patron, en `NuevoPedidoDelivery` (que ademas expone `refrescarPreciosCatalogoTactil()`: al cambiar delivery↔take-away puede cambiar la lista activa por forma de venta, y ese metodo re-cotiza los precios del snapshot y los parchea en Alpine via el evento `catalogo-tactil-precios`, escuchado en `nuevo-pedido-delivery.blade.php`).
...
**Componente Alpine `domicilioMapa`** (`resources/js/domicilio-mapa.js`): registrado en `alpine:init`. Recibe `{key, mapId, txtGeoError, autocompletarDireccion, autoAbrir, conBuscador}`. Carga perezosa: `init()` solo registra el `$wire.$watch('domLocalidadCentro')`; el SDK se carga en `abrir()` (mapa) o en `montarBuscador()` (buscador) la primera vez. El mapa se construye una sola vez; llamadas sucesivas a `abrir()` solo recentran. Usa `PlaceAutocompleteElement` (API Places nueva, vigente para clientes nuevos desde 2025; el widget legacy `Autocomplete` fue discontinuado).

**Buscador desacoplado del mapa (`conBuscador`, 2026-08-04)**: el `PlaceAutocompleteElement` se creaba dentro de `construir()`, asi que con el mapa cerrado no existia y solo se podia corregir el TEXTO de la direccion, nunca la ubicacion. Ahora `montarBuscador()` (idempotente, comparte la promesa `_montaje`) lo monta en `x-ref="autocompleteSlot"` y lo llaman tanto el campo señuelo del form como `construir()`. Con `conBuscador` (default: el valor de `autocompletarDireccion`, o sea los dos flujos de delivery) el slot vive FUERA del bloque `x-show="abierto"` del mapa, dentro de un `wire:ignore`; al elegir una prediccion con el mapa cerrado, `seleccionarPrediccion()` lo abre para confirmar el pin. `colocar()` guarda `this.coords` antes de dibujar y `construir()`/`abrir()` lo usan como fallback, porque la prop Livewire (`domLatitud`/`domLongitud`) puede no haber vuelto todavia. `aplicarLocalidad()` aplica la `locationRestriction` al buscador aunque no haya mapa.
...
| `palabra_clave_entrega` | varchar(30) nullable | RF-T64: palabra simple generada al confirmar (si `config_delivery.usar_palabra_clave`), idempotente -- reconfirmaciones no la rotan. Solo pedidos `tipo=delivery` |
...
- **`categorias`**: `imagen_path` (varchar(255) nullable), `orden` (int, 0). `badges_tienda` json nullable (RF-T36, 2026-07-29, migracion `add_badges_tienda_a_categorias`, `AFTER imagen_path`) — MISMO shape que `articulos.badges_tienda` (arriba): `[{"tipo":"sin_tacc"}, {"tipo":"custom","texto":"..."}]`, maximo `Articulo::MAX_BADGES_TIENDA = 4`, catalogo cerrado en `Articulo::BADGES_TIENDA`. Saneador espejo `Categoria::badgesTienda()` (mismas reglas que `Articulo::badgesTienda()`: descarta tipos invalidos, `custom` sin texto o > 30 chars, trunca a 4). `imagen_focal_x` / `imagen_focal_y` decimal(5,2) NOT NULL default `50.00` (RF-T62, 2026-08-03, migracion `add_imagen_focal_a_categorias`, `AFTER imagen_path`) — punto focal (%) del banner de `imagen_path`, espejo de las columnas homonimas de `articulos` (RF-T14). Ver RF-T62 mas abajo.
...
|---|---|---|
| `tiendas` | `comercio_id` FK, `sucursal_id` (FK logico tenant), `slug` UNIQUE(60), `habilitada`, `dominio_propio` UNIQUE nullable, `ga4_measurement_id` varchar(30) nullable, `meta_pixel_id` varchar(30) nullable, `tema` json nullable, `logo_path` varchar(255) nullable, `portada_path` varchar(255) nullable, `historias` json nullable | UNIQUE(`comercio_id`,`sucursal_id`). La tienda es POR SUCURSAL (D15): el slug identifica comercio+sucursal, sin ambiguedad. `ga4_measurement_id`/`meta_pixel_id` (RF-T7, migracion `add_analytics_y_tema_a_tiendas`): IDs de Google Analytics 4 (formato `G-...`) y Meta Pixel (numerico); `null` = no inyectar el script de ese proveedor. `tema` (RF-T6, Principio 10): design tokens JSON PARCIAL o `null` — ver `Tienda::temaCompleto()` abajo. `logo_path`/`portada_path` (RF-T11, migracion `add_logo_portada_a_tiendas`, 2026-07-17): paths relativos al disco `public` (`tiendas/{comercio_id}/{uuid}.webp`), re-encodeados a WebP por `ImagenTiendaService` — ver mas abajo. `historias` (RF-T24, migracion `add_historias_a_tiendas`, 2026-07-28): array JSON `[{id, path, orden}]`, maximo `Tienda::MAX_HISTORIAS = 3`, path `tiendas/{comercio_id}/historias/{uuid}.webp` — ver `ImagenTiendaService::agregarHistoria()`/`eliminarHistoria()`/`reordenarHistorias()` mas abajo. Desde RF-T10 se alta y edita desde el panel (`Configuracion > Delivery / Take Away`, apartado "Tienda Online"), ya no solo por consola/soporte; desde RF-T11 la creacion y `habilitada` las escribe EXCLUSIVAMENTE el padre `ConfiguracionDelivery` (switch maestro) — ver "Alta y publicacion de una tienda" mas abajo |
...
| `consumidores` | `nombre`, `email` UNIQUE, `password` nullable, `telefono`, `fecha_nacimiento` date nullable, `email_verified_at`, `remember_token`, `google_id` varchar(64) nullable UNIQUE | Cuenta GLOBAL cross-comercio (D8), guard `consumidores` + Sanctum. Auth completa (registro/login/logout/verificacion/reset) implementada en el CORE (RF-T1, Fase 0 de la spec tienda-online) — `App\Http\Controllers\Api\V1\Consumidores\AuthController`. `fecha_nacimiento` (RF-T19, 2026-07-21, migracion `add_fecha_nacimiento_to_consumidores`): se copia del checkout de CUALQUIER tienda cuando el pedido lo trae y el consumidor esta logueado — pre-llena el cumpleanios en el checkout de otras tiendas del ecosistema. Expuesto en `GET /v1/consumidores/me`. `google_id` (RF-T49, 2026-07-30, migracion `add_google_id_a_consumidores`): claim `sub` del ID token de Google Identity Services (identificador estable de la cuenta Google — el email puede cambiar); `hidden` en el modelo y FUERA de `$fillable` a proposito, solo lo asigna el flujo Sign in with Google. `password` paso a NULLABLE en la misma migracion: las cuentas creadas via Google no tienen password |
...
| `consumidor_dispositivos` | `consumidor_id` FK CASCADE, `selector` char(24) UNIQUE, `validator_hash` char(64), `nombre` varchar(120) nullable, `ip_ultima` varchar(45) nullable, `ultimo_uso_at` nullable, `expira_at` (indexado) | Dispositivos recordados del consumidor (RF-T66, spec `tienda-sesion-persistente`, migracion `create_consumidor_dispositivos`, `Schema::connection('config')`, NO es tabla tenant). Par selector/validator estilo recaller: el `validator` NUNCA se persiste plano, solo su sha256 (`validator_hash`); el par en texto plano solo existe en la respuesta del canje, la tienda lo guarda en una cookie cifrada (el Bearer de Sanctum nunca viaja al navegador). `expira_at` es deslizante (+365 dias en cada canje exitoso). Maximo 10 filas por consumidor (`DispositivoService::MAX_DISPOSITIVOS`), se poda la menos usada al emitir la 11° |

`comercios` gana `rubro_id` (FK nullable a `rubros`) y `tienda_alta_cliente_automatica` (boolean, default 0).

**`Tienda::temaCompleto()`** (RF-T6, `app/Models/Tienda.php`): merge profundo (`array_replace_recursive`) de `Tienda::TEMA_DEFAULTS` con el JSON persistido en `tema` (que puede ser `null` o parcial). Las claves de `TEMA_DEFAULTS` son CONTRATO con `bcn-tienda`: agregar claves es aditivo, renombrar/quitar rompe el consumidor y exige v2 del endpoint. Shape (sub-objetos `portada`/`logo`/`textos`/`redes`/`catalogo`/`destacados`/`promos` aditivos, RF-T13 2026-07-18, RF-T25 2026-07-28, RF-T31/T33/T35 2026-07-29):
```php
[
    'colores' => ['primario' => '#4f46e5', 'acento' => '#f59e0b', 'fondo' => '#f9fafb', 'superficie' => '#ffffff', 'texto' => '#111827'],
    'tipografia' => ['fuente' => 'system'],
    'radios' => 'md',                                                // bordes de tarjetas/carrito (UI general)
    'densidad' => 'normal',
    'portada' => ['overlay' => false, 'posicion' => 'center'],       // 'overlay' DEPRECADA (RF-T25): el core la emite SIEMPRE en false (portada siempre cruda, sin fade); se conserva por tolerancia de contrato, bcn-tienda la ignora. 'posicion' = encuadre vertical (top|center|bottom)
    'logo' => ['radio' => 'full'],                                   // forma del logo del hero (RF-T25): none|sm|md|lg|full, INDEPENDIENTE de 'radios'. Default full = circulo (comportamiento previo)
    'textos' => ['slogan' => '', 'descripcion' => ''],               // 'descripcion' (RF-T33): panel "Informacion del comercio" de la tienda ('' = no se muestra); ya NO es una seccion propia de la home (queda a resolver del lado tienda, Fase 4 de la ronda crossmap 2)
    'redes' => ['facebook' => '', 'instagram' => ''],                // URLs ('' = sin boton en el hero)
    'catalogo' => ['layout' => 'grilla', 'categorias_plegables' => false], // layout: grilla|lista; 'categorias_plegables' (RF-T31): true = la tienda muestra los titulos de categoria como acordeon (a resolver del lado tienda, Fase 4)
    'destacados' => ['modo' => 'banner', 'adorno' => 'ninguno', 'titulo' => ''], // modo: banner|tarjeta_grande|ninguno; adorno (RF-T35): en tarjeta_grande decora la card, en banner decora el TITULO del carrusel, solo se deshabilita con modo=ninguno; 'titulo' (RF-T35): titulo de la seccion, '' = "Destacados" (a resolver del lado tienda)
    'promos' => ['mostrar_home' => false],                           // RF-T24: true = agrega las promos vigentes como ULTIMO slide de las historias destacadas (y activa el anillo del logo aunque no haya fotos). Antes de RF-T24 pintaba el aviso "Promociones de hoy" en la home (eliminado del lado core; queda a resolver del lado tienda)
]
```
Cada default de RF-T13 replica el comportamiento previo al RF: una tienda que no toca nada se ve exactamente igual que antes (portada cruda, encuadre centro, sin slogan/descripcion/redes, grilla, destacados en banner, sin promos en historias, logo redondo). `categorias_plegables=false` y `destacados.titulo=''` (RF-T31/T35) tambien replican el comportamiento previo (catalogo sin acordeon, titulo "Destacados" fijo). Un snapshot viejo de `tema` sin estas claves usa los defaults igual (tolerancia a clave ausente en ambos lados del contrato).

Catalogos cerrados (validados en `ConfiguracionTienda::guardarTienda()` y en el `GET /v1/tiendas/{slug}`): `Tienda::FUENTES_DISPONIBLES` (`system`, `inter`, `poppins`, `roboto`, `montserrat`, `lora` — self-hosted en `bcn-tienda`), `Tienda::RADIOS_DISPONIBLES` (`none`, `sm`, `md`, `lg`, `full` — reutilizado tal cual para validar `logoRadio`, RF-T25), `Tienda::DENSIDADES_DISPONIBLES` (`compacta`, `normal`, `amplia`), `Tienda::POSICIONES_PORTADA` (`top`, `center`, `bottom`, RF-T13), `Tienda::LAYOUTS_CATALOGO` (`grilla`, `lista`, RF-T13), `Tienda::MODOS_DESTACADOS` (`banner`, `tarjeta_grande`, `ninguno`, RF-T13), `Tienda::ADORNOS_DESTACADOS` (`glow`, `badge`, `ambos`, `ninguno`, RF-T13). `Tienda::MAX_HISTORIAS = 3` (RF-T24). `Tienda::COMPORTAMIENTO_DEFAULTS` esta reservado (array vacio, sin seteos en v1) — es el bloque `comportamiento` que devuelve la API. `descripcion` pasa de `max:1000` a `max:5000` (RF-T33) y `destacadosTitulo` valida `nullable|string|max:40` (RF-T35).
...
**`Tienda::historiasOrdenadas()`** (RF-T24): mapea el array `historias` (`[{id, path, orden}]`) ordenado por `orden`, a `[{id, url}]` con URL root-relative (`/storage/{path}`, mismo criterio que `logoUrl()`/`portadaUrl()`). `TiendaController::show()` la absolutiza con `url()` para la API.

**`ImagenTiendaService`** (`app/Services/ImagenTiendaService.php`, RF-T11): procesa el upload del logo, la portada y las historias destacadas, mismo patron de defensas que `ImagenArticuloService` (referencia del proyecto):
...
- Logo: `scaleDown` a maximo `LOGO_MAX_DIMENSION = 800px` (cuadrado/alto). Portada: maximo `PORTADA_MAX_ANCHO = 1600` x `PORTADA_MAX_ALTO = 900`. Historia (RF-T24): formato vertical tipo story, `HISTORIA_MAX_ANCHO = 1080` x `HISTORIA_MAX_ALTO = 1920`. Calidad WebP `85` en los tres casos.
- Nombre por UUID, path scopeado por comercio: `tiendas/{comercio_id}/{uuid}.webp` (logo/portada) o `tiendas/{comercio_id}/historias/{uuid}.webp` (historias), disco `public`. Logo/portada reemplazan (borran del disco) la imagen anterior al subir una nueva; las historias son 1:N, no se reemplazan entre si.
- Metodos logo/portada: `actualizarLogo()`, `actualizarPortada()`, `eliminarLogo()`, `eliminarPortada()`. Metodos historias (RF-T24): `agregarHistoria(Tienda, UploadedFile): array` (valida el tope `Tienda::MAX_HISTORIAS = 3` ANTES de procesar el archivo, agrega `{id, path, orden}` al final del array `historias`), `eliminarHistoria(Tienda, string $id): void` (borra el archivo del disco y RENUMERA `orden` 1..N sin huecos en las restantes), `reordenarHistorias(Tienda, array $idsOrdenados): void` (reordena `historias` segun la lista de ids recibida; ids ausentes quedan al final, conserva su posicion relativa). La tabla `tiendas` es GLOBAL (BD `config`): no hay transaccion tenant, es un `update()` de un solo registro.
- Se invoca desde `ConfiguracionTienda::guardarTienda()` (uploads de logo/portada/historias pendientes) y desde `eliminarLogo()`/`eliminarPortada()`/`eliminarHistoria()`/`moverHistoria()` del mismo componente. Mientras el upload esta pendiente (sin guardar) el preview usa `UploadedFile::temporaryUrl()`; si el archivo temporal expiro, degrada a la imagen ya persistida (logo/portada) o se omite la miniatura sin descartar el upload (historias).
...
**Estetica avanzada (RF-T13, 2026-07-18)**: nuevos controles dentro del apartado "Apariencia de la tienda" de `ConfiguracionTienda`, todos persistidos en los sub-objetos de `tema` (ver shape de `Tienda::temaCompleto()` arriba). Props del componente (RF-T13 + RF-T25 + RF-T31/T35): `portadaPosicion`, `logoRadio` (RF-T25, reemplaza a `portadaOverlay` — ver abajo), `slogan`, `descripcion`, `redFacebook`, `redInstagram`, `catalogoLayout`, `categoriasPlegables` (RF-T31), `destacadosModo`, `destacadosAdorno`, `destacadosTitulo` (RF-T35), `promosMostrarHome`.

- **Validaciones** (`guardarTienda()`): `portadaPosicion`/`logoRadio`/`catalogoLayout`/`destacadosModo`/`destacadosAdorno` con `in:` contra los catalogos cerrados del modelo (`logoRadio` valida contra `Tienda::RADIOS_DISPONIBLES`, el mismo catalogo que `radios`); `slogan` `max:120`, `descripcion` `max:5000` (RF-T33, antes `max:1000`, ambos `nullable`); `categoriasPlegables` `boolean`; `destacadosTitulo` `nullable|string|max:40` (RF-T35); `redFacebook`/`redInstagram` con regex de host (`^https://(www\.)?(facebook\.com|fb\.com)/.+` y `^https://(www\.)?instagram\.com/.+`) — rechaza URLs de otro dominio aunque tengan formato valido. `temaDesdeForm()` hace `trim()` de slogan/descripcion/redes/`destacadosTitulo` antes de persistir.
...
- **`restablecerTema()`**: desde RF-T13 el boton "Restablecer al tema default" ya NO restablece slogan/descripcion/redes (son CONTENIDO del comercio, no estetica) — arma `array_replace_recursive(Tienda::TEMA_DEFAULTS, $contenido)` preservando esos 4 campos antes de llamar `aplicarTema()`. Colores/tipografia/radios/logoRadio/densidad/posicion/layout/categorias_plegables/destacados (modo/adorno/titulo)/promos_home si vuelven al default (RF-T31/T35: `categoriasPlegables` y `destacadosTitulo` NO estan en `$contenido`, se resetean como el resto de la estetica). NO toca `historias` (RF-T24): esas se administran aparte (alta/baja/reorden inmediatos, fuera del ciclo guardar/restablecer del tema).
- **Reflejo en vivo vs. server-rendered**: `resources/js/tienda-preview.js` extiende la lista de props observadas (`$wire.$watch`) con `portadaPosicion`, `slogan`, `descripcion`, `redFacebook`, `redInstagram` — viajan por el mismo `postMessage` debounced que los tokens de RF-T12 y el mock (`tienda-preview-mock.blade.php`) los refleja con Alpine (`:style="'object-position: center ' + portadaPosicion"`, `x-show="slogan" x-text="slogan"`). `catalogoLayout`, `categoriasPlegables` (RF-T31), `destacadosModo`, `destacadosAdorno`, `destacadosTitulo` (RF-T35), `logoRadio`, `promosMostrarHome` son **server-rendered del lado bcn-tienda** (no viajan por postMessage): el cambio se ve recien al hacer clic en "Guardar tienda" (dispara `tienda-guardada` → `recargarIframe()`), documentado en el panel con el texto "Los cambios de esta seccion se ven al guardar". Las `historias` YA NO estan en esta lista (RF-T34): se guardan al instante (alta/reorden/baja) y disparan `tienda-guardada` directamente, sin esperar a "Guardar tienda" — ver la seccion de historias mas abajo.

**Historias destacadas (RF-T24) y forma del logo / portada cruda (RF-T25), 2026-07-28**: mismo apartado "Apariencia de la tienda" de `ConfiguracionTienda`.

- **Historias**: seccion "Historias destacadas" (border-t propio, entre el bloque logo/portada y los 5 colores). Prop `array $historiasActuales` (`[{id, url}]`, calculado por `mapHistorias()` en cada `cargar()`/mutacion). Ver **RF-T34** (2026-07-29, mas abajo) para el flujo de subida/reorden actual — el flujo original "pendiente hasta guardar" descripto originalmente por este RF fue REEMPLAZADO.
- **Forma del logo (`logoRadio`)**: select nuevo en el grid de estetica (junto a Tipografia/Bordes/Densidad, ahora 4 columnas en pantallas grandes). Persiste en `tema.logo.radio`, catalogo `Tienda::RADIOS_DISPONIBLES`, default `full`. Documentado en el panel como INDEPENDIENTE de `radios` (que sigue aplicando solo a tarjetas/carrito).
- **Portada cruda**: se quita el checkbox `portadaOverlay`/`wire:model.live="portadaOverlay"` del blade y la prop del componente. `temaDesdeForm()` sigue emitiendo `'portada' => ['overlay' => false, ...]` (hardcodeado, no leido de ningun input) por tolerancia de contrato — no se borra la clave para no forzar v2 del endpoint, pero ya no hay forma de setearla en `true` desde el panel.
- **API** (`TiendaController::show()`, aditivo): agrega `historias: [{id, url}]` (via `$tienda->historiasOrdenadas()`, absolutizado con `url()`) al payload de `GET /v1/tiendas/{slug}`; `tema.portada.overlay` pasa a emitirse siempre `false`; se agrega `tema.logo.radio`. Contrato actualizado en `docs/api-v1-delivery.md`.

**Historias con subida inmediata + tema plegables/titulo destacados + texto descriptivo + badges de categoria (RF-T31/T33/T34/T35/T36), 2026-07-29 — spec `.claude/specs/tienda-ronda-crossmap-2.md` Fase 3**:

- **Historias (RF-T34), subida INMEDIATA**: reemplaza el flujo "pendiente hasta guardar" de RF-T24 por el mismo patron que la galeria de fotos de articulo (`updatedFotosUpload()` de RF-T14). `updatedHistoriaUploads()` (hook de `WithFileUploads`, dispara al terminar CADA seleccion del input `multiple`) valida `historiaUploads.*` => `image|max:5120`, persiste cada archivo con `ImagenTiendaService::agregarHistoria()` (que sigue validando el tope `Tienda::MAX_HISTORIAS = 3` y lanza excepcion — capturada y mostrada con `toast-error` — si se supera) y **vacia el array `historiaUploads` al final del metodo**, sea cual sea el resultado. Vaciar el array en el propio metodo (y no solo confiar en que Livewire lo reemplace) es lo que permite **selecciones sucesivas**: si el array no se vacia, la proxima seleccion de fotos PISA el array `multiple` en vez de sumarse (gotcha ya conocido del proyecto, mismo que resolvio la galeria de articulo). Si al menos una foto se proceso OK, dispara `tienda-guardada` (recarga el visor). Se eliminan de este flujo: la regla de validacion `historiaUploads.*` en `guardarTienda()`, la validacion de tope combinado ahi mismo, el bloque `foreach` que persistia historias dentro de `guardarTienda()`, el metodo `descartarHistoriaUpload()` y la prop calculada `historiaPreviewUrls` de `render()` (ya no hay miniaturas "pendientes": todo lo que se ve en pantalla ya esta persistido).
  - **Reorden por drag & drop**: `moverHistoria(string $id, int $direccion)` (flechas ←/→) se **elimina**; lo reemplaza `reordenarHistorias(array $ids)`, que recibe el array completo de IDs en el nuevo orden (emitido por SortableJS desde el blade, `data-sortable-historias` + `initHistoriasSortable($el)` en `resources/js/tienda-preview.js`) y llama directo a `ImagenTiendaService::reordenarHistorias()` (mismo metodo de service que ya existia, solo cambia el caller). Inmediato, dispara `tienda-guardada`.
  - `eliminarHistoria(string $id)` sigue igual (inmediato, sin cambios de firma).
  - Guard extra en `updatedHistoriaUploads()`: si `$tienda->id !== $this->tiendaId` (la tienda cargada no coincide con la prop montada — defensa ante remount/cambio de sucursal a mitad de un upload en curso) tambien vacia el array y aborta sin persistir.
- **Tema: categorias plegables (RF-T31)**: `tema.catalogo.categorias_plegables` (bool, default `false`). Checkbox "Categorias desplegables" en "Presentacion del catalogo". Solo persiste la preferencia — el render como acordeon en el catalogo publico es tarea del lado `bcn-tienda` (Fase 4 de la ronda, todavia no implementada ahi).
- **Tema: titulo de destacados + adorno en banner (RF-T35)**: `tema.destacados.titulo` (string, default `''` = "Destacados" del lado tienda), input "Titulo de la seccion de destacados" (`max:40`). El selector "Adorno del destacado" deja de deshabilitarse cuando `destacadosModo !== 'tarjeta_grande'`: ahora solo se deshabilita con `destacadosModo === 'ninguno'` — en banner, el mismo catalogo `Tienda::ADORNOS_DESTACADOS` (glow/badge/ambos/ninguno) pasa a decorar el TITULO del carrusel en vez de una tarjeta (semantica nueva a implementar del lado tienda, Fase 4).
- **Texto descriptivo (RF-T33)**: el campo `descripcion` (ya existente desde RF-T13) se renombra en el label de "Texto libre (seccion propia en la pagina principal)" a **"Texto descriptivo (se muestra en «Informacion del comercio»)"** — mismo campo/columna, cambia solo su DESTINO visual del lado tienda (deja de ser una seccion de la home, pasa a integrarse al panel "Informacion del comercio", Fase 4). Limite ampliado de `max:1000` a `max:5000`, textarea de 8 filas (antes 4) en el blade.
- **Badges de categoria (RF-T36)**: ver columna `categorias.badges_tienda` (tabla arriba) y `Categoria::badgesTienda()`. UI en `ConfiguracionTiendaArticulos` (seccion "Articulos de la tienda"), espejo del editor de badges de articulo (RF-T14):
  - Props nuevas: `?int $categoriaBadges` (categoria con el editor expandido, `null` = ninguna), `array $catBadgesSel` (tipos predefinidos activos), `string $catBadgeCustom` (texto del badge propio).
  - `toggleEditorBadgesCategoria(int $categoriaId)` abre/cierra el editor (toggle si ya esta abierta la misma categoria) y precarga `catBadgesSel`/`catBadgeCustom` desde `$categoria->badgesTienda()`. `cerrarBadgesCategoria()` tambien se llama desde `onSucursalChanged()` (cambiar de sucursal cierra cualquier editor abierto, igual que `cerrarEditor()` del articulo).
  - `toggleBadgeCategoria(string $tipo)` agrega/quita del array (contra `Articulo::BADGES_TIENDA`, MISMO catalogo cerrado que articulos — no hay uno propio para categorias) y `updatedCatBadgeCustom()` valida el texto libre (`max:Articulo::MAX_BADGE_CUSTOM_LARGO` = 30, debounce 800ms); ambos llaman a `persistirBadgesCategoria()` (arma el array final y `$categoria->update(['badges_tienda' => $badges ?: null])`), con el mismo limite de `Articulo::MAX_BADGES_TIENDA = 4` validado ANTES de persistir (toast de error si se excede).
  - `categoriaVisible(int $categoriaId)`: puerta de entrada de todos los metodos de escritura de este bloque — solo permite operar sobre una categoria ACTIVA y con articulos visibles en la tienda de la sucursal activa (mismo criterio que arma los grupos del render), ignorando silenciosamente cualquier ID ajeno.
  - El header de cada grupo de categoria en el blade suma: (a) chips READ-ONLY de los badges activos (siempre visibles, sin necesidad de abrir el editor) y (b) un boton con icono de etiqueta (visible solo con `$puedeConfigurar` y `$grupo['id'] !== 0`, la categoria virtual "Sin categoria" no tiene badges) que abre el editor inline debajo del header.
  - `render()` agrega `badges` a cada grupo del listado (`$cat->badgesTienda()`, `[]` para "Sin categoria").
  - **API** (`CatalogoTiendaService::catalogo()`, aditivo): cada entrada de `categorias[]` en `GET /v1/tiendas/{slug}/catalogo` suma `badges` (via `$cat->badgesTienda()`). Mismo shape que `articulos[].badges`. Contrato actualizado en `docs/api-v1-delivery.md`; no rompe consumidores existentes (campo nuevo, aditivo).

**Banner de foto por categoria con punto focal (RF-T62, 2026-08-03)**: cada encabezado de categoria del catalogo de tienda puede mostrar una FOTO de fondo (franja panoramica con `object-cover`, scrim oscuro y nombre encima; sin foto, el encabezado queda igual que antes).

- **Campo reutilizado**: usa `categorias.imagen_path` (RF-17, existia pero nadie lo mostraba en la tienda — quedaba "dormido"). Sin columna nueva para la imagen en si; solo se definio su semantica (banner del encabezado) en el contrato.
- **Punto focal**: columnas nuevas `categorias.imagen_focal_x` / `imagen_focal_y` (ver tabla `categorias` arriba), porque la foto original casi nunca tiene la proporcion panoramica de la franja. El panel ofrece un selector de click sobre una vista previa en la proporcion REAL del banner para elegir que parte de la imagen queda visible (equivalente a `object-position`). Toda imagen NUEVA resetea el focal a 50/50 (centro) — una foto distinta no debe heredar el encuadre de la anterior.
- **`Categoria::imagenUrl()`**: nuevo metodo del modelo, espejo de `Articulo::imagenUrl()` — URL root-relativa (`/storage/{imagen_path}`, NUNCA `Storage::url()`) o `null` sin imagen.
- **`App\Services\ImagenCategoriaTiendaService`** (extiende `ImagenArticuloService` para heredar el pipeline de seguridad: finfo MIME real, whitelist jpg/png/webp, re-encode WebP q85, UUID, strip EXIF): `actualizarBanner(Categoria, UploadedFile): string` (borra el banner anterior si habia, re-encodea a WebP `1600x600` — `BANNER_MAX_ANCHO`/`BANNER_MAX_ALTO`, path `categorias/{comercio_id}/{uuid}.webp`, resetea el focal a 50/50) y `eliminarBanner(Categoria): void` (borra el archivo y limpia `imagen_path` + resetea el focal a 50/50).
- **Panel** (`ConfiguracionTiendaArticulos`, seccion "Articulos de la tienda"): boton de foto en el header de cada grupo (espejo del boton de badges RF-T36) abre/cierra un editor inline (`?int $categoriaBanner`, `toggleEditorBannerCategoria()`/`cerrarBannerCategoria()`, esta ultima tambien se llama desde `onSucursalChanged()` como el resto de los editores inline). Con banner cargado, muestra la vista previa recortada en la proporcion de la franja con un pin de focal encima; `guardarFocalBanner(int $categoriaId, $x, $y)` persiste el click (clamp 0-100 en ambos ejes) y dispara `catalogoCambiado()`. `updatedBannerUpload()` (prop `array $bannerUpload`, keyed por `categoria_id`, `wire:model="bannerUpload.{id}"`) procesa la subida AL INSTANTE via el service (mismo patron que la galeria de articulo RF-T14); `eliminarBannerCategoria(int $categoriaId)` quita el banner. Todos los metodos de escritura pasan por `categoriaVisible($id)` (misma puerta de entrada que badges). `render()` agrega `banner_url`/`banner_focal_x`/`banner_focal_y` a cada grupo (`null`/`50.0`/`50.0` para "Sin categoria").
- **Via vieja unificada**: `GestionarCategorias` (Articulos → Categorias, modal de alta/edicion) tambien pasa por `ImagenCategoriaTiendaService` (antes guardaba el archivo subido crudo con `store()`, sin validar el MIME real ni re-encodear) e invalida el cache del catalogo publico de TODAS las sucursales del comercio tras subir/quitar la imagen (`CatalogoTiendaService::invalidarCache()` — las categorias son globales al comercio, no por sucursal).
- **API** (`CatalogoTiendaService::catalogo()`, aditivo): `categorias[].imagen_url` (ya existia en el contrato) pasa a tener semantica de BANNER definida; se suma `categorias[].imagen_focal: {x, y}` (porcentajes 0-100, default `{50, 50}`, viaja SIEMPRE con o sin imagen). Contrato actualizado en `docs/api-v1-delivery.md`; no rompe consumidores existentes (campo nuevo, tolerancia a ausencia si el core es viejo).
...
- **Canje por puntos en la tienda** (RF-T47, 2026-07-30; costo ajustado RF-T54, 2026-08-03): `toggleCanjeTienda()` invierte `canje_tienda` del pivot `articulos_sucursales` de la sucursal ACTIVA (no una columna del articulo), sin exigir nada mas. Con el canje prendido aparece un **input numerico provisorio** junto a la estrella que persiste `articulos.puntos_canje` via `guardarPuntosCanje()` (campo GLOBAL del articulo, misma columna que guarda el POS en sus items; vacio/0 = null, placeholder "Auto"). Con puntos cargados ese es el costo del canje en la tienda; **vacio, el costo se deriva del precio del dia** (`ceil(precio / valor_punto_canje)`, la regla con la que canjea el POS). Ver seccion 3.14 "Canje de articulos por puntos en la tienda" mas abajo para el circuito completo (catalogo, cotizacion, alta).
...
#### Restriccion por forma de pago con pago dividido — regla espejo de cupones (RF-01/02, 2026-07-24)

Promociones (`Promocion`/`PromocionEspecial`) y listas de precios condicionadas por forma de pago siguen ahora la MISMA regla anti-abuso que ya tenian los cupones (`cupon_formas_pago`, ver arriba): con un pago dividido en dos formas de pago, una restriccion por FP aplica **solo si TODAS las FP declaradas del pago estan habilitadas**. Antes solo se validaba `pagos[0]` (o la FP unica), lo que permitia declarar la FP restringida primero y "colar" una segunda FP no habilitada.

- **`WithCalculoVenta::formasPagoContexto(): array`** (`app/Livewire/Concerns/Carrito/WithCalculoVenta.php`): arma el set COMPLETO de FP declaradas para el contexto de beneficios. Default: el set del desglose armado (`$this->desglosePagos`, si el host usa `WithPagosDesglose`), o la FP unica seleccionada si no hay desglose. `CotizadorCarritoTienda` (tienda) lo overridea para devolver `$this->formasPagoIds` (las FP declaradas por el consumidor).
- **`WithCalculoVenta::formasPagoCumplenRestriccion(array $contexto, array $fpIdsPermitidas): bool`**: normaliza `$contexto['formas_pago_ids']` (set nuevo) o `$contexto['forma_pago_id']` (FP unica legacy) y evalua con `array_diff` que TODAS las declaradas esten en `$fpIdsPermitidas` (comparacion no estricta via `intval`, los ids pueden llegar como string). Sin ninguna FP declarada, la restriccion NUNCA se cumple. Usado por `promocionCumpleCondiciones()` y `promocionEspecialCumpleCondiciones()` en vez de comparar solo `$contexto['forma_pago_id']`.
- **`ListaPrecioCondicion::evaluarFormaPago(array $contexto)`**: misma regla — con `$contexto['formas_pago_ids']`, TODAS las FP declaradas deben coincidir con la FP de la condicion (`foreach` que corta en el primer mismatch). Con una sola FP el comportamiento es identico al historico (comparacion directa).
- El resultado es **orden-independiente**: declarar la FP restringida primero o segunda da el mismo resultado (antes, con pago dividido, el orden podia cambiar si la promo aplicaba o no).
- **Revalidacion reactiva en el panel** (`WithPagosDesglose::revalidarBeneficiosPorFormasPago()`, hook llamado desde `agregarAlDesglose()`, `eliminarDelDesglose()` y `cerrarModalPago()` al vaciar el desglose): cada vez que cambia el SET de FP del desglose, se vuelve a llamar `calcularVenta()` (que usa `formasPagoContexto()` con el desglose actualizado) y el DELTA de `total_final` resultante se suma/resta a `montoPendienteDesglose` — una promo que cae por agregar una segunda FP no habilitada agranda lo que falta cubrir; si se elimina esa FP, la promo puede volver a aplicar y el pendiente se achica. Vetado en cobro rapido (`puedeRevalidarBeneficiosPorFormasPago()`, no hay carrito que recalcular sobre un saldo de pedido ya confirmado).
- **Tienda** (`CotizadorCarritoTienda`, multi-pago RF-T18): `CotizacionController`/`PedidoTiendaService` arman el array de TODAS las FP de `pagos[]` (antes solo tomaban `pagos[0]`) y se lo pasan a `cotizar(..., int|array|null $formaPago)`; con una unica FP se sigue habilitando ademas el camino single-FP del ajuste (`formaPagoId`), con dos el ajuste sale exclusivamente de `desglosarPagos()`. Ver seccion "Multi-pago en la tienda" mas abajo para el detalle completo del contrato.
...
El renglon de envio **NO participa** de descuento general, cupones, promociones, puntos ni del ajuste por forma de pago (hook `baseAjustePagoDesglose` lo excluye de la base sobre la que se calcula el ajuste/recargo de cuotas): se calcula y suma aparte de toda la cascada de beneficios. La forma exacta en que se excluye de la base del ajuste con pago dividido es la regla "bienes-primero con tope" (RF-03, ver subseccion siguiente) — reemplaza al prorrateo proporcional original.

#### Modalidad habilitada por sucursal — delivery vs take-away (RF-T30, 2026-07-29)

`sucursales.usa_delivery` (boolean) es el **modulo de pedidos** (maestro): sin el, el panel de Pedidos Delivery, la tienda online y la card del marketplace no funcionan para esa sucursal (el nombre visible cambio de "Habilitar delivery en esta sucursal" a "Habilitar modulo de pedidos" en el panel, sin migracion — mismo campo). Dentro del modulo, DOS keys de `config_delivery` deciden que MODALIDADES ofrece, en paridad:

- **`delivery_habilitado`** (bool, default `true`, `Sucursal::CONFIG_DELIVERY_DEFAULTS`) — envio a domicilio.
- **`takeaway_habilitado`** (bool, default `true`, preexistente) — retiro en el local.

Una sucursal solo-take-away (`delivery_habilitado=false`) es legitima; el caso simetrico (solo delivery) tambien.

- **Validacion al alta** (`PedidoDeliveryService::validarTipoContraSucursal(Sucursal, string $tipo)`): primero exige `usa_delivery` (modulo maestro); luego, contra `DeliveryEnvioService::configDelivery($sucursal)`, rechaza `tipo=delivery` si `! $config['delivery_habilitado']` y `tipo=take_away` si `! $config['takeaway_habilitado']` (mismo `Exception`, capturada por Livewire/API igual que cualquier otra validacion del alta).
- **Pedidos de tienda/API** (`PedidoTiendaService::crearPedidoExterno()`): la MISMA regla, espejo — `tipo=delivery` con `delivery_habilitado=false` → `Exception` (422 "El delivery esta deshabilitado en esta sucursal"); `tipo=take_away` con `takeaway_habilitado=false` → mensaje equivalente ya existente. `carrito/cotizar` **NO valida modalidad** (cotiza igual sea cual sea el tipo) — el bloqueo es exclusivo del alta (`POST /pedidos`).
- **Panel** (`NuevoPedidoDelivery`): `cargarConfiguracionSucursal()` carga `deliveryHabilitado` desde `config['delivery_habilitado'] ?? true` y, si el tipo cargado quedo sin modalidad valida (por ejemplo se desactivo delivery mientras habia un borrador de ese tipo), lo corrige a la modalidad restante habilitada. El segmented control Delivery/Take-away oculta directamente el boton de la modalidad apagada (`@if($deliveryHabilitado || $tipo === 'delivery')` / equivalente para take-away); `updatedTipo()` bloquea ademas un intento de seleccion programatica (`$set('tipo', ...)`) de la modalidad apagada, revirtiendo el valor y emitiendo un toast de error.
- **API aditiva** (`docs/api-v1-delivery.md`, congelado — cambio ADITIVO, no rompe v1): `delivery_habilitado` (bool) se suma en `GET /v1/tiendas/{slug}` (`TiendaController`), en la card de `GET /v1/tiendas` del marketplace (`MarketplaceTiendasService::listar()`, mismo snapshot cacheado `marketplace_tienda_{id}` ~5 min) y en `GET /v1/delivery/config` (`ConfigController`, integraciones). Campo AUSENTE en una respuesta vieja/cacheada ⇒ la tienda/integracion debe asumir `true` (retrocompatibilidad; el snapshot del marketplace cacheado antes del deploy de esta feature no trae la key hasta que expire el TTL).
- **Auto-encendido del modulo al publicar la tienda** (`ConfiguracionDelivery::toggleTiendaOnline()`, fix de bug estructural): antes de este cambio, publicar la tienda online con `usa_delivery=false` la dejaba publicada (`habilitada=true`) pero **inaccesible** (404), porque el middleware de la API valida `activa && usa_delivery`. Ahora, al publicar, si `usa_delivery` estaba apagado se enciende junto con la publicacion (`$sucursal->update(['usa_delivery' => true])` + toast "Se habilito el modulo de pedidos (la tienda online lo necesita)"), y se invalida el cache `marketplace_tienda_{id}` (`Cache::forget`) para que la card del marketplace refleje el alta sin esperar el TTL de 5 minutos. Si se apaga el modulo de pedidos con la tienda YA publicada (`tiendaPublicadaPersistida=true`), la vista muestra un aviso rojo "La tienda online quedara fuera de linea" (advertencia, no bloquea el guardado).

#### Palabra clave de entrega (RF-T64, 2026-08-03)

Feature opcional de seguridad en la puerta: valida que quien recibe el pedido es efectivamente el cliente, sin depender de nada mas que decir una palabra.

- **Config** (`config_delivery.usar_palabra_clave`, bool, default `false`, `Sucursal::CONFIG_DELIVERY_DEFAULTS`): checkbox "Palabra clave de entrega" en `ConfiguracionDelivery` (apartado General, junto a `exigirRepartidor`), en la whitelist de auto-guardado (`PROPS_AUTOGUARDADO`).
- **Generacion** (`PedidoDeliveryService::asignarPalabraClaveEntrega(PedidoDelivery, Sucursal)`): se dispara en los dos puntos donde un pedido delivery pasa a CONFIRMADO -- `crearPedido()` (pedidos que nacen confirmados: panel, aceptacion automatica) y `confirmarBorrador()` (aceptacion manual de pedidos externos). Condiciones: `tipo === 'delivery'` (take-away no genera), sucursal con `usar_palabra_clave` activo, y **sin palabra previa** -- idempotente, una reconfirmacion no la rota. La palabra sale al azar (`array_rand`) de la lista fija `PedidoDeliveryService::PALABRAS_CLAVE_ENTREGA` (~38 sustantivos simples, sin acentos ni ambiguedad fonetica: Tigre, Luna, Sol, Rio, etc.), sin garantia de no-repeticion entre pedidos.
- **Persistencia**: `pedidos_delivery.palabra_clave_entrega` (ver tabla arriba).
- **Panel** (`pedidos-delivery.blade.php`, modal "Ver detalle"): si el pedido tiene palabra generada, se muestra junto al repartidor y su telefono con un badge indigo "Palabra clave: {palabra}".
- **Seguimiento publico y broadcast** (RF-T63, ver seccion 3.15): la palabra viaja en el GET y en `SeguimientoActualizado` con la MISMA regla -- solo con `repartidor_id` asignado (no antes: no aporta) y pedido NO cancelado (se corta la vigencia); `null` si la sucursal no usa el feature o el pedido no la genero.

#### Base del ajuste por FP en pago dividido — bienes-primero con tope (RF-03, 2026-07-24)

Con el envio pisado como valor FIJO (sin descuentos/recargos por FP, ver arriba), la pregunta es: cuando el pago se divide en dos formas de pago, ¿que porcion de CADA pago es "base de ajuste" (bienes) y que porcion es "envio" (no ajustable)? La version anterior prorrateaba el envio PROPORCIONALMENTE entre los pagos (`factorBase = (total - envio) / total`, la base de cada FP era `monto * factorBase`); esto distribuia el descuento de una FP tambien sobre una porcion de envio no declarada como tal, y el resultado dependia de como el operador repartia los montos entre pagos.

- **`App\Services\Pedidos\AsignadorBasesAjustePagos::asignar(array $pagos, float $bienes): array`** (service PURO, sin BD/transacciones/estado): recibe una lista de pagos `{monto, ajuste_porcentaje}` y el total de BIENES a repartir (sin envio) y devuelve los mismos pagos, en el MISMO orden, con `base_ajuste` agregada. Algoritmo:
  1. Ordena los pagos por `ajuste_porcentaje` ASCENDENTE (mayor descuento primero, recargo al final) — a igual ajuste, orden de declaracion. Greedy, determinista, **orden-independiente** en el resultado final.
  2. A cada pago (en ese orden) le asigna `base_ajuste = min(monto, bienes_restantes)` y descuenta esa base de `bienes_restantes`.
  3. Invariante: `Σ base_ajuste ≤ bienes` — dos FP con descuento nunca descuentan dos veces sobre la misma mercaderia, y ninguna FP recibe ajuste sobre una porcion de envio.
  4. Con un pago unico que cubre todo el total, `base_ajuste = bienes` (identico al calculo single-FP historico).
  - Ejemplo: bienes $1000 + envio $500 (total $1500), pago 1 = efectivo $900 (-10%), pago 2 = transferencia $600 (0%). Orden de asignacion: efectivo primero (ajuste mas negativo) → `base_ajuste efectivo = min(900, 1000) = 900` → `bienes_restantes = 100` → `base_ajuste transferencia = min(600, 100) = 100`. Ajuste efectivo = `900 * -10% = -90` (antes, con prorrateo proporcional, hubiera sido `-60`).
- **Compartido por los DOS canales** que arman un pago dividido, para que den EXACTAMENTE el mismo total (principio de paridad tienda/panel):
  - **Tienda**: `CotizadorCarritoTienda::desglosarPagos()` arma la lista de pagos, llama `AsignadorBasesAjustePagos::asignar($pagos, $totalACubrir - $costoEnvio)` y recien despues calcula `monto_ajuste`/`monto_final`/`vuelto` de cada pago con la `base_ajuste` recibida.
  - **Panel delivery**: `NuevoPedidoDelivery::reasignarBasesAjustePagosDesglose()` (override del hook `WithPagosDesglose::reasignarBasesAjustePagosDesglose()`, no-op en venta/mostrador donde no hay envio) corre dentro de `recalcularTotalConAjustes()` cada vez que el desglose cambia: re-asigna las bases de TODO el desglose con la misma regla y recalcula `monto_ajuste`/`monto_final`/`monto_recargo_cuotas`/`vuelto` de cada fila. `WithPagosDesglose::baseAjustePagoDesglose()` (hook que corre al AGREGAR un pago nuevo al desglose) da solo una primera aproximacion con los bienes aun sin asignar; la fuente de verdad final es `reasignarBasesAjustePagosDesglose()`.
- **UX**: el modal de pago mixto y el resumen de totales muestran la base de cada ajuste ("-10% s/ $900 de articulos") y aclaran cuando un pago no recibe ajuste por cubrir solo envio — ver seccion 3.1/15.4 del manual de usuario (RF-04).
...
#### Burbuja flotante + correccion de direccion desde el detalle + mapa con contexto de reparto (spec `delivery-burbuja-y-mapa`, 2026-08-04)

Reemplaza la banda naranja fija de "por aceptar" por una burbuja flotante en escritorio y agrega la posibilidad de corregir la direccion de un pedido YA cargado (con recotizacion), ademas de dar contexto visual al picker de domicilio en todo el circuito de delivery.

**Correccion de direccion — `PedidoDeliveryService::corregirDireccion()` / `previsualizarCorreccionDireccion()`**

Distinto de `establecerDireccion()` (alta/edicion completa del pedido): corrige un pedido YA existente desde el modal "Ver" del panel (`PedidosDelivery`).

- Guardas de estado (en `corregirDireccion()` y espejadas server-side en `PedidosDelivery::pedidoDireccionEnEdicion()` para re-validar si el estado avanzo mientras el modal estaba abierto): solo `tipo=delivery`; rechaza (`Exception`) si `estado_pedido` es `entregado`, `facturado` o `cancelado` — la direccion ya es historica.
- `previsualizarCorreccionDireccion(PedidoDelivery, ?lat, ?lng): ?array` calcula el DELTA antes de guardar, **sin persistir nada**. Devuelve `null` (nada que confirmar, se guarda directo) cuando:
  - el pedido ya tiene pagos (`estado_pago !== ESTADO_PAGO_PENDIENTE`) — el costo de envio nunca se toca con cobros ya registrados;
  - `georreferenciar_pedidos` esta apagado o la sucursal no tiene lat/lng cargadas — misma condicion que ya exige `cotizar()`;
  - la zona y el costo resultantes coinciden con los actuales (tolerancia $0.01).
  - Si hay algo que cambia, devuelve `{alcance: 'ok'|'fuera', zona_antes, zona_despues, costo_antes, costo_despues, distancia_km, costo_manual}`. Con `costo_manual=true` el costo NO se recalcula (mismo espejo de `recotizarEnvio()`), solo zona/distancia informativas.
  - Fuera de alcance (`CotizacionEnvio::ALCANCE_FUERA`): `zona_despues=null`, `costo_despues=costo_antes` (se conserva si el operador confirma "Guardar igual").
- `corregirDireccion(PedidoDelivery, array $datos): PedidoDelivery` persiste `direccion_entrega`/`direccion_referencia`/`localidad_entrega_id`/`latitud`/`longitud` en transaccion `pymes_tenant`; si `estado_pago === ESTADO_PAGO_PENDIENTE` recotiza (`recotizarEnvio()`) DESPUES de guardar la direccion — con pagos ya registrados, guarda SOLO la direccion.
- **Livewire** (`PedidosDelivery`, suma el trait `ManejaDomicilio` con `setCoordenadasDesdeMapa` sobreescrito para limpiar el preview pendiente cada vez que se mueve el pin): `abrirEditarDireccion()` cierra el detalle y precarga el form de domicilio (respeta el patron de UN modal a la vez del componente, igual que `abrirModalEditarPedido`); `guardarDireccion()` llama a `previsualizarCorreccionDireccion()` — si devuelve algo, lo guarda en `$direccionPreview` y espera `confirmarGuardarDireccion()` (el operador ve el delta); si `null`, persiste directo. `cerrarEditarDireccion()` reabre el detalle (`verDetalle`) al cerrar o tras guardar. Los tres campos nuevos (`showDireccionModal`, `pedidoDireccionId`, `direccionPreview`) se suman a `resetEstadoComponente()` (cierre de modales al cambiar sucursal/caja).

**Mapa con contexto de reparto — `DeliveryEnvioService::mapaPayload()`**

`mapaPayload(Sucursal): array` arma `{zonas: [{id, nombre, poligono, activo}], radioKm, centro: {lat,lng}|null}` — extrae la logica que antes vivia duplicada en `ConfiguracionDeliveryEnvio::zonasMapaPayload()`, que ahora delega ahi y solo agrega `radioKm` con el valor del FORM (puede diferir del guardado mientras se edita la configuracion). Lo consumen 3 lugares: el mapa de config de zonas (preexistente), el modal de alta de pedido (`NuevoPedidoDelivery::getMapaContextoEntregaProperty()`) y el modal de correccion de direccion (`PedidosDelivery::getMapaContextoDireccionProperty()`) — estos dos ultimos solo calculan el payload si el modal esta abierto Y `georreferenciar_pedidos` esta activo (evita la query en cada render normal del componente).

- `domicilio-form.blade.php` recibe el payload en la prop `$mapaContexto` (default `null`) y lo inyecta en un `<script type="application/json" x-ref="mapaContexto">` — **fuera** del `x-data`: interpolar un payload dinamico DENTRO del x-data reinicializaria Alpine en cada morph de Livewire (gotcha ya conocido, `project_gotcha_xdata_dinamico_reinit`). `domicilio-mapa.js::dibujarContexto()` lo lee una vez, al construir el mapa (`contexto()`, con try/catch por si el JSON esta vacio o corrupto), y dibuja: pin naranja del LOCAL (`crearPinLocal()`, mismo diseno de gota con icono BCN que ya usaba `zonas-mapa.js`), circulo gris punteado del radio general (solo si NO hay zonas dibujadas activas) y poligonos de zonas activas con su nombre en el centroide (paleta ciclada `COLORES_ZONAS`, exportada desde `domicilio-mapa.js` y reusada por `zonas-mapa.js` para que una zona tenga el MISMO color en los dos mapas). Es **solo visual**: no decide nada, la cotizacion real la sigue haciendo `cotizar()` en el backend — duplicar el ray-casting en JS seria una segunda fuente de verdad.
- El pin del domicilio (el punto que se esta editando) dejo de usar `crearPin()` (gota naranja con icono PWA, funcion eliminada) y pasa a ser el marcador **rojo ESTANDAR** de Google Maps (`AdvancedMarkerElement` sin `content` custom) — distingue visualmente "esto es lo que estas moviendo" del pin naranja del local. Sigue siendo arrastrable.
- Nueva prop `$conGeolocalizacion` (bool, default `true`) en `domicilio-form`: oculta el boton "Usar mi ubicacion actual" cuando la direccion es de OTRA persona. Se pasa en `false` en: alta/edicion de pedido delivery, el modal nuevo de correccion de direccion y domicilio de clientes. Sigue en `true` (default, sin tocar) en: sucursales, integraciones MP y domicilio fiscal del CUIT — ahi el operador SI esta parado en el lugar que georreferencia. El metodo `usarMiUbicacion()` del JS queda intacto, simplemente sin uso desde los flujos donde se apago.
- El modal de alta de pedido delivery ya NO fuerza `mapaAutoAbrir=true` (se quito la prop del `@include`): el mapa nace **cerrado** con el boton "Abrir mapa", igual que en el resto del sistema. Sin tocar ese boton, cero llamadas a `maps.googleapis.com`.

**Burbuja flotante de pedidos por aceptar (solo escritorio)**

La banda naranja de `pedidos-delivery.blade.php` se mantiene, sin cambios de fondo, PERO ahora solo se renderiza en **movil** (`sm:hidden`) como bloque compacto plegable en el flujo de la pagina. En **escritorio** (`hidden sm:block`) se agrega una burbuja flotante que reemplaza esa funcion:

- `Alpine.data('burbujaPorAceptar', config)` (`resources/js/burbuja-por-aceptar.js`, registrado en `bootstrap.js`): `x-data` ESTATICO — solo recibe la `clave` de localStorage (`bcn-burbuja-pa-{comercio_activo_id}-{sucursalActual()}`, cambia unicamente al cambiar de sucursal) para sobrevivir a los morphs del `wire:poll` sin re-inicializarse (mismo gotcha de payload dinamico en x-data).
- Cerrada: pildora `<button data-burbuja-por-aceptar>` con SOLO la cantidad (`$pedidosPorAceptar->count()`) y el rotulo "por aceptar", roja si `$hayAceptacionDemorada` (algun pedido supero `timeout_aceptacion_min`) o naranja si no. El `@if($pedidosPorAceptar->isNotEmpty())` que envuelve todo el bloque hace que ni la burbuja ni el bloque movil se rendericen con 0 pedidos.
- Arrastre (`empezarDrag(ev)`): distingue click de arrastre por umbral de 6px (`Math.abs(dx)+Math.abs(dy) <= 6`, listeners `pointermove`/`pointerup` en `window`); al soltar, `anclar(x,y)` calcula el borde mas cercano (distancia de `(x,y)` a cada uno de los 4 bordes, midiendo el superior contra `safeTop`) y la fraccion 0..1 a lo largo de ese borde, persistidos en `localStorage[clave]` como `{borde, offset}`. Los getters `anclaX`/`anclaY` re-acotan esa fraccion contra `vw`/`vh` (actualizados por un listener de `resize`), asi que la posicion es resize-safe sin tocar lo guardado. El estilo (`estiloBurbuja`) se aplica via `:style`, que Livewire preserva en cada morph.
- **GOTCHA CENTRAL — `:style` con string PISA el `display:none` de `x-show`** (causa raiz del "se abre al soltar el arrastre y no se cierra con nada", diagnosticado en vivo el 2026-08-04): Alpine aplica un `:style` que devuelve **string** con `setAttribute('style', …)`, reescribiendo el atributo ENTERO — y ahi vive el `display:none` que puso `x-show`. Al soltar el arrastre cambian `borde`/`offset` → se recalcula `estiloPanel` → el panel **aparece** con `abierto === false`. Y como el estado ya era `false`, la X / `Escape` / `@click.outside` lo ponen `false → false`: sin cambio de estado, `x-show` no vuelve a correr y el `display:none` NUNCA regresa (panel visible e insensible). Sintoma verificable: `getComputedStyle(panel).display === 'flex'` con `Alpine.$data(burbuja).abierto === false`. **Fix**: `estiloBurbuja`/`estiloPanel` devuelven un **objeto** — Alpine lo aplica con `setProperty` propiedad por propiedad y no toca `display`. Declarar SIEMPRE todas las claves de posicion (`left/right/top/bottom`, con `auto` donde no apliquen) para que no quede pegada la del borde anterior. Regla general: **cualquier `:style` dinamico sobre un elemento con `x-show` va como objeto, nunca como string** (hermano del gotcha de estilos inline perdidos en el morph).
- Supresion del click post-arrastre: el browser dispara un `click` sintetico despues del `pointerup`; con una bandera de instancia no alcanza (al soltar, la burbuja salta al borde y el click cae en otro elemento). `soltar()` registra un listener de `click` **en fase de captura** que hace `stopPropagation()+preventDefault()` y se remueve en ese mismo click, con timeout de respaldo a 400ms (un `setTimeout(…, 0)` le ganaba de mano al click, que Chrome despacha en un task aparte). Tambien se escucha `pointercancel`, si no los listeners quedaban vivos y `arrastrando` en `true`.
- Click sin arrastre (`clickBurbuja()`) expande la burbuja en una **tarjeta flotante** anclada al mismo punto (`estiloPanel`: ancho ~352px acotado a la ventana, `max-height` calculada contra la franja segura, `transform-origin` en el borde/esquina de la burbuja). La tarjeta **solo existe en el DOM mientras esta abierta** (`x-show` + `x-transition` de escala): antes vivia siempre en el DOM con `transition-transform`, y al cambiar la burbuja de borde se veia la ventana cruzando la pantalla. Cierra con la X, `@keydown.escape.window` y `@click.outside` (la burbuja usa `@click.stop` para que abrir no dispare el outside en el mismo evento).
- **Apilamiento**: el widget va en `z-50` y el navbar en `relative z-40`, asi que la burbuja puede anclarse al margen superior REAL (por encima del menu) y seguir siendo clickeable. El panel viejo era `z-40` — empataba con el navbar y por eso su X quedaba inalcanzable anclado arriba. La burbuja va pegada al borde (4px); la tarjeta conserva 12px.
- **Aviso periodico** (`AVISO_MS = 5000`): mientras el componente exista (o sea, mientras haya pedidos por aceptar) suena un chime sintetizado con WebAudio — mismo enfoque que `llamador.js`, sin archivos de audio — y la burbuja vibra (`.bcn-vibra`, 600ms); el contador late siempre (`.bcn-late`). Guard con `document.hidden` (en segundo plano el browser frena los timers y sonaria a destiempo) y desbloqueo del `AudioContext` con el primer `pointerdown`/`keydown` de la pagina (politica de autoplay). Los `setInterval`/`setTimeout` se limpian en `destroy()`.
- **La posicion va en `translate`, no en `transform`**: son propiedades CSS independientes, asi que la animacion de vibrado (que usa `transform`) no pisa el centrado `-50%` de la burbuja. Los keyframes viven en `resources/css/app.css` (CSS plano: Tailwind no los purga y las clases se aplican desde `:class` de Alpine).
- La fila de cada pedido (`por-aceptar-lista.blade.php`) va en **columna** (datos con `truncate` arriba, monto + acciones abajo): en la tarjeta angosta, el `flex-wrap` con `flex-1` estrangulaba el texto hasta partirlo letra por letra.
- El panel (desktop) y el bloque plegado (movil) comparten el mismo partial de lista, `resources/views/livewire/pedidos/partials/por-aceptar-lista.blade.php` (extraido del markup inline que antes vivia solo en la banda), con los botones Ver (`verDetalle`)/Aceptar (`abrirAceptar`)/Rechazar (`abrirRechazar`) sin cambios de firma — el `render()` sigue pasando `pedidosPorAceptar` igual que antes, por lo que los tests de aceptacion existentes (`SmokePedidosDeliveryTest`) no requirieron tocarse.
- El listener del destello (`@pedido-por-aceptar.window`, evento ya emitido por `onPedidoBroadcast`) se replica igual en el bloque movil y en la burbuja de escritorio — cada `x-data` es independiente, no comparten estado entre si.
- La posicion de la burbuja vive SOLO en `localStorage` del navegador (preferencia de puesto de trabajo: en que monitor/borde le queda comodo a ESE operador), nunca viaja al servidor ni se persiste en BD — no requiere migracion.
...
#### Canje de articulos por puntos en la tienda — costo configurado o derivado (RF-T47, 2026-07-30; costo ajustado RF-T54, 2026-08-03)

Un articulo es canjeable por puntos EN LA TIENDA cuando se cumplen 2 condiciones a la vez: el comercio prendio `canje_tienda` (pivot `articulos_sucursales` de ESA sucursal, toggle en el panel — ver "Configuracion de tienda POR ARTICULO" mas arriba) y el programa de puntos esta activo para el cliente. El COSTO es el configurado del articulo (`articulos.puntos_canje`, la MISMA columna que guarda el POS en sus items) si esta cargado; **sin configurar se deriva del precio del dia** (`ceil(precio / valor_punto_canje)`) — que es exactamente la regla con la que el POS canjea articulos (`WithPuntos::calcularPuntosCanjePorPrecio()` deriva SIEMPRE del precio; `articulos.puntos_canje` hoy no tiene UI de mantenimiento propia, solo el input provisorio del panel de tienda). *(Historia: la version original de RF-T54 del 2026-07-31 exigia `puntos_canje > 0` por una supuesta "paridad POS" — premisa equivocada, ajustada el 2026-08-03.)*

- **Panel** (`ConfiguracionTiendaArticulos`): con el canje prendido aparece un input numerico provisorio junto a la estrella que persiste `articulos.puntos_canje` via `guardarPuntosCanje()` (vacio/0 = null = costo derivado, placeholder "Auto"). Es un campo GLOBAL del articulo (lo comparte el POS), no por sucursal.
- **Catalogo** (`GET /v1/tiendas/{slug}/catalogo`, `CatalogoTiendaService::catalogo()`): cada articulo suma `puntos_canje` (int, el costo EFECTIVO: configurado o derivado) solo si toggle + programa activo, ademas de precio > 0 y no agotado. Ausente ⇒ no canjeable, la tienda no muestra nada.
- **Cotizacion** (`POST /v1/tiendas/{slug}/carrito/cotizar`, requiere Bearer con cliente): `items[].canjear_con_puntos` marca el renglon como canjeado. `CotizadorCarritoTienda::construirItem()` rechaza el renglon con `Exception` si `canje_tienda` esta apagado (o si no hay valor de canje activo y el articulo tampoco tiene puntos configurados); ademas exige cantidad 1 y sin opcionales con precio (un opcional pago quedaria regalado). El costo efectivo queda en `item['puntos_canje']`; `puntosUsadosEnArticulos()` (ya NO recibe `valor_punto_canje` como parametro) lo suma x cantidad y ese total se valida contra el saldo del cliente. El bloque `puntos` de la respuesta suma `usados_en_articulos` con este mismo total.
- **Alta** (`POST /v1/tiendas/{slug}/pedidos`, `PedidoTiendaService`): `construirDetalles()` (ya NO recibe `valorPuntoCanje` como parametro) persiste `pedidos_delivery_detalle.puntos_usados` con el costo efectivo que el cotizador dejo en el item x cantidad; la conversion a venta arma el `MovimientoPunto` canje-articulo desde ese valor, igual que el panel.

#### Multi-pago en la tienda — hasta 2 formas de pago (RF-T18 F2, 2026-07-21; semantica de precio RF-01/02/03, 2026-07-24)

Contrato ADITIVO: `pago.{forma_pago_id, paga_con}` singular sigue funcionando (1 FP); si viaja `pagos[]`, el singular se ignora. **TODAS las FP declaradas participan del precio** (promos/listas condicionadas por FP + restriccion de cupon evaluan contra el set COMPLETO, regla espejo de los cupones — ver seccion "Restriccion por forma de pago con pago dividido" mas arriba); el resultado es orden-independiente. *(Antes de 2026-07-24: solo `pagos[0]` era la FP "principal" que participaba del precio y el resultado dependia del orden declarado — comportamiento corregido, no es un cambio de shape del contrato.)*

- **`CotizadorCarritoTienda::MAX_PAGOS_TIENDA = 2`**, propiedad **`$formasPagoIds: array`** (el set COMPLETO de FP declaradas, alimenta `formasPagoContexto()` — ver arriba) y **`cotizar(..., int|array|null $formaPago = null)`**: acepta una FP unica (int, retrocompatible) o el array completo del multi-pago; con una unica FP en el array, `formaPagoId` tambien queda seteado (habilita ademas el camino single-FP del ajuste, `WithAjusteFormaPago`); con dos, el ajuste sale exclusivamente de `desglosarPagos()`. `CotizacionController`/`PedidoTiendaService` arman ese array desde `pagos[].forma_pago_id` (antes tomaban solo `pagos[0]`).
- **`desglosarPagos(Sucursal, array $pagosInput, float $totalACubrir, float $costoEnvio = 0.0): array`** (`app/Services/Pedidos/CotizadorCarritoTienda.php`): valida (entre 1 y 2 FP, sin IDs repetidos, suma de montos = `$totalACubrir` con tolerancia de redondeo `0.05`), arma cada pago con su `ajuste_porcentaje` (override de `formas_pago_sucursales.ajuste_porcentaje` > general) y delega en **`AsignadorBasesAjustePagos::asignar()`** (RF-03, ver subseccion "Base del ajuste por FP en pago dividido" mas arriba) la asignacion de la base de BIENES de cada FP antes de calcular `monto_ajuste`/`monto_final` — misma regla que el panel (`WithAjusteFormaPago`/`WithPagosDesglose`). Cada FP declarada debe pasar `FormaPago::esDeclarableEnTienda($sucursalId)` (mismo filtro que RF-T18 F1); si no, `Exception` (422). Devuelve `list<{forma_pago_id, nombre, monto_base, ajuste_porcentaje, monto_ajuste, monto_final, permite_vuelto, paga_con, vuelto}>`.
- **Un pago SIN monto cubre el resto** (aditivo 2026-07-22): `pagos.*.monto` pasa a `nullable` (antes `required`) tanto en `carrito/cotizar` como en el alta. A LO SUMO una FP del array puede venir sin `monto` (ausente, `null` o `''`); `desglosarPagos()` la detecta (`$sinMonto`), suma los montos EXPLICITOS de las demas y le asigna `monto = $totalACubrir - $sumaExplicitos` a esa FP — si el resto da `<= 0` (los montos explicitos ya cubren o superan el total), `Exception` (422 "Los montos de las formas de pago no suman el total del pedido"). Mas de una FP sin monto → `Exception` (422 "Solo una forma de pago puede ir sin monto (cubre el resto)"). Con una unica FP en `pagos[]` y sin monto, cubre el 100% (equivalente a no mandar `monto` en el pago singular). Refuerza la regla de oro: la tienda nunca necesita calcular "cuanto falta" para la segunda forma de pago de un pago mixto, el core lo resuelve.
- **Base del ajuste = bienes-primero con tope, no prorrateo proporcional (RF-03, 2026-07-24)**: el envio es un valor FIJO, sin descuentos/recargos por FP. `desglosarPagos` recibe `$costoEnvio` (parte de `$totalACubrir`) y llama `AsignadorBasesAjustePagos::asignar($pagos, $totalACubrir - $costoEnvio)`: cada FP recibe como base `min(su monto, bienes aun sin asignar)`, asignando primero a la FP de mayor descuento — nunca una porcion de envio integra la base de ajuste de ninguna FP. *(Antes: `factorBase = ($totalACubrir - $costoEnvio) / $totalACubrir`, la base de cada FP era `monto * factorBase` — prorrateaba el envio proporcionalmente entre TODAS las FP en vez de excluirlo del todo; ver ejemplo numerico en la subseccion RF-03.)*
...
- Contrato `docs/api-v1-delivery.md` documenta el bloque `pagos` y, desde 2026-07-24, la semantica RF-01/02/03 (aditivo, sin romper v1 ni cambiar shape); fixtures/contract tests de `bcn-tienda` deben incorporar el bloque `pagos` y reflejar el nuevo calculo de `monto_ajuste`.

#### Cupon enriquecido en el carrito — `aplica_a` y articulos objetivo (aditivo 2026-07-22)

El bloque `cupon` de `POST carrito/cotizar` (`CotizadorCarritoTienda::cotizar()`) suma, cuando hay un cupon aplicado, tres claves ADITIVAS para que la tienda pueda explicar por que un cupon "no descuenta nada" sin necesidad de conocer las reglas de negocio:

- **`aplica_a`**: copia de `cupones.aplica_a` (`'total'` o el modo de articulo especifico); default `'total'` si el cupon aplicado no trae el campo.
- **`articulos`**: `list<string>` con los NOMBRES de los articulos objetivo del cupon (`Cupon::aplicaAArticulos()` + `->articulos()->pluck('nombre')`); vacio si el cupon aplica a `'total'`. Se resuelve en `aplicarCuponServerSide()` al momento de validar el cupon (no requiere una query aparte del lado tienda).
- **`articulos_bonificados`**: `list<int>` con los IDs de articulo efectivamente bonificados por el cupon EN ESTE carrito (`array_values(array_map('intval', $this->cuponArticulosBonificados))`) — distinto de `articulos` (que son los objetivo declarados en el cupon, esten o no en el carrito).

Caso de uso: un cupon "2x1 en gaseosas" aplicado a un carrito sin ninguna gaseosa sigue "aplicandose" (codigo valido) pero `descuento = 0`; con `aplica_a` + `articulos` la tienda puede mostrar "Este cupon aplica a: Gaseosa 500ml, Gaseosa 1L" en vez de solo un descuento en $0 sin explicacion.

#### Datos del cliente en el checkout — email, cumpleanios y entre calles (RF-T19, 2026-07-21/22)

Patron de claves aditivas en `config_delivery` (igual que Encargos, RF-T16): `Sucursal::CONFIG_DELIVERY_DEFAULTS['checkout'] = ['pedir_email' => 'opcional', 'pedir_cumpleanios' => false, 'pedir_entre_calles' => 'opcional']`, mergeado por clave en `Sucursal::getConfigDelivery()` (un JSON guardado antes de esta key no queda sin default).

- **Config panel** (`App\Livewire\Pedidos\ConfiguracionDelivery`, apartado "Pedidos externos" → seccion nueva "Datos del cliente en el checkout", `config-pedidos-externos.blade.php`): `checkoutPedirEmail` (`'no'|'opcional'|'obligatorio'`, `<select>`), `checkoutPedirCumpleanios` (bool, checkbox) y `checkoutPedirEntreCalles` (`'no'|'opcional'|'obligatorio'`, `<select>`, aditivo 2026-07-22) — los tres en la whitelist de auto-guardado de `ConfiguracionDelivery` (RF-T15, sin boton propio). `persistirConfig()` sanea `checkoutPedirEmail` y `checkoutPedirEntreCalles` contra su enum (fallback `'opcional'` si viene un valor invalido).
- **`GET /v1/tiendas/{slug}`**: suma bloque `checkout: {pedir_email, pedir_cumpleanios, pedir_entre_calles}` (aditivo). El cumpleanios NUNCA es obligatorio del lado servidor, sea cual sea la config.
- **Validacion server-side** (`PedidoTiendaService::crearPedidoExterno()`): si `checkout.pedir_email = 'obligatorio'` y no llega `cliente.email` NI el consumidor logueado tiene email en su cuenta → `Exception` (422). `cliente.fecha_nacimiento` (validacion `nullable|date|before:today`) es SIEMPRE opcional en el payload, sea cual sea la config — la tienda decide si lo pide o no, el core nunca lo exige. **Entre calles** (aditivo 2026-07-22): si `tipo === 'delivery'` y `checkout.pedir_entre_calles = 'obligatorio'` y `direccion.entre_calles` viene vacio → `Exception` (422 "Indica entre que calles esta la direccion"); no aplica a take-away. Con `'no'`/`'opcional'` el campo nunca bloquea el alta, sea cual sea el valor recibido — el core no fuerza el enum del lado servidor mas alla de la validacion `obligatorio`.
- **Persistencia de entre calles**: `direccion.entre_calles` (`nullable|string|max:150`) NO tiene columna propia — se concatena DENTRO de `direccion_referencia` con el formato `"Entre calles: {calles} · {referencia}"` (usando `collect(...)->filter()->implode(' · ')`, cualquiera de las dos partes puede faltar). Aparece asi en el kanban, el detalle del pedido y la comanda sin requerir migracion ni cambios en el resto del sistema que ya lee `direccion_referencia`.
...
- Migraciones aditivas: `clientes.fecha_nacimiento` (tenant, `add_fecha_nacimiento_to_clientes`) y `config.consumidores.fecha_nacimiento` (`add_fecha_nacimiento_to_consumidores`) — ver tablas arriba. Entre calles NO agrega migracion (se persiste en un campo existente, ver arriba).

#### Aclaracion del cliente por item — tienda online (aditivo 2026-07-22)

El consumidor puede dejar una nota puntual por cada renglon del carrito (ej. "sin pepino", "bien cocida"), ademas de las `observaciones` generales del pedido (que ya existian a nivel encabezado):

- **`items[].observaciones`** (`nullable|string|max:255`) en `POST /v1/tiendas/{slug}/pedidos`. `PedidoTiendaService::construirDetalles()` recibe ahora el payload de items crudo (ademas del resultado del cotizador) y mapea `observaciones` por **indice** (`$itemsPayload[$index]`) — el cotizador preserva el orden del payload al construir `itemsCotizados()`, por eso el mapeo por indice es seguro. Se persiste en la columna `pedidos_delivery_detalle.observaciones` (ver 2.16).
- **Panel** (`pedidos-delivery.blade.php`, modal "Ver detalle" / lista expandida): cada item con `observaciones` no vacio muestra la nota en texto ambar debajo del nombre del articulo (icono de nota).
- **Comandas** (`App\Services\Impresion\PlantillasComanda`): tanto `generarComandaESCPOS()` como `generarComandaHTML()` imprimen la aclaracion del item (prefijo `*`) debajo de sus opcionales, ANTES de las observaciones generales del pedido — cocina la ve pegada al renglon que corresponde.
...
#### Vinculacion retroactiva de pedido invitado (RF-T56, 2026-07-31)

`POST /v1/tiendas/{slug}/pedidos/{token}/vincular` (Bearer de consumidor obligatorio, token de seguimiento como credencial — misma regla que el seguimiento y la cancelacion) permite atar a la cuenta del consumidor un pedido que se hizo como INVITADO (sin login), incluso si el pedido ya se convirtio a venta.

- **`PedidoTiendaService::vincularConsumidor(PedidoDelivery, Sucursal, Consumidor): array`**: si el pedido YA tiene `consumidor_id`, es no-op **idempotente** (`vinculado: true` si es la MISMA cuenta, `false` si es OTRA cuenta — nunca se pisa). Si no, dentro de una transaccion tenant: resuelve/crea el `cliente_id` via `resolverClienteId()` (MISMA logica D11 de arriba, respeta `tienda_alta_cliente_automatica`), setea `consumidor_id` y `cliente_id` en el pedido y, si el pedido YA se convirtio a venta (`venta_id` presente), adopta esa venta (`Venta::where('id', ...)->whereNull('cliente_id')->update(['cliente_id' => ...])` — solo si la venta no tenia cliente propio, nunca lo pisa).
- **Acreditacion de puntos**: si hubo adopcion de venta, llama a `PedidoDeliveryService::acreditarPuntosGanados()` (metodo que RF-T56 pasa de `protected` a **`public`** para poder reusarlo desde aca tal cual) con el guard `venta.puntos_ganados == 0` (evita acreditar dos veces si la venta ya los tenia por otra via — el metodo NO es idempotente por si mismo). Si el pedido AUN no se convirtio a venta, no acredita nada: la conversion normal los acredita sola al encontrar `cliente_id` ya seteado.
- **Respuesta**: `{data: {vinculado: bool, puntos_acreditados: int}}`. Errores: `401` sin Bearer de consumidor; `404` token inexistente o de otra tienda (`sucursal_id` del pedido no coincide con la sucursal resuelta por slug).
- **Seguimiento publico enriquecido** (`GET /v1/tiendas/{slug}/pedidos/{token}`): suma el bloque `puntos: {activo, a_ganar, vinculado}` cuando el programa de puntos esta activo, el pedido NO esta cancelado y hay algo que ganar. `a_ganar` sale de `PuntosTiendaService::estimarAGanar()` (formula real de acumulacion, multiplicador de la FP declarada incluido) sobre el monto de los pagos reales NO-puntos del pedido (`es_pago_puntos=false`, estados `activo`/`planificado`), o sobre `total_final` si el pedido no tiene ninguna FP declarada. `vinculado` refleja si ya hay `consumidor_id`. `null`/clave ausente ⇒ la tienda no muestra nada. Con esto la tienda arma el CTA para invitados: "este pedido hubiese sumado N puntos — registrate y sumalos", y tras el registro/login llama a `/vincular` para efectivizarlo.
...
- `GET /v1/tiendas/{slug}/catalogo?tipo=`: catalogo segun criterio RF-17 (activo + vendible + `visible_tienda` + disponible por tipo); agotados vienen marcados `pedible:false`. Precios FINALES calculados por `PrecioService` (nunca localmente, D12). Los grupos de opcionales que viajan por articulo son los ASIGNADOS a ese articulo EN LA SUCURSAL de la tienda (`ArticuloGrupoOpcional`, mismo criterio que `OpcionalService::obtenerOpcionalesParaVenta` del panel), con `precio_extra` de la asignacion (override por articulo, no el del catalogo global de `Opcional`); un grupo sin opciones vivas no se publica. Shape: `{grupo_id, nombre, tipo, obligatorio, min, max, opciones:[{opcional_id, nombre, precio_extra, disponible}]}`. **Cache HTTP (RF-T5)**: responde con `ETag` (md5 del payload) + `Cache-Control: public, max-age=60`; si el cliente manda `If-None-Match` con el mismo ETag devuelve `304` sin body (es el endpoint mas golpeado de la tienda). **Cache server-side (aditivo 2026-07-17, RF-T5; centralizado 2026-07-20, RF-T14)**: ademas del ETag, el armado del payload (`CatalogoTiendaService::catalogo()`) se cachea 60s con `Cache::remember(CatalogoTiendaService::cacheKey($comercioId, $sucursalId, $tipo), 60, ...)` — sin esto, cada revalidacion `If-None-Match` (que igual pega al servidor) recalculaba el catalogo completo contra la BD tenant y el motor de precios aunque terminara respondiendo `304`. Efecto: un cambio de catalogo/precio puede demorar hasta 60s en reflejarse en la tienda. `CatalogoTiendaService::cacheKey()` (key `tienda_catalogo:{comercio_id}:{sucursal_id}:{tipo}`) e `invalidarCache($comercioId, $sucursalId)` (forgetea ambos tipos de pedido) son METODOS ESTATICOS pensados para que otros puntos del sistema invaliden el cache al guardar algo que afecte al catalogo — hoy los usa `ConfiguracionTiendaArticulos` (RF-T14, ver seccion del panel arriba) tras cada guardado inmediato de galeria/badges/destacado/orden.
  - **Precios tachados y promos genericas** (aditivo 2026-07-18, RF-T13): cada articulo suma `precio_lista` (float o `null`) — el precio ANTES de promociones, para mostrar tachado junto al precio de oferta. `CatalogoTiendaService::precioLista()` lo deriva del MISMO `$precioInfo` de `PrecioService` (nunca lo recalcula): `precio_final * subtotal_sin_descuento / subtotal_con_descuento`, redondeado a 2 decimales; devuelve `null` si no hubo descuento o si los subtotales no permiten escalar (`descuento_total <= 0`, `subtotal_con_descuento <= 0`, etc.) — la tienda NO muestra tachado en ese caso. La respuesta del catalogo suma ademas `promociones_genericas: [{nombre, descripcion, precio_fijo, condiciones}]` (`CatalogoTiendaService::promocionesGenericas()`): promociones vigentes HOY para la sucursal — `Promocion` automatica (sin cupon, `activas()->vigentes()->porSucursal()->automaticas()->conUsosDisponibles()`), **TODAS** (aditivo 2026-07-29: antes se excluian las de condicion `por_articulo` por estar ya reflejadas en el precio tachado del catalogo; el comerciante espera verlas igual en el aviso/historia, su alcance sale legible como condicion "Articulo: X") + `PromocionEspecial` automatica (`MODO_AUTOMATICA`, NxM/grupos) de la sucursal cuyo `canal_venta_id` es nulo o es el canal TIENDA. "Vigente hoy" filtra por `dias_semana` (vacio = todos los dias, `dayOfWeek` con domingo=0); las que ademas tienen horario limitado igual entran en la lista (el detalle de horario queda en `descripcion`). Alimenta el aviso "Promociones de hoy" de la home (`tema.promos.mostrar_home`); lista vacia = sin aviso. **`precio_fijo` y `condiciones` enriquecidos** (aditivo 2026-07-21, RF-T21): `precio_fijo` es `float|null` — el precio final fijo cuando la promo es de ese tipo (`Promocion::tipo === 'precio_fijo'` con `valor`, o `PromocionEspecial` de combo/menu con `precio_tipo === PRECIO_FIJO` y `precio_valor`); en cualquier otro tipo de promo va `null`. `condiciones` es `list<string>` legible por humanos, generado por `condicionesLegiblesComun()`/`condicionesLegiblesEspecial()`: para `Promocion` reusa `PromocionCondicion::obtenerDescripcion()` (mismo texto que ve el operador en el panel, incluye "Articulo: X" para condicion `por_articulo`); para `PromocionEspecial` describe la mecanica NxM ("Llevas 3, pagas 2" / "Llevas 3 y 1 va de regalo" segun tenga `nxm_paga` o `nxm_bonifica`) **+ el ALCANCE legible** (aditivo 2026-07-29, `alcanceLegibleEspecial()`): "Aplica a: A, B, C y N mas" (tope de 3 nombres, `listaAplicaA()`) — NxM basico usa los articulos (`nxm_articulos_ids`/`nxm_articulo_id` legado) o, si no hay articulos, las categorias (`nxm_categorias_ids`/`nxm_categoria_id`); NxM avanzado y combo/menu juntan los articulos UNICOS de todos sus `grupos()->articulos`; sin alcance detectable (promos por total, etc.) no agrega nada. AMBOS tipos suman la restriccion de dias/horario si la promo la tiene (`restriccionDiasYHorario()`: "Solo Lun, Mar" / "De 18:00 a 23:00" / "Desde las 20:00" / "Hasta las 15:00"). `condiciones` es `[]` si la promo no tiene ninguna restriccion estructurada mas alla de estar vigente.
  ...
  - **`puntos_canje`** (aditivo 2026-07-30, RF-T47; costo ajustado 2026-08-03, RF-T54): int, el costo EFECTIVO del canje — el configurado (`articulos.puntos_canje`) si esta cargado, si no el derivado del precio del dia (`ceil(precio / valor_punto_canje)`, regla del POS) — ausente si el toggle `canje_tienda` esta apagado, el programa no esta activo, el articulo no tiene precio o esta agotado. Ver seccion "Canje de articulos por puntos en la tienda" en 3.14 para el circuito completo.
...
- `POST /v1/tiendas/{slug}/carrito/cotizar`: cotizacion server-side del carrito completo (**`CotizadorCarritoTienda`**, harness headless del trait `WithCalculoVenta` — mismo motor de precios/promos/cupones que el panel). Con Bearer de consumidor, cotiza con SU cliente materializado (mismo total que el checkout, D12). Los `opcional_id` de `items.*.opcionales` se validan contra las asignaciones (`ArticuloGrupoOpcional`) del articulo EN LA SUCURSAL de la tienda — no asignado o `disponible:false` → `422`. Se cobran al `precio_extra` de la asignacion (no el global del `Opcional`) y el precio del item que ve el motor de calculo YA INCLUYE los opcionales (paridad con el panel `WithOpcionales`: las promociones aplican sobre el precio con opcionales, `precio_opcionales` viaja aparte como desglose). **Encargo** (aditivo 2026-07-20, RF-T16): con `entrega.programado_para`, el controller valida TEMPRANO contra `DeliveryEnvioService::validarProgramado()` (slot vencido/invalido o articulo sin `permite_encargo` → `422 {code:'encargo_invalido'}`) antes de delegar en `CotizadorCarritoTienda` — el checkout falla ahi y no recien en el alta. **Multi-pago** (aditivo 2026-07-21, RF-T18 F2): `pagos: [{forma_pago_id, monto}]` (`min:1,max:2`, `monto` ahora `nullable` desde 2026-07-22 — a lo sumo UNO sin monto cubre el resto, ver seccion "Multi-pago en la tienda") + `costo_envio` (nullable numeric) — si viaja `pagos[]`, el `forma_pago_id` singular se ignora y la respuesta suma `pagos: [{forma_pago_id, nombre, monto_base, ajuste_porcentaje, monto_ajuste, monto_final, permite_vuelto}]` con `total_a_pagar` recalculado (suma de `monto_final`); incompatible con `usar_puntos` (`422 {code:'pagos_invalidos'}`). Ver seccion "Multi-pago en la tienda" mas abajo para el algoritmo completo. **Cupon enriquecido** (aditivo 2026-07-22): el bloque `cupon` de la respuesta suma `aplica_a`, `articulos` (nombres objetivo) y `articulos_bonificados` (ids) — ver seccion "Cupon enriquecido en el carrito" mas abajo. **Canje de articulos por puntos** (RF-T47/RF-T54, requiere Bearer con cliente): `items[].canjear_con_puntos` — ver seccion "Canje de articulos por puntos en la tienda" en 3.14 para el circuito completo (costo configurado del articulo o, sin configurar, derivado del precio del dia).
- `POST /v1/tiendas/{slug}/pedidos` (throttle 15/min, `PedidoTiendaService`): alta de pedido invitado o consumidor. `entrega.{franja|lo_antes_posible}` validados contra la config (franja inventada/vencida → 422); `entrega.programado_para` (RF-T16, aditivo 2026-07-20) es la rama ENCARGO — valida contra el calendario propio de encargos y los articulos del carrito (`validarProgramado()`), tiene prioridad sobre franja/asap y **saltea el bloqueo de tienda cerrada** (`estaAbierto()`); `pago.{forma_pago_id, paga_con}` crea un pago PLANIFICADO (nunca cobra); `pagos: [{forma_pago_id, monto, paga_con?}]` (aditivo 2026-07-21, RF-T18 F2, `min:1,max:2`, `monto` nullable desde 2026-07-22) reemplaza al singular cuando viaja y crea N pagos PLANIFICADOS (uno por FP, mismo shape que el alta manual del panel) — incompatible con `usar_puntos`. `cliente.fecha_nacimiento` (aditivo 2026-07-21, RF-T19, `nullable|date|before:today`) se persiste en el cliente tenant y, con Bearer de consumidor, en su cuenta global; email obligatorio segun `checkout.pedir_email` de la sucursal. `direccion.entre_calles` (aditivo 2026-07-22, RF-T19, `nullable|string|max:150`) obligatorio en delivery segun `checkout.pedir_entre_calles`, se persiste concatenado dentro de `direccion_referencia` (ver seccion "Datos del cliente en el checkout"). `items[].observaciones` (aditivo 2026-07-22, `nullable|string|max:255`) aclaracion del cliente POR ITEM (ver seccion "Aclaracion del cliente por item"). Bloqueos: fuera de horario/alcance/agotados → 422 (un encargo valido no cuenta como "fuera de horario"); (aditivo 2026-07-29, RF-T30) `tipo=delivery` con `delivery_habilitado=false` en la sucursal → 422 (mismo tratamiento que el take-away deshabilitado ya existente; `carrito/cotizar` no valida modalidad, el bloqueo es exclusivo de este endpoint). Entra "por aceptar" o `confirmado` segun `aceptacion_pedidos_externos`. Articulos canjeados por puntos persisten `pedidos_delivery_detalle.puntos_usados` con el costo efectivo del canje — configurado o derivado del precio (RF-T54, ver seccion "Canje de articulos por puntos en la tienda" en 3.14).
- `GET /v1/tiendas/{slug}/pedidos/{token_seguimiento}`: seguimiento publico. El estado interno `facturado` **NUNCA se expone** (el GET lo mapea a `entregado`, el broadcast no lo emite). Incluye `lo_antes_posible`, `demorado` (timeout vencido), `repartidor_en_camino` (solo delivery); cada item de `items` suma `observaciones` (aditivo 2026-07-22, string o `null` — la aclaracion cargada al pedir). **`puntos`** (aditivo 2026-07-31, RF-T56): `{activo, a_ganar, vinculado}` o ausente — ver seccion "Vinculacion retroactiva de pedido invitado" en 3.14. **Aditivos 2026-08-03 (RF-T63/RF-T64)**: la pantalla se completa desde CUALQUIER dispositivo con el token como credencial (antes direccion/cliente vivian solo en la sesion del navegador que hizo el pedido) — `direccion: {direccion, referencia}|null` (solo delivery con direccion cargada), `cliente: {nombre, telefono}|null` (declarados en el checkout o del cliente vinculado), `repartidor: {nombre, telefono}|null` (desde que el comercio lo ASIGNA, no solo `en_camino`; `repartidor_en_camino` sigue viajando igual por compatibilidad) y `palabra_clave` (string|null, RF-T64: viaja solo con repartidor asignado y pedido no cancelado, ver seccion "Palabra clave de entrega" en 3.14).
...
- `POST /v1/tiendas/{slug}/pedidos/{token_seguimiento}/vincular` (aditivo 2026-07-31, RF-T56, requiere Bearer de consumidor `auth:sanctum` + `api.consumidor`, throttle 10/min): vinculacion retroactiva del pedido de INVITADO a la cuenta logueada — ver seccion "Vinculacion retroactiva de pedido invitado" en 3.14 para el detalle completo.

#### Endpoints de consumidores (RF-T1..T3 + RF-T49, `App\Http\Controllers\Api\V1\Consumidores\*`)
...
- **Auth** (`AuthController`):
  - **`recordarme` (RF-T66, spec `tienda-sesion-persistente`, 2026-08-06)**: `registro`, `login`, `auth/google` y `magic-login` aceptan `recordarme?: bool` (default de la tienda: siempre `true`). Con `true`, la respuesta suma `dispositivo: {selector, validator}` (`DispositivoService::emitir()`, ver tabla `consumidor_dispositivos` arriba); sin el campo, `dispositivo` viaja `null` (shape estable, no cambia segun el flujo). El validator en texto plano SOLO existe en esa respuesta — la tienda lo mete en una cookie cifrada, nunca en el Bearer.
  - `POST /registro` (throttle 5/min): `{nombre, email, password (min 8), telefono?, recordarme?}` → `201 {data:{token, consumidor, dispositivo}}`, crea el `Consumidor`, dispara el email de verificacion (best-effort: si el mailer falla solo se loguea, el token igual se devuelve) y el Bearer sirve YA. **Turnstile (RF-T72)**: si esta configurado, exige `turnstile_token` valido ANTES de validar el resto (ver mas abajo).
  - `POST /login` (throttle 10/min): `{email, password, recordarme?}` → `{data:{token, consumidor, dispositivo}}`; credenciales invalidas → `422 validacion` (mensaje generico, no revela si el email existe). Una cuenta sin `password` (creada via Google) tambien cae en este mismo error generico — no revela el metodo de acceso de la cuenta. **Lockout por email (RF-T73, 2026-08-06)**: ademas del throttle por IP (10/min), 5 intentos fallidos del MISMO email (clave `login-email:{sha256(email en minuscula)}`) bloquean el login **15 min**; cada lockout consecutivo DUPLICA la ventana del siguiente bucket (tope 4 h — el "nivel" vive en cache 24h). Durante el lockout responde el MISMO `422` generico (no confirma que el email exista ni que haya lockout activo). Login exitoso limpia `RateLimiter` y el nivel. Depende de que la tienda siga reenviando la IP real (`X-Forwarded-For`).
  - `POST /auth/google` (throttle 10/min, `GoogleIdTokenService`, RF-T49, 2026-07-30): **Sign in with Google** de la tienda online. `{credential, recordarme?}` es el ID token (JWT) que la tienda obtiene en el navegador via Google Identity Services y reenvia tal cual. El controller: 1) si `GoogleIdTokenService::configurado()` es `false` (sin `GOOGLE_CLIENT_ID`) → `503 {codigo:'google_no_configurado'}`; 2) `GoogleIdTokenService::verificar($credential)` valida firma (JWKS de Google), `iss`, `aud === config('services.google.client_id')` y expiracion — `null` → `422 validacion`; 3) resuelve la cuenta por `google_id === claims.sub` (login) → si no existe, por `email === claims.email` (LINKEA: `forceFill(['google_id' => sub])`, el ID token prueba la posesion de ESE email) → si tampoco existe, CREA el `Consumidor` sin password (`nombre` del claim `name` o de la parte local del email, `email`, `google_id`); 4) si `GoogleIdTokenService::esAutoritativo($claims)` y la cuenta no estaba verificada, marca `email_verified_at = now()` YA (sin mail ni plazo de RF-T40); si la cuenta es nueva y Google NO es autoritativo, dispara la verificacion por email normal. Respuesta `{data:{token, consumidor, creado, dispositivo}}` (`201` si creo la cuenta, `200` si hizo login/link).
    - **`esAutoritativo()`** (regla de negocio): Google es autoritativo sobre el email — y por lo tanto prueba su posesion sin verificacion adicional — si el email termina en `@gmail.com`, O si `claims.email_verified === true` Y viene el claim `hd` (cuenta Google Workspace verificada). Cualquier otro caso (dominio custom sin Workspace, `email_verified=false`) sigue el flujo de verificacion por email estandar.
    - **`GoogleIdTokenService`** (`app/Services/Consumidores/`, sin dependencias de BD): `configurado()`, `verificar(string $credential): ?array` (claims o `null`), `esAutoritativo(array $claims): bool`. Verifica firma/exp contra las JWKS de Google (`https://www.googleapis.com/oauth2/v3/certs`, libreria `firebase/php-jwt`), cacheadas 1h (`Cache::remember('google_jwks_certs', 3600)`); ante un `kid` desconocido (rotacion de claves) reintenta UNA vez forzando refresco de las JWKS antes de rechazar. `services.google.client_id` (env `GOOGLE_CLIENT_ID`, compartida con el `.env` de `bcn-tienda`) es el `aud` esperado.
  - `POST /auth/recordar` (throttle 10/min, `c-recordar`, RF-T66): `{selector, validator}` → re-login SILENCIOSO cuando la sesion de la tienda murio pero la cookie de "dispositivo recordado" sigue viva. Delegado en `DispositivoService::canjear()`: par valido y no vencido ⇒ **rota el validator**, desliza `expira_at` (+365 dias), actualiza `ultimo_uso_at`/`ip_ultima` y responde `{data:{token, consumidor, dispositivo:{selector, validator}}}` — el par NUEVO reemplaza al de la cookie, el viejo queda inservible. Selector inexistente/vencido, o validator que NO matchea (reuso de una copia vieja de la cookie, indicio de robo) ⇒ `401 dispositivo_invalido` generico Y **revoca TODOS los dispositivos del consumidor** (familia completa comprometida), con `Log::warning`.
  - `POST /auth/magic-link` (throttle 3/min, `c-magic`, RF-T69/T70): `{email, volver?, pairing?}` → **siempre `200 {data:{ok:true}}`** (anti-enumeracion, patron `recuperar()`). Si el email tiene cuenta, manda `MagicLinkConsumidor` (link a `{TIENDA_URL}/entrar?token=...&volver=...&pairing=...`; `volver`/`pairing` son OPACOS para el core, los interpreta la tienda al aterrizar). Tope silencioso propio de **1 mail cada 10 min por casilla** (`RateLimiter` con clave `magic-email:{sha256(email)}`, ADEMAS del throttle de ruta) — asi el endpoint neutro no sirve para bombardear un email ajeno. Es TAMBIEN la "deteccion de cuenta existente" del checkout invitado (RF-T70): la tienda llama a este mismo endpoint con el email del invitado + `volver` al checkout, y muestra siempre el mismo aviso discreto (no bloqueante) exista o no la cuenta — no hay endpoint `sugerir-cuenta` propio, es un alias de uso de este mismo.
  - `POST /auth/magic-login` (throttle 10/min, `c-mglogin`, RF-T69): `{token, recordarme?}` → canjea el magic link (`ConsumidorTokenService::consumirTokenMagic()`, tipo `mgc`, TTL **15 min**, single-use via `jti`) por un Bearer, mismo shape que `login()`/`registro()`. Si el email NO estaba verificado, el canje lo **verifica de una** (probo control de la casilla — mismo criterio que Google autoritativo, RF-T40). Token invalido, vencido o ya usado → excepcion generica `422`.
  ...
  - `POST /recuperar`: `{email}` → SIEMPRE `200 {ok:true}` (no revela existencia); si el consumidor existe manda el link de reset. **Turnstile (RF-T72)**: mismo gate que `registro`/`restablecer`.
  - `POST /restablecer`: `{token, password}` → cambia el password y **revoca TODOS los tokens** del consumidor (`$consumidor->tokens()->delete()`), fuerza re-login en cualquier sesion abierta. **Ademas revoca todos los dispositivos recordados (RF-T66)** (`DispositivoService::revocarTodos()`): si el cambio de password fue porque robaron la cuenta, las cookies "recordarme" del atacante tambien quedan muertas. **Turnstile (RF-T72)**: mismo gate.
  ...
  - **`ConsumidorTokenService`** (`app/Services/Consumidores/`): tokens de verificacion/reset **STATELESS** (sin tabla), formato `base64url("{tipo}|{consumidor_id}|{expira_ts}").firma`, firma HMAC-SHA256 con `APP_KEY` + una "sal" que invalida el token cuando pierde sentido: verificacion usa el `email` actual (si cambia, los tokens viejos mueren); reset usa un fragmento (24 chars) del hash de password actual (usar el token para resetear cambia el hash → la firma deja de matchear → **single-use sin storage**). TTLs: verificacion 48h, reset 60min. **Tipo `mgc` (magic link, RF-T69, 2026-08-06)**: suma un `jti` random (16 chars) al payload (4 partes en vez de 3) porque el HMAC puro no puede ser single-use por si solo (a diferencia del reset, no hay estado propio del consumidor que cambie al canjearlo); `consumirTokenMagic()` marca el `jti` usado con `Cache::add('consumidor-mgc:{jti}', 1, hasta que vence el token)` — atomico: si el `jti` ya esta en cache, el segundo canje falla solo. Sal = email (misma que verificacion). TTL: **15 min**.
  - **`TurnstileService`** (`app/Services/Consumidores/TurnstileService.php`, RF-T72, 2026-08-06): anti-bot de Cloudflare Turnstile en `registro`, `recuperar` y `restablecer` (los tres puntos de entrada que mandan mail y son blanco de bots). `configurado()` = `config('services.turnstile.secret')` no vacio (env `TURNSTILE_SECRET_KEY`, `config/services.php`). Sin secret configurado, el feature queda **APAGADO** (los endpoints no exigen el campo `turnstile_token`, degradacion honesta, mismo patron que `GoogleIdTokenService::configurado()`). Con secret configurado: token ausente o rechazado por `https://challenges.cloudflare.com/turnstile/v0/siteverify` → `422 {codigo:'turnstile_invalido'}`; si Cloudflare esta inaccesible (timeout/excepcion) ⇒ **fail-open** con `Log::warning` (un registro legitimo no puede depender de la disponibilidad de CF). La tienda renderiza el widget managed con su propio `services.turnstile.site_key`; sin site key no se renderiza ni se manda el campo. Login NO lleva Turnstile permanente (fricción); su defensa es el lockout de RF-T73.
  - **Dispositivos** (`DispositivosController`, RF-T66/T74, Bearer, base `/v1/consumidores/dispositivos`): "Mis dispositivos" de la cuenta. `GET` lista `{id, nombre, ip_ultima, ultimo_uso_at, creado_el, actual}` (mas usado primero, `orderByRaw('COALESCE(ultimo_uso_at, created_at) desc')`); el **selector NUNCA viaja en el listado** (es media credencial) — la tienda manda el suyo en el header `X-Dispositivo` y el core solo devuelve el flag `actual` (`hash_equals`). `DELETE /dispositivos/{id}` revoca uno (`404 no_encontrado` si no pertenece al consumidor autenticado — el query siempre parte de `$request->user()->dispositivos()`). `DELETE /dispositivos` (sin id) = "cerrar sesion en los demas dispositivos": revoca todos MENOS el del header `X-Dispositivo` (sin header, revoca TODOS) → `{data:{ok, revocados}}`.
  - **`DispositivoService`** (`app/Services/Consumidores/DispositivoService.php`, RF-T66): `emitir()` (poda al 11° — conserva `MAX_DISPOSITIVOS = 10`, borra las filas MENOS usadas primero, `orderByRaw('COALESCE(ultimo_uso_at, created_at) asc')`), `canjear()` (rotacion + deteccion de reuso + revocacion de familia, ver `auth/recordar` arriba), `revocarTodos()`. El nombre amigable del dispositivo (`nombreDesdeUserAgent()`, best-effort sin libreria externa — detecta Instagram/Facebook/Firefox/Edge/Chrome/Safari y Android/iPhone/Windows/Mac/Linux por substring del User-Agent, ej. `"Chrome · Android"`, `"Instagram · iPhone"`) se calcula UNA vez al emitir, no se recalcula en cada canje.
...
- `GET /v1/delivery/config` (`ConfigController`, `config:read`): suma `delivery_habilitado` (aditivo 2026-07-29, RF-T30) junto al `takeaway_habilitado` existente, mismo default `?? true` (ver 3.14) / `GET /v1/repartidores` (`config:read`).

#### Tiempo real (Reverb)

- **Seguimiento publico** (canal PUBLICO, sin auth): `pedidos-delivery.seguimiento.{token_seguimiento}`, evento `SeguimientoActualizado` (`PedidoSeguimientoPublicoBroadcast`, `ShouldBroadcastNow`) con `{estado, estado_label, repartidor, hora_pactada_at, lo_antes_posible, at}` en cada cambio de estado de un pedido externo. **Aditivos 2026-08-03 (RF-T63/RF-T64)**: suma `repartidor_asignado` (`{nombre, telefono}|null`) y `palabra_clave` (string|null), mismas reglas que el GET de seguimiento (ver arriba); a diferencia del resto de los campos, este evento se emite TAMBIEN al asignar repartidor (`PedidoDeliveryService::asignarRepartidor()`) aunque el estado del pedido no cambie -- es el unico disparo del broadcast que no viene de un cambio de `estado_pedido`.
