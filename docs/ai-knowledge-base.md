...
- **Canje por puntos en la tienda** (RF-T47, 2026-07-30; costo configurado RF-T54, 2026-07-31): `toggleCanjeTienda()` invierte `canje_tienda` del pivot `articulos_sucursales` de la sucursal ACTIVA (no una columna del articulo). El costo en puntos NO se configura en este toggle: es `articulos.puntos_canje` (misma columna que usa el canje del POS, RF-10 mas abajo) — **prenderlo exige `puntos_canje > 0`**; sin eso el boton (icono junto al de destacado) queda `disabled` con opacidad reducida y tooltip "Cargale puntos de canje en el articulo". Apagarlo, en cambio, siempre esta permitido. Ver seccion 3.14 "Canje de articulos por puntos en la tienda" mas abajo para el circuito completo (catalogo, cotizacion, alta).
...
#### Canje de articulos por puntos en la tienda — costo configurado (RF-T47, 2026-07-30; costo corregido RF-T54, 2026-07-31)

Un articulo es canjeable por puntos EN LA TIENDA cuando se cumplen 3 condiciones a la vez: el comercio prendio `canje_tienda` (pivot `articulos_sucursales` de ESA sucursal, toggle en el panel — ver "Configuracion de tienda POR ARTICULO" mas arriba), el programa de puntos esta activo para el cliente, y el articulo tiene **puntos de canje configurados** (`articulos.puntos_canje > 0`, la MISMA columna que usa el canje del POS, RF-10 mas abajo). El costo es SIEMPRE el configurado — hasta RF-T54 se derivaba del precio del dia (`ceil(precio / valor_punto_canje)`, regla RETIRADA por poder divergir del costo real que usa el POS para ese mismo articulo).

- **Catalogo** (`GET /v1/tiendas/{slug}/catalogo`, `CatalogoTiendaService::catalogo()`): cada articulo suma `puntos_canje` (int, = `articulos.puntos_canje`) solo si las 3 condiciones se cumplen ademas de precio > 0 y no agotado. Ausente ⇒ no canjeable, la tienda no muestra nada.
- **Cotizacion** (`POST /v1/tiendas/{slug}/carrito/cotizar`, requiere Bearer con cliente): `items[].canjear_con_puntos` marca el renglon como canjeado. `CotizadorCarritoTienda::construirItem()` rechaza el renglon con `Exception` si `canje_tienda` esta apagado O `puntos_canje <= 0`; ademas exige cantidad 1 y sin opcionales con precio (un opcional pago quedaria regalado). `puntosUsadosEnArticulos()` (ya NO recibe `valor_punto_canje` como parametro) suma el `puntos_canje` de cada item canjeado x cantidad; ese total se valida contra el saldo del cliente. El bloque `puntos` de la respuesta suma `usados_en_articulos` con este mismo total.
- **Alta** (`POST /v1/tiendas/{slug}/pedidos`, `PedidoTiendaService`): `construirDetalles()` (ya NO recibe `valorPuntoCanje` como parametro) persiste `pedidos_delivery_detalle.puntos_usados` con el `puntos_canje` configurado del articulo x cantidad; la conversion a venta arma el `MovimientoPunto` canje-articulo desde ese valor, igual que el panel.
...
- `GET /v1/tiendas/{slug}/catalogo?tipo=`: catalogo segun criterio RF-17 (activo + vendible + `visible_tienda` + disponible por tipo); agotados vienen marcados `pedible:false`. Precios FINALES calculados por `PrecioService` (nunca localmente, D12). Los grupos de opcionales que viajan por articulo son los ASIGNADOS a ese articulo EN LA SUCURSAL de la tienda (`ArticuloGrupoOpcional`, mismo criterio que `OpcionalService::obtenerOpcionalesParaVenta` del panel), con `precio_extra` de la asignacion (override por articulo, no el del catalogo global de `Opcional`); un grupo sin opciones vivas no se publica. Shape: `{grupo_id, nombre, tipo, obligatorio, min, max, opciones:[{opcional_id, nombre, precio_extra, disponible}]}`. **Cache HTTP (RF-T5)**: responde con `ETag` (md5 del payload) + `Cache-Control: public, max-age=60`; si el cliente manda `If-None-Match` con el mismo ETag devuelve `304` sin body (es el endpoint mas golpeado de la tienda). **Cache server-side (aditivo 2026-07-17, RF-T5; centralizado 2026-07-20, RF-T14)**: ademas del ETag, el armado del payload (`CatalogoTiendaService::catalogo()`) se cachea 60s con `Cache::remember(CatalogoTiendaService::cacheKey($comercioId, $sucursalId, $tipo), 60, ...)` — sin esto, cada revalidacion `If-None-Match` (que igual pega al servidor) recalculaba el catalogo completo contra la BD tenant y el motor de precios aunque terminara respondiendo `304`. Efecto: un cambio de catalogo/precio puede demorar hasta 60s en reflejarse en la tienda. `CatalogoTiendaService::cacheKey()` (key `tienda_catalogo:{comercio_id}:{sucursal_id}:{tipo}`) e `invalidarCache($comercioId, $sucursalId)` (forgetea ambos tipos de pedido) son METODOS ESTATICOS pensados para que otros puntos del sistema invaliden el cache al guardar algo que afecte al catalogo — hoy los usa `ConfiguracionTiendaArticulos` (RF-T14, ver seccion del panel arriba) tras cada guardado inmediato de galeria/badges/destacado/orden.
  ...
  - **`puntos_canje`** (aditivo 2026-07-30, RF-T47; costo corregido 2026-07-31, RF-T54): int, el costo CONFIGURADO (`articulos.puntos_canje`, misma fuente que el POS) — ausente si el toggle `canje_tienda` esta apagado, el programa no esta activo, el articulo no tiene puntos configurados, no tiene precio o esta agotado. Ver seccion "Canje de articulos por puntos en la tienda" en 3.14 para el circuito completo.
