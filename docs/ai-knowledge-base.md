...
- Cerrada: boton circular `<button data-burbuja-por-aceptar>` con SOLO la cantidad (`$pedidosPorAceptar->count()`), rojo si `$hayAceptacionDemorada` (algun pedido supero `timeout_aceptacion_min`) o naranja si no. El `@if($pedidosPorAceptar->isNotEmpty())` que envuelve todo el bloque hace que ni la burbuja ni el bloque movil se rendericen con 0 pedidos.
- Arrastre (`empezarDrag(ev)`): distingue click de arrastre por umbral de 6px (`Math.abs(dx)+Math.abs(dy) <= 6`, listeners `pointermove`/`pointerup` en `window`); al soltar, `anclar(x,y)` calcula el borde mas cercano (distancia de `(x,y)` a cada uno de los 4 bordes de la ventana) y la fraccion 0..1 a lo largo de ese borde (clamp 0.05–0.95, resize-safe), persistidos en `localStorage[clave]` como `{borde, offset}`. El estilo (`estiloBurbuja`, get computed) aplica esa posicion via `:style`, que Livewire preserva en cada morph.
- Click sin arrastre (`clickBurbuja()`, suprime el click sintetico que el browser dispara despues del `pointerup` de un arrastre con la bandera `_suprimirClick`) abre el panel expandido: `position:fixed`, 50vw (bordes izquierda/derecha) o 50vh (bordes arriba/abajo), deslizandose DESDE el borde anclado (`clasesPanel`/`clasePanelOculto`, transicion `translate`), con backdrop (`@click="abierto = false"`) y cierre por `@keydown.escape.window`.
