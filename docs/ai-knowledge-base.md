...
**Estetica avanzada (RF-T13, 2026-07-18)**: nuevos controles dentro del apartado "Apariencia de la tienda" de `ConfiguracionTienda`, todos persistidos en los sub-objetos de `tema` (ver shape de `Tienda::temaCompleto()` arriba). Props del componente (RF-T13 + RF-T25): `portadaPosicion`, `logoRadio` (RF-T25, reemplaza a `portadaOverlay` — ver abajo), `slogan`, `descripcion`, `redFacebook`, `redInstagram`, `catalogoLayout`, `destacadosModo`, `destacadosAdorno`, `promosMostrarHome`.

- **Validaciones** (`guardarTienda()`): `portadaPosicion`/`logoRadio`/`catalogoLayout`/`destacadosModo`/`destacadosAdorno` con `in:` contra los catalogos cerrados del modelo (`logoRadio` valida contra `Tienda::RADIOS_DISPONIBLES`, el mismo catalogo que `radios`); `slogan` `max:120`, `descripcion` `max:1000` (ambos `nullable`); `redFacebook`/`redInstagram` con regex de host (`^https://(www\.)?(facebook\.com|fb\.com)/.+` y `^https://(www\.)?instagram\.com/.+`) — rechaza URLs de otro dominio aunque tengan formato valido. `temaDesdeForm()` hace `trim()` de slogan/descripcion/redes antes de persistir.
...
- **Historias**: nueva seccion "Historias destacadas" (border-t propio, entre el bloque logo/portada y los 5 colores). Props: `array $historiaUploads` (input `multiple`, `wire:model="historiaUploads"`, validado `historiaUploads.*` => `nullable|image|max:5120`) y `array $historiasActuales` (`[{id, url}]`, calculado por `mapHistorias()` en cada `cargar()`/mutacion). A diferencia de logo/portada (1 upload = 1 campo), las historias nuevas se acumulan en el array y **recien se persisten en `guardarTienda()`** (`foreach (array_filter($this->historiaUploads) as $upload) { $imagenes->agregarHistoria($tienda, $upload); }`) — un upload roto/vacio del array (slot descartado) se filtra con `array_filter`. Antes de guardar, `guardarTienda()` valida el tope combinado `count($historiasActuales) + count(array_filter($historiaUploads)) > Tienda::MAX_HISTORIAS` y aborta con error si se excede.
  - `eliminarHistoria(string $id)` y `moverHistoria(string $id, int $direccion)` (-1/+1, swap con el vecino en `historiasActuales`, llama a `reordenarHistorias()`) son **inmediatos** (mismo patron que `eliminarLogo()`): no esperan al boton "Guardar tienda", y ambos disparan `tienda-guardada` (recarga el iframe del visor) al terminar.
  - `descartarHistoriaUpload(int $index)`: solo quita del array `historiaUploads` en memoria (nada persistido todavia).
  - `render()` calcula `historiaPreviewUrls` (mapea `historiaUploads` a `temporaryUrl()`, `try/catch` — si el archivo temporal expiro se omite SOLO la miniatura, el upload sigue pendiente para guardar).
  - El checkbox `promosMostrarHome` (prop ya existente de RF-T13, sin cambio de nombre) se **reubico** en esta seccion (antes vivia en "Presentacion del catalogo"): sigue siendo `tema.promos.mostrar_home`, pero ahora documentado del lado tienda como "ultimo slide de las historias" en vez de aviso en la home.
