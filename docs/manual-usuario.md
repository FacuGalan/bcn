...
> Version: 0.1.x | Ultima actualizacion: 2026-08-06 (Tienda Online -- pago online con Mercado Pago Checkout Pro (RF-T75..T83, core): en Configuracion > Integraciones de Pago se suma la integracion **"Mercado Pago - Checkout Online"** (credenciales propias, producto separado de QR y Point) que se asigna a una forma de pago desde el mismo bloque "Integraciones de pago" de Formas de Pago (seccion 12.4) -- una forma de pago con esta integracion activa se ofrece en la tienda **exclusivamente como pago online** (el consumidor paga en la pagina segura de Mercado Pago antes de que el pedido llegue al comercio; por ahora no convive con la variante "pagar al recibir" de esa misma forma de pago); mientras el pago no se acredita, el pedido queda **invisible para el comercio** (no entra a "por aceptar", no suena la burbuja) y se ve en el listado general con el badge naranja **"Esperando pago online"** (visible filtrando por borradores); al acreditarse Mercado Pago via webhook, el pedido pasa solo a "por aceptar" (o queda confirmado directo, segun la aceptacion configurada) y el modal "Ver detalle" muestra el bloque **"Pago online (Mercado Pago)"** con su estado (Acreditado / Esperando / Devuelto al cliente / A devolver); un pedido con pago online ya acreditado no admite cambios de monto (aceptarlo como esta o rechazarlo); al **rechazar o cancelar** un pedido pagado online el sistema intenta la **devolucion automatica** en Mercado Pago (incluida la propina) sin bloquear nunca el rechazo -- si el reintegro falla, el pedido queda igual rechazado pero el cobro aparece con el badge rojo **"Pago a devolver"** en el listado y el boton **"Reintentar devolucion"** en el detalle; en Configuracion > Delivery / Take Away, apartado "Pedidos externos", se suma el tilde **"Ofrecer propina al pagar online (para el repartidor)"** con los porcentajes sugeridos configurables -- la propina se cobra junto con el pedido pero nunca integra su total ni lo facturable, y el detalle del pedido la muestra aparte ("Propina para el repartidor: $X"); anterior: Acceso al sistema -- pantalla de inicio de sesion mejorada en celular y mensajes traducidos: la pantalla de "Iniciar sesion" (y las de "Olvide mi contrasena", "Restablecer contrasena", "Confirmar contrasena" y "Verificar email") se ven mejor en el celular -- el logo y el formulario quedan centrados juntos en el medio de la pantalla en vez de que el logo quede solo arriba con mucho espacio vacio y el formulario abajo; en computadora no cambia nada; los mensajes de esas pantallas (por ejemplo "Estas credenciales no coinciden con nuestros registros", "Le hemos enviado por correo electronico el enlace para restablecer la contrasena", "El campo contrasena es obligatorio") ahora se muestran traducidos en espanol y portugues -- antes salian en ingles aunque el resto del sistema estuviera en el idioma elegido; anterior: Pedidos Delivery -- burbuja flotante de pedidos por aceptar y correccion de direccion con mapa de contexto: en el panel de Pedidos Delivery (escritorio), la franja naranja "Pedidos por aceptar" se reemplaza por una **burbuja circular flotante** -- cerrada muestra solo la cantidad, se puede **arrastrar** a cualquier borde de la pantalla (se ancla sola al borde mas cercano al soltar y la posicion queda guardada en ese equipo para esa sucursal, sobrevive a recargar la pagina), un click la expande ahi mismo en una **tarjeta flotante** (no una ventana que ocupa medio monitor) con el mismo listado y los botones Ver/Aceptar/Rechazar de siempre (se cierra con la X, con Escape o clickeando afuera, y ni la burbuja ni la tarjeta quedan nunca debajo del menu superior), y se pone **roja** cuando algun pedido supera el tiempo de aceptacion configurado (antes badge "Demorado" con la franja plegada); en celular se mantiene el bloque plegable de siempre, en el flujo de la pagina; en el modal "Ver detalle" de un pedido delivery que todavia no esta entregado/facturado/cancelado, junto a la direccion de entrega se suma un boton de mapa **"Editar direccion de entrega"** que abre un modal para corregir la direccion escrita y, si la sucursal georreferencia pedidos, arrastrar el pin del domicilio en el mapa -- al guardar, si la nueva ubicacion cambia de zona o de costo de envio, el sistema muestra el cambio (zona y costo antes -> despues) y pide confirmarlo antes de guardar; si la ubicacion queda fuera del area de reparto, avisa y permite "Guardar igual" (el pedido queda sin zona, conserva el costo de envio anterior); si el pedido ya tiene pagos registrados, el costo de envio no se toca -- se guarda solo la direccion y se avisa; con la georreferenciacion apagada el modal es solo texto, sin mapa; el mapa del selector de direccion (tanto en el alta de pedido como en esta correccion) ahora arranca siempre **cerrado** (ya no se abre solo al abrir el modal de direccion; se abre unicamente al tocar "Abrir mapa", sin ninguna llamada a Google mientras tanto), suma arriba un **buscador de direcciones de Google** siempre visible -- asi se puede corregir tambien la ubicacion geolocalizada y no solo el texto escrito, sin tener que abrir el mapa (al elegir una direccion, el mapa se abre solo para confirmar donde cayo el pin; el buscador tambien se carga recien al tocarlo) --, deja de mostrar el boton **"Usar mi ubicacion actual"** (la direccion es la del cliente, no la del operador que la carga), el pin del domicilio pasa a ser el **marcador rojo estandar** de Google Maps (arrastrable, igual que antes) y, si la sucursal tiene coordenadas cargadas, se dibuja como contexto el **pin naranja del local** y los **poligonos de las zonas de reparto activas con su nombre** (o el circulo del radio general si no hay zonas dibujadas) -- es solo referencia visual, la cotizacion real la sigue calculando el sistema; anterior: Pedidos Delivery -- palabra clave de entrega (RF-T64): en Configuracion > Delivery / Take Away, apartado General, se suma el checkbox "Palabra clave de entrega" (junto a "Exigir repartidor") -- con el tilde activo, cada pedido delivery genera al confirmarse una palabra simple (por ejemplo "Tigre" o "Luna") que el cliente ve en el seguimiento de la tienda apenas hay repartidor asignado y dice al recibir el pedido; el panel de Pedidos Delivery la muestra en el detalle del pedido, junto al repartidor, con un badge con icono de llave "Palabra clave: ..."; anterior: Tienda Online -- banner de foto por categoria con punto focal (RF-T62): en Configuracion > Delivery / Take Away > Tienda Online > Articulos de la tienda, cada categoria suma un boton de foto (junto al de badges) que abre un editor con vista previa en la proporcion real de la franja del encabezado, un selector de **punto focal** (clic sobre la vista previa para elegir que parte de la foto queda visible, porque la foto original casi nunca tiene la proporcion de la franja) y los botones Subir/Cambiar/Quitar banner de la categoria -- guardado al instante y visor en vivo actualizado solo, igual que el resto de esta seccion; la pantalla Articulos > Categorias suma el campo "Imagen para la tienda (opcional)" en el alta/edicion, con el mismo pipeline seguro de re-encode que el resto de las imagenes de la tienda; anterior: Tienda Online -- canje de articulos por puntos con costo configurado (RF-T54): en Configuracion > Delivery / Take Away > Tienda Online > Articulos de la tienda, el icono que permite el canje por puntos de cada articulo ahora exige que el articulo tenga puntos de canje cargados -- sin eso el icono queda deshabilitado con el aviso "Cargale puntos de canje en el articulo" (deshabilitar el canje, en cambio, siempre esta disponible); el costo en puntos que ve el cliente en la tienda y el que se descuenta de su saldo al canjear pasan a ser siempre los puntos de canje configurados del articulo (la misma configuracion que usa el canje del punto de venta) -- antes se calculaban en base al precio del dia, lo que podia divergir del costo real del punto de venta para el mismo articulo; anterior: Tienda Online -- historias con subida inmediata, presentacion del catalogo y texto descriptivo, badges de categoria (RF-T31/T33/T34/T35/T36): en Configuracion > Delivery / Take Away > Tienda Online > Apariencia de la tienda, la seccion "Historias destacadas" ahora sube y persiste las fotos AL INSTANTE (ya no espera a "Guardar tienda"; se pueden elegir varias fotos de una vez o en tandas sucesivas, todas se acumulan hasta el maximo de 3) y las historias ya guardadas se reordenan **arrastrandolas** (drag & drop) en vez de con flechas; el campo antes llamado "Texto libre (seccion propia en la pagina principal)" pasa a llamarse **"Texto descriptivo (se muestra en «Informacion del comercio»)"**, con limite ampliado de 1000 a 5000 caracteres (se mostrara en el panel de informacion del comercio de la tienda, no como seccion de la home); "Presentacion del catalogo" suma el checkbox **"Categorias desplegables"** (acordeon por categoria en el catalogo de la tienda) y el campo **"Titulo de la seccion de destacados"** (hasta 40 caracteres, vacio usa "Destacados"); el selector "Adorno del destacado" ya no se deshabilita en modo "Tarjeta grande" unicamente -- ahora tambien aplica al modo "Banner deslizable" (decorando el titulo de la seccion en vez de la tarjeta) y solo se deshabilita si los destacados estan en "Ninguno"; en "Articulos de la tienda" se suma un editor de **badges por categoria** (boton de etiqueta junto al titulo de cada categoria, mismos chips predefinidos + badge propio, hasta 4, guardado inmediato, chips visibles en el header de la categoria); anterior: Pedidos Delivery -- modulo de pedidos y modalidad delivery propia (RF-T30): en Configuracion > Delivery / Take Away, apartado General, el tilde que antes decia "Habilitar delivery en esta sucursal" ahora se llama **"Habilitar modulo de pedidos"** (hint: "Activa el panel de pedidos, la tienda online y el marketplace (delivery y/o take-away)"); si se apaga con la tienda online ya publicada, aparece un aviso rojo "La tienda online quedara fuera de linea"; se suma un tilde propio, **"Delivery habilitado"** (hint: "Permite pedidos con envio a domicilio."), en paridad con "Habilitar take-away" -- una sucursal puede operar SOLO con take-away apagando este tilde; el selector Delivery/Take-away del alta de pedido (seccion 15.2) oculta el boton de la modalidad que este apagada, corrige el tipo seleccionado si ya no esta disponible y, si de todos modos se intenta forzar, avisa "El delivery esta deshabilitado en esta sucursal" y vuelve a la modalidad habilitada; publicar la tienda online (switch maestro, seccion 15.6) con el modulo de pedidos apagado ahora lo prende solo -antes la tienda quedaba publicada pero inaccesible porque el modulo estaba apagado- con el aviso "Se habilito el modulo de pedidos (la tienda online lo necesita)"; anterior: Promociones -- forma de venta, canal de venta y montos/cantidades maximas (RF-T28/RF-T29): en el wizard de Promociones (Configuracion > Promociones, paso 4 "Condiciones") y en el wizard de Promociones Especiales (paso 3 "Condiciones"), los campos "Forma de venta" y "Canal de venta" dejan de ser un unico select y pasan a ser listas de checkboxes con seleccion multiple (misma presentacion que "Formas de pago": lista con scroll, contador de seleccionados y la leyenda "(todas)"/"(todos)" cuando no se marca ninguna); una promocion puede aplicar a varias formas de venta y/o varios canales a la vez (por ejemplo Local + Delivery), y si no se marca ninguna sigue aplicando a todas/todos como antes; en las Promociones comunes (no en las Especiales) el paso 4 suma "Monto maximo" y "Cantidad maxima" junto a los minimos existentes -- dejando el campo vacio no hay tope; si se completan ambos (minimo y maximo), la promocion solo aplica cuando el total o la cantidad de la venta cae DENTRO del rango; el listado de condiciones y el simulador de venta muestran el rango de forma legible (por ejemplo "Total: entre $1000 y $5000") y, si la promocion no aplico por superar el maximo, el simulador lo aclara ("Monto maximo superado"/"Cantidad maxima superada"); fix: una promocion con 2 o mas formas de pago (o, con este cambio, 2 o mas formas/canales de venta) antes podia no aplicarse nunca en el catalogo de la tienda online ni en la pantalla de consulta de precios -- ahora se evalua correctamente (alcanza con que se cumpla UNA de las formas de pago/venta/canales declarados, no todas a la vez); anterior: Tienda Online -- historias destacadas y forma del logo (RF-T24/RF-T25): en Configuracion > Delivery / Take Away, apartado "Tienda Online" > "Apariencia de la tienda", se suma la seccion "Historias destacadas" -- hasta 3 fotos que rodean el logo del encabezado con un anillo estilo historias (se aplican recien al hacer clic en "Guardar tienda"), reordenables con flechas y eliminables al instante; el toggle de promociones se mudo a esta seccion y ahora agrega las promociones vigentes como PRIMER slide de las historias en vez del aviso "Promociones de hoy" de la pagina principal (que desaparece); se suma el selector "Forma del logo" (Recto / Suave / Medio / Amplio / Redondo, por defecto Redondo), independiente de "Bordes redondeados" que sigue aplicando solo a tarjetas y carrito; se elimina la opcion "Fundir la portada con el color de la tienda (fade)" -- la portada se muestra siempre tal cual, solo queda el selector de Encuadre; anterior: Pedidos Delivery -- franja "Pedidos por aceptar" plegable y promesa del cliente al aceptar (RF-T26/RF-T27): la franja "Pedidos por aceptar" del panel de Pedidos Delivery ahora arranca PLEGADA por defecto, con un contador pulsante en el encabezado y, si algun pedido supera el tiempo de aceptacion configurado, un badge rojo "Demorado" visible aunque este plegada; al desplegarla, cada pedido muestra ademas la promesa de entrega que eligio el cliente en la tienda (franja horaria, "Lo antes posible" o dia/hora del encargo); el modal "Aceptar pedido", cuando el pedido trae esa promesa, la muestra primero ("El cliente eligio: ...") con un boton "Aceptar como lo pidio" para confirmarlo sin tocar la hora -- las opciones de demora/franja quedan como alternativa si no se puede cumplir tal cual; los pedidos por encargue ya no reciben una hora calculada por distancia al aceptarse (se respeta la fecha/hora del encargo); los pedidos externos que llegan como borrador (aceptacion manual) ahora aparecen en la franja al instante, sin refrescar la pagina, con un destello visual en el encabezado; fix: los opcionales que el cliente elige en un pedido de la tienda online ahora se guardan siempre con su grupo -- antes podian perderse en silencio y faltar en el detalle del pedido, el seguimiento publico o al convertir el pedido en venta; anterior: Pedidos Delivery -- envio desglosado en totales y consistencia de pago dividido (RF-01/02/03/04/05): en el panel de Pedidos Delivery, cuando el pedido tiene costo de envio, el resumen de totales y el modal de pago mixto muestran "Total articulos" + "Envio (fijo, sin ajustes)" + "Total pedido" en vez de un unico total mezclado, y el modal de pago dividido aclara la base de cada descuento/recargo por forma de pago (por ejemplo "-10% s/ $900 de articulos") o si un pago no recibe ajuste por cubrir solo envio; en el listado, el modal "Ver detalle" del pedido muestra el envio en una fila propia (con badge "manual" si fue cargado a mano) y aclara que el ajuste por forma de pago es "s/ articulos"; una promocion o lista de precios condicionada por forma de pago (por ejemplo "solo efectivo") ahora se valida contra TODAS las formas de pago de un pago dividido, no solo la primera declarada -- si parte del pago usa una forma de pago no habilitada, el descuento o precio especial no aplica (regla identica a la que ya tenian los cupones); con pago dividido, el descuento o recargo de cada forma de pago se calcula sobre su porcion de articulos (nunca sobre una porcion de envio), asignando primero la mercaderia a la forma de pago con mayor descuento; fix: la grilla del panel tactil (Pedidos por Mostrador y Pedidos Delivery) ahora muestra el precio EFECTIVO del articulo (con override por sucursal y lista de precios activa), no el precio de lista global -- lo que se ve en la grilla es lo que se agrega al carrito; anterior: Tienda Online -- entre calles en el checkout y aclaracion del cliente por item: en Configuracion > Delivery, apartado "Datos del cliente en el checkout", se suma el selector "Entre calles (solo delivery)" con las mismas opciones que Email (No pedir / Pedir opcional / Pedir obligatorio), auto-guardado igual que el resto del apartado; en un pedido de la tienda, si el cliente dejo una aclaracion puntual en un item del carrito (por ejemplo "sin cebolla"), esa nota aparece en texto ambar debajo del nombre del articulo tanto en el modal "Ver detalle" del panel como en la comanda de cocina impresa (ESC/POS y HTML); anterior: Tienda Online -- pago con mas de una forma de pago y datos del cliente en el checkout: el carrito/alta de pedidos de la tienda ahora puede declararse con hasta 2 formas de pago (cada una con su propio monto), el sistema calcula el descuento/recargo de cada una sobre su porcion y el pedido queda con esos pagos planificados igual que si se hubiera cargado a mano en el panel (incompatible con el canje de puntos); en Configuracion > Delivery, apartado "Pedidos externos", se suma la seccion "Datos del cliente en el checkout" con un selector de **Email** (No pedir / Pedir opcional / Pedir obligatorio) y el checkbox "Pedir fecha de cumpleanios" (siempre opcional, con leyenda de promociones), auto-guardado igual que el resto del apartado; en el visor en vivo de Configuracion > Tienda se suma, junto al link "Abrir en pestana nueva", un boton con icono de sol/luna para previsualizar la tienda en modo oscuro o claro sin salir del panel; anterior: Formas de Pago: en el modal de alta/edicion (Configuracion > Formas de Pago) se suma, para formas de pago no mixtas, la seccion "Disponible en tienda online" con un checkbox por sucursal -- define en que sucursales los clientes de la tienda online pueden elegir esa forma de pago para pagar contra entrega o retiro (independiente de si esta activa en el punto de venta presencial); anterior: Encargos -- pedidos delivery/take-away para un dia futuro (RF-T16): en Configuracion > Delivery / Take Away se suma el apartado "Tomar pedidos por encargue" (switch maestro, calendario PROPIO de dias/horarios/feriados precargado desde el de atencion la primera vez, anticipacion minima en horas, hasta cuantos dias adelante y minutos antes de la hora en que el encargo aparece en el panel); el panel de Pedidos Delivery suma un tercer boton "Encargos" (con contador) que agrupa por dia los encargos aceptados todavia fuera de ventana, con acciones Ver / Preparar ahora / Cancelar, badge fucsia "Encargo · fecha/hora" en Lista/Kanban/Encargos y el filtro "Encargos programados" en el estado del pedido; nuevo reporte "Produccion de encargos" (rango de fechas, por dia y articulo con cantidades sumadas de los pedidos aceptados, drill-down a los pedidos, imprimible); en "Articulos de la tienda" se suma el checkbox "Disponible para encargos" por articulo (guardado inmediato); anterior: configuracion de tienda POR ARTICULO: dentro de "Apariencia de la tienda", debajo de "Presentacion del catalogo", se suma la seccion "Articulos de la tienda" -- lista agrupada por categoria de los articulos visibles en la tienda de la sucursal, con miniatura, toggle de destacado, chips de badges y boton "Fotos" que expande una galeria inline de hasta 5 fotos por articulo (ademas de la imagen del articulo del panel); badges predefinidos (Sin TACC, Vegetariano, Vegano, Picante, Nuevo, Mas vendido, Artesanal, Sin azucar, hasta 4 por articulo) mas un badge de texto libre propio; reordenamiento por arrastre (drag & drop) de categorias y de articulos dentro de cada categoria, con los destacados mostrandose siempre primero en la tienda; todos los cambios de esta seccion se guardan al instante (no participan del boton "Guardar tienda") y el visor de la derecha se actualiza solo poco despues de cada cambio; anterior: personalizacion estetica avanzada de la Tienda Online: en el apartado "Apariencia de la tienda" se suman el toggle "Fundir la portada con el color de la tienda (fade)" y el selector de encuadre vertical Arriba/Centro/Abajo (con la miniatura mostrando el recorte real y el visor reflejandolo en vivo), el bloque "Contenido y redes" (slogan en el encabezado, texto libre como seccion propia de la home, URLs de Facebook e Instagram con boton en el encabezado) y el bloque "Presentacion del catalogo" (layout Grilla/Renglones, modo de destacados Banner deslizable/Tarjeta grande/Sin seccion con su adorno Ninguno/Brillo/Badge/Ambos, y el aviso "Promociones de hoy"); el boton "Restablecer al tema default" ya no borra el slogan, el texto ni las redes; anterior: visor en vivo de la Tienda Online en el panel: en pantallas anchas el apartado "Tienda Online" pasa a dos columnas -- configuracion a la izquierda, un panel fijo a la derecha con la vista previa; con la tienda publicada, ese panel muestra la TIENDA REAL en un recuadro embebido y los cambios de apariencia/logo/portada se ven al instante ahi, con un link para abrirla en una pestana nueva; despublicada, sigue mostrando la simulacion de siempre; en pantallas angostas se mantiene el boton "Vista previa" con esa misma simulacion; anterior: rediseno de la pantalla de Configuracion de Delivery / Take Away: layout mas ancho con General y Promesa de entrega lado a lado; el apartado "Tienda Online" se movio al final con un **switch maestro** que la publica/despublica al guardar y la crea al instante -despublicada- si todavia no existia; Calendario de atencion y Pedidos externos ahora aparecen dentro de "Tienda Online" cuando esta desplegada (misma configuracion de siempre); nuevo uploader de **logo y portada** de la tienda; anteriores: la Configuracion de Delivery se mudo al menu Configuracion como item propio "Delivery / Take Away" en `/configuracion/delivery` -- el link viejo desde el engranaje de Pedidos Delivery sigue funcionando y redirige; nuevo apartado "Tienda Online" en esa misma pantalla para crear y personalizar la tienda publica de la sucursal: slug, publicacion, IDs de analytics de Google Analytics 4 y Meta Pixel, y apariencia (colores, tipografia, bordes, densidad); anteriores: modulo Compras reescrito por completo: editor de compra en modal fullscreen con grilla tipo planilla, costos y utilidad en Articulos/Categorias/Configuracion, revision de precios post-compra y repricing automatico, cuenta corriente y pagos a proveedores, reportes de compras; nuevo menu padre "Compras"; + factura de servicio y percepciones habituales por proveedor; + navegacion con Enter en el encabezado de compras, boton "Usar como precio" en Articulos, precio de venta unico por sucursal, aviso de CUITs con condiciones de IVA mixtas y precarga de descuentos de la ultima compra; + coeficiente computable de percepciones sufridas (base/monto sugeridos, campo "Coef." editable por compra), perfil fiscal del proveedor en modal propio y reorden del ABM de Articulos en secciones "Costos" / "Utilidad y precio" / "Configuracion en la sucursal"; + hardening del circuito de precios e impuestos: el precio de venta es siempre FINAL con IVA incluido (se quita el switch del ABM), el precio de venta en comercios de una sola sucursal tambien edita el efectivo de la sucursal, revision de precios post-compra con piso de costo (badge "bajo costo"), percepciones con comprador no inscripto van 100% al costo, notas de credito de proveedor con precarga de percepciones de la compra origen y aviso si exceden a la origen, movimientos fiscales generados por compras/ventas/conciliacion ya no se anulan a mano, y cambio masivo de precios extendido a costos; + hardening fiscal saliente/ventas: la percepcion aplicada en ventas ahora se calcula solo sobre el neto gravado (baja si hay items exentos), los pedidos delivery convertidos en venta cobran la percepcion correspondiente antes de convertir, el reintento de facturacion y el cambio de forma de pago conservan las percepciones del comprobante original, y la cortesia total con concepto libre ya no da error al confirmar)
...
- [15. Pedidos Delivery / Take-Away](#15-pedidos-delivery--take-away)
  ...
  - [15.8 Encargos (pedidos por dia futuro)](#158-encargos-pedidos-por-dia-futuro)
- [Glosario](#glosario)
...
**Promociones/listas condicionadas por forma de pago con pago dividido**: si una promocion, lista de precios o cupon esta restringido a determinada/s forma/s de pago (por ejemplo "solo efectivo"), con un pago dividido en varias formas de pago el beneficio se mantiene **solo si TODAS las formas de pago del desglose estan habilitadas** para ese beneficio. Si agrega al desglose una forma de pago no habilitada, el beneficio se cae automaticamente y el monto pendiente se actualiza; si despues la quita, el beneficio puede volver a aplicar. No importa en que orden declare las formas de pago: el resultado es siempre el mismo.

**En Pedidos Delivery, si el pedido tiene costo de envio**: el envio es un valor fijo que nunca recibe descuento ni recargo por forma de pago. Con el pago dividido en varias formas de pago, el sistema asigna primero los **articulos** (nunca el envio) a cada forma de pago -- empezando por la que tiene mayor descuento -- y calcula el ajuste de cada una solo sobre la porcion de articulos que le toco. El modal muestra esta base debajo de cada ajuste (por ejemplo "-10% s/ $900 de articulos"); si una forma de pago del desglose termina cubriendo unicamente el envio, se aclara "sin ajuste: este pago cubre solo envio". Ver seccion 15.4 para el desglose completo de totales en Delivery.
...
> **En Pedidos Delivery, si el pedido tiene costo de envio**, esta misma seccion cambia el renglon "Total productos" por tres lineas: **"Total articulos"** (sin el envio), **"Envio (fijo, sin ajustes)"** y **"Total pedido"** (ver seccion 15.4). El envio nunca recibe descuento ni recargo por forma de pago, aunque el pago se divida en varias formas de pago.
...
- **Imagen para la tienda (opcional)**: la foto que se usa como banner del encabezado de esta categoria en el catalogo de la tienda online (ver seccion 15.6, "Articulos de la tienda" -- ahi tambien se puede subir/cambiar y ademas elegir el punto focal). Se sube arrastrando o eligiendo un archivo (JPG, PNG o WebP); con una imagen ya cargada aparece la miniatura y el boton **"Quitar"**.
...
- **Disponible en tienda online**: Solo para formas de pago no mixtas. Debajo de "Sucursales" aparece una seccion con un checkbox por sucursal que define en cuales los clientes de la tienda online pueden elegir esta forma de pago para pagar contra entrega o retiro. Por defecto se marcan todas las sucursales seleccionadas. Si una sucursal no esta tildada aqui, esa forma de pago no aparece como opcion en la tienda online de esa sucursal (aunque siga activa en el punto de venta presencial).
...
3. Elija el **modo de cobro**: determina como se procesa el pago al usar esta forma de pago en el punto de venta.
   ...
   - **Checkout Online**: unico modo de la integracion "Mercado Pago - Checkout Online" (ver seccion 12.11). No se usa en el punto de venta presencial: define que esta forma de pago se cobre online en la Tienda Online (ver seccion 15.1, "Pago online en la tienda"). Con esta integracion activa, la forma de pago se ofrece en la tienda EXCLUSIVAMENTE como pago online (por ahora no convive con la variante "pagar al recibir" de la misma forma de pago).
...
**Paso 4 - Condiciones:**
- Forma de venta (seleccion multiple: lista de checkboxes con contador de seleccionados; sin marcar ninguna aplica a todas).
- Canal de venta (seleccion multiple, igual dinamica que forma de venta; sin marcar ninguno aplica a todos).
- Formas de pago (seleccion multiple).
- Monto minimo y monto maximo de la venta (dejar vacio = sin tope; si se cargan ambos, la promocion solo aplica cuando el total cae dentro del rango).
- Cantidad minima y maxima de articulos (misma logica de rango que el monto).
- Vigencia (fechas, dias de la semana, horarios).
- Usos maximos.
- Combinable: Si puede aplicarse junto con otras promociones.

Cuando se marca mas de una forma de venta o canal, alcanza con que la venta coincida con UNA de las seleccionadas para que la condicion se cumpla (no hace falta cumplir todas). El listado de promociones y el simulador muestran el rango de monto/cantidad de forma legible (por ejemplo "Total: entre $1000 y $5000" o "Cantidad maxima: 10").
...
**Paso 3 - Condiciones:**
Similar a las promociones comunes: vigencia, dias, horarios, forma de venta (seleccion multiple), canal (seleccion multiple), formas de pago (seleccion multiple), usos maximos. Las promociones especiales NO tienen monto/cantidad minima ni maxima (su mecanica NxM/combo/menu ya define las cantidades).
...
Permite conectar las sucursales y cajas del comercio con pasarelas de pago externas. Cada producto de Mercado Pago (QR, Point y Checkout Online) es una integracion independiente con sus propias credenciales y configuracion.
...
#### Mercado Pago - Checkout Online (pago online en la tienda)

Checkout Online es el producto de Mercado Pago que permite que el consumidor **pague desde la Tienda Online**, en la propia pagina segura de Mercado Pago, antes de que el pedido llegue al comercio. Es un producto MP separado de QR y de Point: requiere su **propia aplicacion y sus propias credenciales**, aunque sea de la misma cuenta de Mercado Pago.

##### Configurar la integracion Checkout Online

Haga clic en **"Configurar"** junto a Mercado Pago - Checkout Online:

1. **Modo**: "Test" o "Produccion".
2. **User ID Externo**: ID de la cuenta MP (puede ser el mismo que QR/Point si es la misma cuenta).
3. **Access Token (Produccion)** y **Access Token (Test)** correspondientes a la aplicacion de Checkout Pro.
4. Haga clic en **"Probar Conexion"** y luego en **"Guardar"**.

La ayuda desplegable ("¿Como obtener las credenciales?") de esta integracion indica los pasos propios de Checkout Online: en el panel de desarrolladores de Mercado Pago, tipo de integracion **"Pagos online"** y producto **"Checkout Pro"** (en vez de "Pagos presenciales"/"QR" o "Point").

No requiere sincronizar sucursal ni cajas (no usa Store/POS ni terminales): una vez guardadas las credenciales, la integracion queda lista para asignarse a una forma de pago (seccion 12.4, modo **"Checkout Online"**). A partir de ahi, esa forma de pago se ofrece en la Tienda Online como pago online (ver seccion 15.1, "Pago online en la tienda").

> Igual que con QR y Point, si las mismas credenciales (mismo User ID Externo) se usan en varias integraciones o sucursales, todos los cobros -- QR presencial, Point y Checkout Online -- convergen a la misma Cuenta Empresa (ver "Cuenta empresa y saldo del proveedor" mas abajo).

#### Confirmacion en tiempo real (webhook — aplica a QR, Point y Checkout Online)

Cuando Mercado Pago esta configurado con un **Webhook Secret**, las confirmaciones de cobro llegan al sistema de forma instantanea: MP envia una notificacion al servidor en el momento en que el cliente aprueba el pago. Para QR y Point, el modal de espera del cajero se cierra al instante sin necesidad de esperar ciclos de consulta. Para Checkout Online, la acreditacion saca al pedido de "esperando pago online" y lo pasa a "por aceptar" (o lo confirma) apenas Mercado Pago avisa, sin intervencion del comercio.
...
Debajo del campo "Webhook Secret" hay un boton **"¿Como obtener el Webhook Secret?"** que despliega la guia paso a paso: en la aplicacion creada en Mercado Pago ir a "Notificaciones" → "Webhooks" → "Configurar notificaciones", asegurarse de estar en la pestaña **"Modo productivo"**, pegar la URL del sistema (el panel la muestra con un boton "Copiar"), seleccionar unicamente el evento **"Order (Mercado Pago)"** (QR y Point) o **"Pagos"** (Checkout Online), guardar y copiar la clave secreta generada al campo del formulario. Cada aplicacion de MP (QR, Point, Checkout Online) tiene su propio webhook y su propia clave: los pasos se repiten en cada una.

> Si el Webhook Secret no esta configurado, el sistema igual funciona correctamente (la confirmacion via webhook sigue llegando; el secret solo agrega la verificacion de firma). Para QR y Point, el modal del cajero ademas consulta el estado del pago cada 3 segundos como respaldo. Checkout Online no tiene ese polling del lado del panel (nadie esta esperando frente a una pantalla del comercio): la acreditacion depende del webhook, y del lado del consumidor la pagina de retorno de la tienda tambien puede reconsultar el estado.
...
- Cuando un cobro por integracion se **confirma** (via webhook automatico, polling o confirmacion manual), el sistema registra un movimiento de ingreso en esa cuenta con el concepto "Cobro por integracion de pago". Si el pedido tenia propina online (seccion 15.6), la propina se registra como un ingreso APARTE con concepto propio "Propina online" -- nunca se mezcla con el cobro del pedido.
...
- Si dos sucursales comparten la misma cuenta de Mercado Pago (mismo User ID Externo), sus cobros convergen al saldo de una sola Cuenta Empresa -- incluye QR, Point y Checkout Online por igual.
- Los cobros en modo **Test** no generan ningun movimiento ni afectan el saldo.

**Anular una venta cobrada por integracion (QR / Point):**
...
**Rechazar/cancelar un pedido de la tienda pagado con Checkout Online:**

A diferencia de QR/Point, el pago online SI tiene devolucion real por API (ver seccion 15.1, "Pago online en la tienda"): al rechazar o cancelar el pedido, el sistema devuelve el cobro automaticamente en Mercado Pago (propina incluida) y registra el egreso como contraasiento en la Cuenta Empresa -- el movimiento de ingreso original nunca se edita ni se borra.
...
Igual que Pedidos por Mostrador, la pagina es fullscreen con vistas **Lista** y **Kanban** intercambiables (la preferencia se guarda en el dispositivo). Si la sucursal tiene activados los **encargos** (pedidos para un dia futuro, seccion 15.6), el toggle suma un tercer boton, **Encargos** (con contador), descripto en la seccion 15.8. El header incluye contador de pedidos, badge de nuevos, chips de filtros activos, buscador, boton Filtros, boton Refrescar, toggle Lista/Kanban(/Encargos), boton Nuevo Pedido y el **engranaje de Configuracion** (abre la Configuracion de Delivery, seccion 15.6, con el permiso `func.pedidos_delivery.config`). Esa misma pantalla tambien es accesible directo desde **Menu > Configuracion > Delivery / Take Away**.

#### Pedidos por aceptar: burbuja flotante (escritorio) / bloque plegable (celular)

Cuando la sucursal acepta pedidos externos (tienda/API) de forma **manual**, aparecen los pedidos que llegaron sin confirmar todavia:

- **En escritorio**: una **burbuja flotante** (no una franja fija en el flujo de la pagina), con forma de pastilla: el numero de pedidos y el rotulo "por aceptar", sin datos de pedidos. Con la burbuja cerrada se puede **arrastrar** con el mouse a cualquier punto de la pantalla: al soltar, se **ancla sola al borde mas cercano** (izquierda, derecha, arriba o abajo) y esa posicion (borde + punto a lo largo del borde) queda guardada en ese equipo para esa sucursal -- sobrevive a recargar la pagina y a navegar entre pantallas. Arrastrar NO abre nada; un **click** (sin arrastrar) hace que la burbuja **se expanda ahi mismo** en una tarjeta flotante con el listado (no es una ventana que ocupa medio monitor: es una tarjeta angosta, con esquinas redondeadas, que crece desde donde esta la burbuja y no tapa el tablero de atras). La tarjeta se cierra con la **X**, con la tecla **Escape** o clickeando afuera de ella. La burbuja se puede llevar hasta el margen superior de la pantalla, por encima del menu, y sigue funcionando (queda siempre por delante).

Mientras haya pedidos sin aceptar, la burbuja **avisa cada 5 segundos**: suena un tono corto y la burbuja vibra en ese momento; ademas el numero **late** de forma permanente. El sonido se corta si la pestana esta en segundo plano, y (por una restriccion de los navegadores) recien puede sonar despues del primer clic o tecla en la pagina. Al aceptar o rechazar todos los pedidos pendientes, la burbuja desaparece y el aviso se detiene.
- Si algun pedido supera el tiempo configurado sin aceptarse (`timeout_aceptacion_min`), la burbuja se pone **roja** (en vez del naranja habitual) y la tarjeta muestra el badge **"Demorado"** -- el pedido no se cancela solo.
- Si no hay pedidos por aceptar, la burbuja **no aparece**.
- **En celular**: se mantiene un bloque compacto plegable dentro del flujo normal de la pagina (la burbuja arrastrable es solo de escritorio, donde hay espacio de sobra para moverla). La cabecera muestra el mismo contador pulsante y el mismo badge "Demorado"; un click la despliega.

Un pedido externo nuevo (borrador) aparece **al instante**, sin necesidad de refrescar la pagina, con un destello visual (borde/anillo) durante unos segundos para llamar la atencion, tanto en la burbuja de escritorio como en el bloque de celular.

Cada pedido por aceptar del listado (dentro de la tarjeta de la burbuja o del bloque plegado) muestra:
- La **promesa de entrega elegida por el cliente** en la tienda (si trajo alguna): franja horaria, "Lo antes posible" o el dia/hora si es un encargo (seccion 15.8).
- Boton **Aceptar**: si el pedido trae una promesa del cliente, el modal la muestra primero ("El cliente eligio: ...") con un boton **"Aceptar como lo pidio"** que confirma el pedido tal cual, sin recalcular ni pisar la hora -- las opciones de demora/franja quedan debajo como alternativa para cuando no se puede cumplir tal cual se pidio. Si el pedido no trae promesa propia: en modo de promesa manual abre el modal para elegir la demora (botones +0, +10, +15... configurables) y fija la hora pactada informada al consumidor; en modo automatico confirma directo (salvo que sea un encargo, que conserva su fecha/hora sin recalculo por distancia).
- Boton **Rechazar**: pide un motivo. Si el pedido tenia un pago online (Mercado Pago Checkout) ya acreditado, el sistema intenta la **devolucion automatica** antes de confirmar el rechazo (nunca lo bloquea): si el reintegro sale bien, el pago queda devuelto y se avisa al consumidor por su canal de seguimiento; si Mercado Pago no responde o falla, el pedido igual queda rechazado pero el cobro queda marcado **"Pago a devolver"** (badge rojo, ver mas abajo) con reintento manual desde el detalle.
- Si pasa el tiempo configurado (`timeout_aceptacion_min`) sin aceptarse, el pedido se resalta como **"Demorado"** (no se cancela solo).

#### Pago online en la tienda (Mercado Pago Checkout)

Cuando una forma de pago tiene la integracion **"Mercado Pago - Checkout Online"** activa (seccion 12.11), esa forma de pago se ofrece en la tienda **solo como pago online**: el consumidor paga en la pagina segura de Mercado Pago antes de que el pedido llegue al panel.

- **Mientras el pago no se acredita**, el pedido es un borrador **invisible para el comercio**: no aparece en "por aceptar", no suena la burbuja ni el bloque plegable. En el listado general (con el filtro de borradores) se ve con el badge naranja **"Esperando pago online"**.
- **Al acreditarse el pago** (Mercado Pago confirma via webhook), el pedido pasa recien ahi a "por aceptar" (aceptacion manual) o queda confirmado directo (aceptacion automatica) -- desde ese momento se comporta como cualquier otro pedido.
- **Si la tx de pago vence o falla** sin que el consumidor pague, el borrador se cancela solo (motivo "Pago online no completado"); el consumidor puede reintentar el pago desde la tienda mientras el pedido siga esperando.
- **Modal "Ver detalle"**: los pedidos con pago online muestran el bloque **"Pago online (Mercado Pago)"** con el monto y el estado (**Acreditado**, **Esperando pago online**, **Devuelto al cliente** o **Pago a devolver**, este ultimo con el boton **"Reintentar devolucion"**). Si el pedido tiene propina cargada, se muestra debajo: **"Propina para el repartidor: $X"**.
- **Pedido ya pagado online = sin cambios de monto**: no se pueden editar items ni corregir la direccion con delta de costo de envio; las unicas opciones son aceptarlo tal cual o rechazarlo (con devolucion). Un ajuste chico se resuelve contra entrega, fuera del pago online.
- **Cancelado con cobro sin devolver**: en el listado, un pedido cancelado cuyo refund fallo se distingue con el badge rojo **"Pago a devolver"**.
...
**Aclaracion del cliente por item**: en un pedido que vino de la tienda online, el cliente puede dejar una nota puntual en un item del carrito (por ejemplo "sin cebolla", "bien cocida"), ademas de las observaciones generales del pedido. Esa aclaracion se muestra en texto ambar debajo del nombre del articulo, tanto en el modal "Ver detalle" como en la comanda impresa (ticket de cocina) y en la comanda HTML.

**Desglose de envio en el detalle del pedido**: en el modal "Ver detalle", si el pedido tiene costo de envio, los totales muestran **"Subtotal articulos"** y **"Envio"** en lineas separadas (en vez de un unico "Subtotal") -- el envio con un badge ambar **"manual"** si el operario lo cargo a mano en vez de dejarlo calculado automaticamente. Si hay ajuste por forma de pago, se aclara que aplica "s/ articulos" (el envio nunca recibe descuento ni recargo).

**Palabra clave de entrega**: si la sucursal tiene activada esta opcion (seccion 15.6) y el pedido delivery ya tiene su palabra generada, el modal "Ver detalle" la muestra junto al repartidor y su telefono, con un badge "Palabra clave: ..." -- sirve para que el repartidor la confirme con el cliente al entregar.

**Editar direccion de entrega desde "Ver detalle"**: junto a la direccion, en pedidos delivery que todavia no estan entregados, facturados ni cancelados (esten por aceptar o ya confirmados), un boton con icono de mapa abre un modal para corregir la direccion escrita, la referencia y (si la sucursal georreferencia pedidos) la ubicacion en el mapa arrastrando el pin. Abrir este modal cierra el detalle; al guardar o cancelar, el detalle se reabre solo. Si mover el pin cambia la zona o el costo de envio calculado, el sistema muestra el cambio (zona y costo antes -> despues, mas la distancia) y pide **confirmarlo antes de guardar**; si la nueva ubicacion queda fuera del area de reparto, avisa y ofrece **"Guardar igual"** (el pedido queda sin zona, con el costo de envio anterior); si el pedido ya tiene pagos registrados, el envio no se recalcula -- se guarda solo la direccion y se avisa. Con la georreferenciacion apagada, el modal es solo texto (sin mapa) y guarda sin recotizar.
...
Si la sucursal tiene apagado alguno de los dos tildes de modalidad (seccion 15.6, "Delivery habilitado" o "Habilitar take-away"), el boton de esa modalidad directamente no aparece en el selector y, si el pedido ya estaba en esa modalidad al cambiar la configuracion, el sistema lo pasa solo a la modalidad habilitada.

#### Direccion de entrega

Boton **"Direccion de entrega"** abre un modal con el mismo picker de Google Maps usado en el resto del sistema (autocomplete, click/arrastre del marcador) mas un campo de **referencia** (piso/depto/indicaciones). Si el cliente elegido ya tiene una direccion de entrega guardada, se precarga con mapa y coordenadas. La opcion **"Entregar en otra direccion"** permite usar una direccion distinta sin pisar la guardada del cliente.

Arriba del mapa hay siempre un **buscador de direcciones de Google**: se toca, se escribe la direccion y se elige de la lista. Al elegirla, se completan de una la direccion escrita y la ubicacion en el mapa (que se abre solo para que se vea donde cayo el pin, y ahi se puede terminar de ajustar arrastrandolo). El buscador tambien se carga recien al tocarlo: si solo se corrige el texto o la referencia, no se hace ninguna llamada a Google.

El mapa arranca **cerrado** (boton "Abrir mapa"): mientras no se lo toque, no se hace ninguna llamada a Google. Al abrirlo, el pin del domicilio es el marcador **rojo estandar** y, si la sucursal tiene su ubicacion cargada, se dibuja como contexto (solo visual, no decide nada) el **pin naranja del local** y las **zonas de reparto activas con su nombre** (o el circulo del radio general si no hay zonas dibujadas) -- ayuda a ver de un vistazo si la direccion cae adentro del area de reparto antes incluso de cotizar. No se muestra el boton "Usar mi ubicacion actual": la direccion es la del cliente, no la del operador que la esta cargando.
...
#### Totales y pago con envio

Cuando el pedido tiene costo de envio, el resumen de totales de la columna derecha desglosa **"Total articulos"** + **"Envio (fijo, sin ajustes)"** + **"Total pedido"** en vez de un unico total mezclado (ver seccion 3.1, Resumen de totales). El envio nunca recibe descuento ni recargo por promocion, cupon, puntos ni forma de pago.

Si el pago se divide en varias formas de pago (pago mixto, seccion 3.1), el modal de pago aclara la base de cada ajuste: el sistema asigna primero los articulos a cada forma de pago -- priorizando la que tiene mayor descuento -- y nunca asigna una porcion de envio a ninguna. Por ejemplo, con $1000 de articulos + $500 de envio y un pago de $900 en efectivo con 10% de descuento, el descuento se calcula sobre los $900 de articulos que cubre esa forma de pago ("-10% s/ $900 de articulos"), no sobre una porcion proporcional que incluyera envio.

Una promocion o lista de precios condicionada a una forma de pago especifica (por ejemplo "solo efectivo") se valida contra **todas** las formas de pago del desglose: si alguna porcion del pago usa una forma no habilitada, el beneficio no se aplica (misma regla que ya tenian los cupones).
...
#### General
- **Habilitar modulo de pedidos** (antes "Usar Delivery"/"Habilitar delivery en esta sucursal"): activa el panel de Pedidos Delivery, la tienda online y el marketplace para la sucursal (delivery y/o take-away). Si se apaga con la tienda online ya publicada, aparece un aviso rojo "La tienda online quedara fuera de linea" (la tienda deja de responder). Al **publicar** la tienda online (seccion "Tienda Online" mas abajo) estando este tilde apagado, se enciende automaticamente con un aviso "Se habilito el modulo de pedidos (la tienda online lo necesita)" -- publicar la tienda sin el modulo de pedidos prendido la dejaba publicada pero inaccesible.
- **Delivery habilitado**: permite pedidos con envio a domicilio. En paridad con "Habilitar take-away": una sucursal puede tener uno solo de los dos apagado (por ejemplo, un local que solo atiende para retirar apaga este tilde y deja prendido take-away) pero no ambos a la vez sin perder el modulo. Con este tilde apagado, el alta de pedido (seccion 15.2) no ofrece la opcion Delivery.
- Habilitar take-away.
- Exigir repartidor para despachar.
- **Palabra clave de entrega**: cada pedido delivery genera, al confirmarse, una palabra simple (por ejemplo "Tigre" o "Luna", de una lista fija) que sirve para validar la entrega en la puerta. El cliente la ve en el seguimiento de la tienda online apenas el pedido tiene repartidor asignado y la dice al recibirlo; el repartidor la ve en el panel (seccion 15.2). Una vez generada, la palabra no cambia aunque el pedido se reconfirme.
...
- **Datos del cliente en el checkout**: dentro del mismo apartado "Pedidos externos", que datos pide la tienda al cliente antes de confirmar el pedido:
  - **Email**: **No pedir**, **Pedir (opcional)** o **Pedir (obligatorio)**. Con "obligatorio", el pedido no se puede confirmar sin un email (el del cliente logueado en su cuenta tambien cuenta).
  - **Pedir fecha de cumpleanios**: checkbox, siempre **opcional** (nunca se puede volver obligatorio). Cuando esta activo, la tienda muestra el campo con la leyenda de que se usa para promociones y descuentos. El cumpleanios cargado queda guardado en la ficha del cliente (seccion 11.1) y, si el pedido es de un cliente con cuenta en la tienda, tambien en su cuenta para no volver a pedirlo en otro comercio.
  - **Entre calles** (solo aplica a pedidos de tipo Delivery, no a Take-away): **No pedir**, **Pedir (opcional)** o **Pedir (obligatorio)**. Con "obligatorio", un pedido de delivery no se puede confirmar sin ese dato. Lo que el cliente escribe (por ejemplo "San Martin y Belgrano") se agrega junto con la referencia (piso/depto/indicaciones) en la direccion de entrega del pedido, visible en el Kanban, la Lista y el detalle del pedido.
- **Ofrecer propina al pagar online (para el repartidor)**: checkbox, solo tiene efecto si la sucursal tiene alguna forma de pago con la integracion de Checkout Online activa (seccion 12.11). Con el tilde prendido aparece el campo **"Porcentajes sugeridos de propina"** (por ejemplo `5, 10, 15`, separados por coma) que la tienda ofrece como chips al pagar online, ademas de un monto libre a criterio del cliente. La propina se cobra junto con el pedido pero se registra por separado: nunca forma parte del total del pedido ni de lo facturable al convertirlo en venta, y se muestra aparte en el detalle del pedido ("Propina para el repartidor: $X"). Pensada para que un modulo futuro la rinda al repartidor.
...
#### Encargos (pedidos por dia futuro)

Aparece en la misma zona y con el mismo auto-guardado que "Pedidos externos" y "Calendario de atencion" (fuera de "Tienda Online" si la tienda no existe o esta apagada; dentro de "Tienda Online" si esta prendida). Habilita un circuito aparte al delivery/take-away normal: pedidos para un **dia y horario futuros** (por ejemplo, tortas o pedidos grandes que se preparan con anticipacion). Ver seccion 15.8 para como se ven y gestionan estos pedidos en el panel.

- **Tomar pedidos por encargue** (switch maestro): al activarlo por primera vez, el sistema **precarga** el calendario de encargos con los mismos dias/horarios/feriados que el Calendario de atencion; a partir de ahi son dos calendarios **independientes** (editar uno no afecta al otro).
- **Anticipacion minima (horas)**: cuanto tiempo antes, como minimo, se puede encargar (por ejemplo, 24 horas).
- **Hasta cuantos dias adelante**: el horizonte maximo hacia el futuro para encargar (por ejemplo, 30 dias).
- **Aparecen en el panel (min antes)**: cuantos minutos antes de la hora del encargo el pedido pasa solo al tablero de atencion (Kanban/Lista); hasta ese momento vive unicamente en la solapa **Encargos** (seccion 15.8).
- **Dias con encargos**: dias de la semana habilitados en este calendario propio.
- **Horarios de entrega/retiro de encargos**: rangos horarios por dia (vacio = todo el dia), con la misma mecanica del calendario de atencion (repeater con dias + desde/hasta, se pueden agregar varios rangos).
- **Fechas sin encargos**: feriados propios de este calendario (independientes de los feriados de atencion).

Este calendario es independiente: permite tomar encargos para dias en que el local no atiende al publico (por ejemplo, encargar en un dia cerrado para retirar mas adelante). Ademas del calendario, cada articulo se habilita o deshabilita para encargos **individualmente** desde "Articulos de la tienda" (ver mas abajo) con el checkbox **"Disponible para encargos"**.
...
- Si al guardar la publicacion el apartado **General** (mas arriba) tiene apagado **"Habilitar modulo de pedidos"**, se enciende automaticamente (la tienda online lo necesita para responder) y aparece un aviso "Se habilito el modulo de pedidos (la tienda online lo necesita)".
...
- Con la tienda **publicada** (con datos ya guardados), el marco embebe **la tienda real**: los cambios de colores, tipografia, bordes redondeados y densidad, y tambien el logo/portada recien elegidos (aunque todavia no se hayan guardado), se ven reflejados ahi al instante, como si se estuviera navegando la tienda de verdad. Al hacer clic en "Guardar tienda", el panel se actualiza mostrando ya lo persistido. Arriba del marco, junto al link **"Abrir en pestana nueva"** (para ver la tienda a pantalla completa), hay un boton con icono de **sol/luna** que fuerza el modo oscuro o claro dentro del visor -- util para revisar que los colores personalizados se vean bien en ambos modos sin tener que cambiar el modo del propio dispositivo ni salir del panel.
...
- **Apariencia de la tienda**: identidad visual y personalizacion de la tienda publica (design tokens que consume el sitio):
  ...
  - Bordes redondeados: Rectos / Suaves / Medios / Amplios / Redondos. Aplica a tarjetas y carrito (no al logo, que tiene su propia forma — ver abajo).
  - **Forma del logo**: Recto / Suave / Medio / Amplio / Redondo (por defecto Redondo). Selector independiente de "Bordes redondeados": define solo la forma del logo en el encabezado de la tienda, sin afectar tarjetas ni carrito.
  - Densidad del contenido: Compacta / Normal / Amplia.
  - Con una portada cargada, aparece un control adicional sobre la miniatura: el selector de **Encuadre** (**Arriba** / **Centro** / **Abajo**), que define que parte vertical de la imagen se recorta cuando no entra completa. La miniatura del panel muestra el recorte real elegido. La portada se muestra siempre tal cual (ya no existe la opcion de fundirla con un velo del color primario).
  - **Historias destacadas**: hasta **3 fotos** que rodean el logo del encabezado con un anillo estilo "historias" (como redes sociales); al tocar el logo se reproducen en orden. Se suben con el boton **"Agregar"** (arrastrar o elegir archivo — JPG, PNG o WebP, maximo 5MB, ideal formato vertical 1080x1920) y **se persisten al instante** (no forman parte del boton "Guardar tienda"): se pueden elegir varias fotos en la misma seleccion o en selecciones sucesivas, todas se van sumando hasta llegar al maximo de 3 (si se supera, aparece un aviso). Las fotos ya guardadas se **reordenan arrastrandolas** (drag & drop) y se **eliminan** al instante, sin esperar a ningun guardado. Debajo, el checkbox **"Mostrar tambien las promociones disponibles como historia"** agrega, al final de las historias, un slide con las promociones vigentes del dia y sus condiciones (agrupa lo que antes era el aviso "Promociones de hoy" de la pagina principal); tambien activa el anillo del logo aunque todavia no se haya cargado ninguna foto.
  - **Contenido y redes**: **Slogan** (hasta 120 caracteres, se muestra en el encabezado de la tienda debajo del nombre), **Texto descriptivo** (hasta 5000 caracteres, se muestra dentro del panel "Informacion del comercio" de la tienda publica — si se deja vacio, no aparece ese texto), y las URLs de **Facebook** e **Instagram** (deben ser el link completo a la pagina/perfil, ej. `https://facebook.com/micomercio`; con la URL cargada aparece el boton de esa red con su icono en el encabezado de la tienda).
  - **Presentacion del catalogo**: como se muestran los articulos (**Grilla**, foto protagonista, o **Renglones**, foto mas chica con el detalle al lado), el checkbox **"Categorias desplegables"** (cada titulo de categoria del catalogo se puede plegar/desplegar tocandolo, como un acordeon), el modo de los **Articulos destacados** (**Banner deslizable** arriba de todo, **Tarjeta grande** intercalada entre los articulos, o **Sin seccion de destacados**), el **Adorno del destacado** (**Ninguno**, **Brillo alrededor (glow)**, **Badge con icono de destacado** o **Brillo + badge** — en tarjeta grande decora la tarjeta, en banner decora el titulo de la seccion; solo se deshabilita cuando el modo es "Sin seccion de destacados") y el **Titulo de la seccion de destacados** (texto libre hasta 40 caracteres; vacio usa el titulo por defecto "Destacados"). Los cambios de esta seccion se ven en la tienda recien al guardar.
  - **Articulos de la tienda**: seccion para personalizar CADA articulo de la vidriera (fotos, badges, destacado y orden) y los **badges y el banner de cada categoria**, debajo de "Presentacion del catalogo". A diferencia del resto del apartado, ACA CADA ACCION SE GUARDA AL INSTANTE (no forma parte del formulario ni del boton "Guardar tienda") y el visor de la derecha se actualiza solo, poco despues de cada cambio (no hace falta recargar a mano).
    ...
    - **Badges de categoria**: junto al titulo de cada categoria hay un boton con icono de etiqueta que abre un editor con los mismos chips predefinidos que los badges de articulo (ver mas abajo) mas un badge propio de texto libre, hasta **4 por categoria**; se guardan al instante. Los badges activos quedan visibles como chips de solo lectura junto al titulo de la categoria (no hace falta abrir el editor para verlos). La tienda los muestra junto al titulo del grupo de esa categoria.
    - **Banner de la categoria**: junto al boton de badges hay un boton con icono de foto que abre/cierra un editor con la foto de fondo del encabezado de esa categoria en la tienda (la tienda la muestra como una franja panoramica con velo oscuro y el nombre de la categoria encima). Con una foto cargada, el editor muestra una vista previa recortada en la MISMA proporcion que va a tener la franja en la tienda, con un punto marcado encima (el "punto focal"): como la foto casi nunca tiene esa proporcion, un clic en cualquier parte de la vista previa elige que parte de la imagen queda visible al recortarse, y se guarda al instante (sin boton aparte). Los botones **"Subir banner de la categoria"** / **"Cambiar banner de la categoria"** (arrastrar o elegir archivo -- JPG, PNG o WebP, maximo 5MB, ideal 1600x600 px o mas ancha) y **"Quitar banner de la categoria"** completan el editor; sin foto cargada, el encabezado de esa categoria se ve en la tienda igual que si nunca se hubiera tocado esta opcion. Es la misma imagen que se puede cargar desde Articulos > Categorias (seccion 10.2); cualquiera de los dos lugares la actualiza.
    - Cada fila muestra: una miniatura (la primera foto de la galeria de tienda si tiene, o si no la imagen del articulo del panel), el nombre, los badges asignados (si tiene), una **estrella** para marcar/desmarcar el articulo como **destacado** (reutiliza el mismo destacado que se usa en el modo "Tarjeta grande"/"Banner deslizable" de arriba), un icono para permitir el **canje por puntos en la tienda** (ver mas abajo) y un boton **"Fotos (n)"** que expande, debajo de la fila, el editor de galeria y badges de ese articulo.
    - **Canje por puntos en la tienda**: junto a la estrella de destacado, otro icono habilita o deshabilita que el articulo se pueda canjear por puntos desde la tienda online (guardado al instante, propio de la sucursal activa). Con el canje habilitado aparece al lado un **campo numerico opcional** para definir cuantos puntos cuesta canjear ESE articulo (es el mismo valor de puntos de canje del articulo que usa el punto de venta). **Si se deja vacio ("Auto"), el costo se calcula solo a partir del precio del dia** y el valor del punto configurado en el programa de fidelizacion -- igual que funciona el canje en una venta del mostrador. Cargar un numero fija ese costo exacto; borrarlo vuelve al calculo automatico.
    ...
    - **Disponible para encargos** (RF-T16): checkbox en la fila expandida del articulo (junto a la descripcion para la tienda) que habilita o no ese articulo para pedidos por encargue (dia futuro, seccion 15.8). Se guarda al instante, igual que el resto de esta seccion. Solo tiene efecto si la sucursal tiene activado "Tomar pedidos por encargue" (mas arriba); si no, el checkbox queda visible pero sin uso.
  ...
  - Boton **"Vista previa"** (visible solo en pantallas angostas, donde no entra el panel fijo de la derecha): abre un panel lateral con una simulacion de como se ve la tienda (encabezado con logo y portada, chips de categorias, tarjetas de producto, barra de carrito) que se actualiza al instante a medida que se cambian los colores, la tipografia, los bordes redondeados, la forma del logo, la densidad, el encuadre de la portada y el slogan — util para probar combinaciones antes de guardar. Es la misma simulacion que se ve en el panel fijo cuando la tienda todavia no esta publicada.
  - Boton **"Restablecer al tema default"**: vuelve los 5 colores, la tipografia, los bordes, la forma del logo, la densidad, el encuadre de la portada y la presentacion del catalogo a los valores de fabrica del sistema (no requiere guardar aparte, pero los cambios quedan pendientes hasta hacer clic en "Guardar tienda"). No afecta las historias destacadas ya guardadas (esas se guardan y se borran al instante, no forman parte de este boton). El **slogan, el texto descriptivo y las redes sociales NO se borran** al restablecer: son contenido propio del comercio, no estetica.
- Boton **"Guardar tienda"**: confirma slug, analytics, apariencia, contenido/redes y presentacion del catalogo en un solo paso (NO toca la publicacion: eso lo maneja el switch + "Guardar configuracion", arriba). Las historias destacadas, y los badges/fotos/destacado/orden de "Articulos de la tienda", ya se guardan al instante y no dependen de este boton.

Con la tienda **publicada**, el panel de vista previa (fijo o el boton "Vista previa") refleja EN VIVO, sin necesidad de guardar, los colores/tipografia/bordes/forma del logo/densidad, el logo/portada, el encuadre de la portada, el slogan, el texto descriptivo y las redes sociales. Las historias destacadas tambien se ven reflejadas apenas se suben/reordenan/eliminan (se guardan al instante). La presentacion del catalogo (grilla/renglones, categorias desplegables, destacados, adorno, titulo de destacados) se ve recien al hacer clic en "Guardar tienda" (la vista previa se recarga sola en ese momento). Los cambios de la seccion **"Articulos de la tienda"** (fotos, badges de articulo y de categoria, banner de categoria, destacado, orden) tambien hacen que la vista previa se recargue sola, unos instantes despues de cada accion (no hace falta tocar "Guardar tienda", porque esos cambios ya se guardaron al instante).
...
### 15.8 Encargos (pedidos por dia futuro)

Los **encargos** son pedidos delivery/take-away para un dia y horario **futuros** (por ejemplo, tortas o pedidos grandes que se preparan con anticipacion), habilitados desde Configuracion > Delivery / Take Away, apartado "Encargos" (seccion 15.6). Reusan el mismo circuito de pedidos delivery/take-away (carrito, pagos, conversion a venta): lo unico distinto es CUANDO entran al tablero operativo del dia a dia.

#### Como se generan

- Desde la **tienda online o la API publica** (RF-T16): con los encargos activados, la tienda ofrece "Encargar para otro dia" con las fechas y horarios disponibles segun el calendario propio de encargos (seccion 15.6) — INCLUSO con la tienda cerrada en ese momento, porque el encargo se valida contra su propio calendario, no contra el de atencion.
- Solo se pueden encargar articulos marcados **"Disponible para encargos"** en la vidriera (seccion 15.6, "Articulos de la tienda"); un pedido con articulos no aptos se rechaza.
- El pedido queda con la hora del encargo como su **hora pactada** (mismo chip de promesa de entrega que el resto de los pedidos, seccion 15.2).

#### Solapa "Encargos" del panel

El toggle Lista/Kanban del panel de Pedidos Delivery (seccion 15.1) suma un tercer boton, **Encargos**, con un contador (badge fucsia) de los encargos pendientes. Ahi se listan, agrupados por dia (Hoy / Manana / dia de la semana), los encargos aceptados que TODAVIA no entraron al tablero de atencion: mientras falten mas minutos que los configurados en "Aparecen en el panel" (seccion 15.6) para su hora, el pedido vive solo en esta solapa — no aparece en la Vista Lista con el filtro "Solo activos" ni en el Kanban, para no saturar el dia a dia con pedidos de dentro de varios dias. Al acercarse la hora, el pedido **pasa solo** al tablero operativo (Lista/Kanban), sin ninguna accion manual.

Cada tarjeta de encargo muestra hora, numero de pedido, cliente, tipo (delivery/take-away), cantidad de items y total, con tres acciones:
- **Ver**: abre el detalle del pedido (igual que en Lista/Kanban).
- **Preparar ahora**: adelanta el encargo al tablero de atencion en este mismo momento (util si conviene arrancarlo antes de lo previsto); conserva la hora pactada original para el semaforo de demora por colores.
- **Cancelar**: cancela el pedido (igual que en Lista/Kanban).

El filtro **"Estado del pedido"** de la Vista Lista (seccion 15.1) suma la opcion **"Encargos programados"**, que muestra solo los encargos todavia fuera de ventana (los mismos que la solapa Encargos, pero en formato tabla/lista).

En cualquier vista (Lista, Kanban o Encargos), un pedido programado para un dia futuro muestra el badge fucsia **"Encargo · fecha/hora"** junto al resto de los badges del pedido.

#### Reporte "Produccion de encargos"

**Ruta**: boton **"Produccion"** dentro de la solapa Encargos, o directo en `/pedidos/encargos/produccion`.

Reporte de que hay que preparar por dia, segun los encargos ya **aceptados** (los pedidos "por aceptar" todavia no cuentan: no son un compromiso en firme). Se elige un rango de fechas (por defecto, hoy y los siguientes 7 dias) y el reporte agrupa, dia por dia, los articulos encargados con la cantidad total sumada de todos los pedidos de ese dia (ordenados de mayor a menor cantidad). Cada renglon de articulo se puede desplegar para ver el detalle: hora, numero de pedido, cliente, tipo y cantidad de cada pedido que lo compone. Incluye boton **Imprimir** (usa la funcion de impresion del navegador) y un boton para volver al panel de pedidos.
...
| **Badge (tienda)** | Etiqueta corta que se muestra sobre un articulo o junto al titulo de una categoria en la tienda online (ej. "Sin TACC", "Nuevo") para destacar una caracteristica. Se configura desde Configuracion > Delivery / Take Away > Tienda Online > Articulos de la tienda, hasta 4 por articulo y hasta 4 por categoria, predefinidos o de texto libre. |
| **Banner de categoria** | Foto de fondo del encabezado de una categoria en el catalogo de la tienda online (franja panoramica con velo oscuro y el nombre encima). Se sube desde Articulos > Categorias o desde Configuracion > Delivery / Take Away > Tienda Online > Articulos de la tienda, donde ademas se elige con un clic el **punto focal** (que parte de la foto queda visible al recortarse en la proporcion de la franja). |
...
| **Encargo** | Pedido delivery/take-away para un dia y horario futuros, con calendario propio de disponibilidad (independiente del de atencion) y articulos habilitados uno a uno. Vive aparte del tablero operativo hasta acercarse su hora. |
...
| **Modulo de pedidos** | Tilde maestro de la sucursal ("Habilitar modulo de pedidos", Configuracion > Delivery / Take Away, General) que activa el panel de Pedidos Delivery, la tienda online y la card del marketplace. Sin el, ninguna de las modalidades (delivery ni take-away) funciona; publicar la tienda online lo enciende solo si estaba apagado. |
