...
- **En escritorio**: una **burbuja circular flotante** (no una franja fija en el flujo de la pagina). Cerrada muestra unicamente la cantidad ("N por aceptar"), sin datos de pedidos. Con la burbuja cerrada se puede **arrastrar** con el mouse a cualquier punto de la pantalla: al soltar, se **ancla sola al borde mas cercano** (izquierda, derecha, arriba o abajo) y esa posicion (borde + punto a lo largo del borde) queda guardada en ese equipo para esa sucursal -- sobrevive a recargar la pagina y a navegar entre pantallas. Arrastrar NO abre nada; un **click** (sin arrastrar) despliega un **panel de media pantalla** que se desliza desde el borde donde quedo anclada la burbuja, superpuesto al tablero (el tablero de atras no se mueve ni se desordena). El panel se cierra con la **X**, con la tecla **Escape** o clickeando afuera de el.
- Si algun pedido supera el tiempo configurado sin aceptarse (`timeout_aceptacion_min`), la burbuja se pone **roja** (en vez del naranja habitual) y el panel muestra el badge **"Demorado"** -- el pedido no se cancela solo.
...
Cada pedido por aceptar del listado (dentro del panel de la burbuja o del bloque plegado) muestra:
