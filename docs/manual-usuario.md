...
- **Apariencia de la tienda**: identidad visual y personalizacion de la tienda publica (design tokens que consume el sitio):
  ...
  - **Articulos de la tienda**: seccion para personalizar CADA articulo de la vidriera (fotos, badges, destacado y orden) y los **badges de cada categoria**, debajo de "Presentacion del catalogo". A diferencia del resto del apartado, ACA CADA ACCION SE GUARDA AL INSTANTE (no forma parte del formulario ni del boton "Guardar tienda") y el visor de la derecha se actualiza solo, poco despues de cada cambio (no hace falta recargar a mano).
    ...
    - **Canje por puntos en la tienda**: junto a la estrella de destacado, otro icono habilita o deshabilita que el articulo se pueda canjear por puntos desde la tienda online (guardado al instante, propio de la sucursal activa). El costo en puntos NO se define aca: es el mismo que carga el articulo para el canje del punto de venta (RF-10) -- **si el articulo todavia no tiene puntos de canje cargados, el icono queda deshabilitado** (atenuado) con el aviso "Cargale puntos de canje en el articulo"; primero hay que cargarle puntos de canje al articulo y recien despues se puede habilitar el canje en la tienda. Deshabilitar el canje, en cambio, siempre esta disponible.
