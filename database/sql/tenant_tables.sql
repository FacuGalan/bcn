-- BCN Pymes - Tablas Tenant (template sin prefijo)
-- El prefijo se agregará dinámicamente al crear el comercio
...
CREATE TABLE `{{PREFIX}}articulos` (
  ...
  `es_servicio` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Si es servicio o producto',
  `controla_stock` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'Si controla stock',
...
CREATE TABLE `{{PREFIX}}proveedores` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `codigo` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Código único del proveedor',
  `nombre` varchar(191) COLLATE utf8mb4_unicode_ci NOT NULL,
  `razon_social` varchar(191) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `nombre_fiscal` varchar(191) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nombre fiscal para facturación',
  `cuit` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `email` varchar(191) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `telefono` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `direccion` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `condicion_iva_id` int(10) unsigned DEFAULT NULL COMMENT 'FK a condiciones_iva en config',
  `es_sucursal_interna` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Si es una sucursal interna (transferencias)',
  `sucursal_id` bigint(20) unsigned DEFAULT NULL COMMENT 'Sucursal vinculada si es interna',
  `cliente_id` bigint(20) unsigned DEFAULT NULL COMMENT 'Cliente vinculado (cuentas unificadas)',
  `activo` tinyint(1) NOT NULL DEFAULT '1',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_proveedor_codigo` (`codigo`),
  KEY `idx_proveedor_cuit` (`cuit`),
  KEY `idx_proveedor_condicion_iva` (`condicion_iva_id`),
  KEY `idx_proveedor_cliente` (`cliente_id`),
  KEY `idx_proveedor_sucursal` (`sucursal_id`),
  CONSTRAINT `fk_proveedor_cliente` FOREIGN KEY (`cliente_id`) REFERENCES `{{PREFIX}}clientes` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_proveedor_sucursal` FOREIGN KEY (`sucursal_id`) REFERENCES `{{PREFIX}}sucursales` (`id`) ON DELETE SET NULL
...
CREATE TABLE `{{PREFIX}}formas_pago` (
  ...
  `concepto` enum('efectivo','tarjeta_debito','tarjeta_credito','transferencia','wallet','cheque','otro') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'otro' COMMENT 'Concepto de la forma de pago',
...
CREATE TABLE `{{PREFIX}}cierres_turno` (
  ...
  `motivo_reversion` text COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
