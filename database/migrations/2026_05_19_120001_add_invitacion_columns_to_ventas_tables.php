...
/**
 * Feature "invitaciones" (cortesias) en Ventas.
 *
 * Agrega columnas de invitacion a `ventas` (cabecera) y `ventas_detalle`
 * (lineas), espejo de las columnas en `pedidos_mostrador*`. Permite marcar
 * una venta completa o renglones individuales como cortesia y que la
 * conversion Pedido -> Venta copie las columnas 1:1.
 *
        ...
        foreach ($comercios as $comercio) {
            ...
            try {
                DB::connection('pymes')->statement("
                    ...
                    ADD INDEX `idx_ventas_es_invitacion_total` (`es_invitacion_total`, `fecha`)
                ");
                ...
                DB::connection('pymes')->statement("
                    ...
                    ADD INDEX `idx_vd_es_invitacion` (`es_invitacion`)
                ");
            } catch (\Exception $e) {
                continue;
            }
    ...
    public function down(): void
    {
        ...
        foreach ($comercios as $comercio) {
            ...
            try {
                DB::connection('pymes')->statement("
                    ALTER TABLE `{$prefix}ventas_detalle`
                    DROP INDEX `idx_vd_es_invitacion`,
                ...
                DB::connection('pymes')->statement("
                    ALTER TABLE `{$prefix}ventas`
                    DROP INDEX `idx_ventas_es_invitacion_total`,
