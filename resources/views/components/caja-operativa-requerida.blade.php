...
@props([
    'estadoCaja' => ['operativa' => false, 'estado' => 'sin_caja', 'mensaje' => __('No hay caja seleccionada'), 'caja' => null],
...
@php
    ...
    $mensaje = $estadoCaja['mensaje'] ?? __('No hay caja seleccionada');
    ...
    $config = match($estado) {
        'sin_caja' => [
            'titulo' => __('Sin Caja Seleccionada'),
            'mensaje' => __('Selecciona una caja para comenzar a operar.'),
            'icono' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
            'color' => 'gray',
            'accion' => __('Seleccionar una caja en la barra superior'),
        ],
        'sin_acceso' => [
            'titulo' => __('Sin Acceso a la Caja'),
            'mensaje' => __('No tienes permisos para operar esta caja.'),
            'icono' => 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z',
            'color' => 'red',
            'accion' => __('Solicitar acceso al administrador'),
        ],
        'sin_turno' => [
            'titulo' => __('Turno No Iniciado'),
            'mensaje' => __('La caja no tiene un turno abierto. Abre un turno para comenzar a operar.'),
            'icono' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
            'color' => 'amber',
            'accion' => __('Abrir turno para comenzar'),
        ],
        'pausada' => [
            'titulo' => __('Caja Inactiva'),
            'mensaje' => __('La caja tiene turno abierto pero se encuentra inactiva. Actívala para continuar operando.'),
            'icono' => 'M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z',
            'color' => 'blue',
            'accion' => __('Activar la caja para continuar'),
        ],
        default => [
            'titulo' => __('Caja No Disponible'),
            'mensaje' => __('La caja no está disponible en este momento.'),
            ...
            'color' => 'gray',
            'accion' => __('Verificar estado de la caja'),
        ],
    };

    // Usar mensaje personalizado si viene en estadoCaja, sino usar el del config
    $mensajeMostrar = ($mensaje && $mensaje !== __('No hay caja seleccionada')) ? $mensaje : $config['mensaje'];
            ...
            <div class="max-w-md w-full mx-4 {{ $colorClasses['bg'] }} {{ $colorClasses['border'] }} border-2 rounded-xl shadow-2xl p-6 transform">
                ...
                @if($caja)
                    <div class="text-center text-sm {{ $colorClasses['text'] }} mb-4 opacity-75">
                        {{ __('Caja:') }} <span class="font-medium">{{ is_array($caja) ? $caja['nombre'] : $caja->nombre }}</span>
                    </div>
                @endif
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-center">
                    ...
                    @elseif($estado === 'sin_caja')
                        <div class="text-center text-sm {{ $colorClasses['text'] }}">
                            ...
                            {{ __('Usa el selector de caja en la barra superior') }}
                        </div>
                    {{-- Estado: Sin acceso --}}
                    @elseif($estado === 'sin_acceso')
                        <div class="text-center text-sm {{ $colorClasses['text'] }}">
                            {{ __('Contacta al administrador para obtener acceso') }}
                        </div>
                    ...
                    @elseif(!$puedeGestionarCaja && in_array($estado, ['sin_turno', 'pausada']))
                        <div class="text-center text-sm {{ $colorClasses['text'] }}">
                            ...
                            {{ __('Solicita que activen la caja') }}
                        </div>
                    @endif
                </div>
