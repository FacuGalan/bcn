...
<div class="p-6">
    {{-- Header --}}
    <div class="mb-6">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Configuraci&oacute;n de Cajas</h3>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Configura los par&aacute;metros de cada caja, asigna puntos de venta AFIP y define grupos de cierre compartido
        </p>
    </div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                ({{ $cajasGrupo->count() }} {{ $cajasGrupo->count() === 1 ? 'caja' : 'cajas' }})
                            </span>
                        </div>
                    </div>
                            ...
                            @if($cajasIndividuales->count() > 0)
                                <div class="mb-6">
                                    <h5 class="text-sm font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                                        ...
                                        Cajas Individuales
                                        <span class="text-xs font-normal text-gray-400">(cierran solas)</span>
                                    </h5>
                                ...
                                @else
                                    <div class="text-center py-6 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-dashed border-gray-300 dark:border-gray-600">
                                        ...
                                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">
                                            Las cajas cierran individualmente
                                        </p>
                                    </div>
                                @endif
                            </div>
    ...
    @else
        ...
        <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No hay cajas</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                No se encontraron cajas configuradas en el sistema.
            </p>
        </div>
    @endif
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-blue-800 dark:text-blue-300">
                        Acerca de la configuraci&oacute;n de Cajas
                    </h3>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                                {{ $modoEdicionGrupo ? 'Editar Grupo de Cierre' : 'Nuevo Grupo de Cierre' }}
                            </h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Las cajas en este grupo compartir&aacute;n el cierre de turno
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 space-y-4">
                        ...
                        <div>
                            <label for="grupoNombre" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                Nombre del grupo <span class="text-gray-400 font-normal">(opcional)</span>
                            </label>
                            <input
                                type="text"
                                id="grupoNombre"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Selecciona las cajas del grupo
                                <span class="text-gray-400 font-normal">(m&iacute;nimo 2)</span>
                            </label>
                            ...
                            @if($cajasDisponibles->count() > 0)
                                ...
                                <p class="mt-2 text-sm {{ count($grupoCajasSeleccionadas) >= 2 ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' }}">
                                    {{ count($grupoCajasSeleccionadas) }} {{ count($grupoCajasSeleccionadas) === 1 ? 'caja seleccionada' : 'cajas seleccionadas' }}
                                    @if(count($grupoCajasSeleccionadas) < 2)
                                        <span class="text-yellow-600 dark:text-yellow-400">(necesitas al menos 2)</span>
                                    @endif
                                </p>
                            @else
                                <div class="text-center py-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                                    ...
                                    <p class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                                        No hay cajas disponibles para agrupar
                                    </p>
                                    <p class="text-xs text-yellow-600 dark:text-yellow-400">
                                        Todas las cajas ya pertenecen a otros grupos
                                    </p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    ...
                    >
                        {{ $modoEdicionGrupo ? 'Guardar Cambios' : 'Crear Grupo' }}
                    </button>
                    <button
                        type="button"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                                Eliminar grupo de cierre
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    &iquest;Est&aacute;s seguro de eliminar este grupo? Las cajas pasar&aacute;n a cerrar de forma individual.
                                </p>
                            </div>
                        </div>
                    </div>
