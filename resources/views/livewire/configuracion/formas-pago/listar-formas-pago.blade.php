<div class="p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Gestión de Formas de Pago') }}</h1>
        <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ __('Configura formas de pago y planes de cuotas') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('Forma de Pago') }}</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('Código') }}</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('Planes de Cuotas') }}</th>
                ...
                @foreach($formasPago as $fp)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span class="text-xs text-gray-400 dark:text-gray-500">{{ __('Sin planes de cuotas') }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <button wire:click="toggleActivo({{ $fp->id }})"
                                ...
                                {{ $fp->activo ? __('Activa') : __('Inactiva') }}
                            </button>
                        </td>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-bold">{{ __('Gestionar Cuotas') }} - {{ $formaPago->nombre }}</h3>
                    ...
                    @if($formaPago->cuotas->count() > 0)
                        <div class="mb-6">
                            <h4 class="text-sm font-semibold mb-2">{{ __('Planes Configurados') }}</h4>
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        <th class="px-4 py-2 text-left">{{ __('Cuotas') }}</th>
                                        <th class="px-4 py-2 text-left">{{ __('Interés %') }}</th>
                                        <th class="px-4 py-2 text-left">{{ __('Coeficiente') }}</th>
                                        <th class="px-4 py-2 text-right">{{ __('Acción') }}</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-4 py-2 text-right">
                                                <button wire:click="eliminarCuota({{ $cuota->id }})"
                                                        wire:confirm="{{ __('¿Eliminar este plan?') }}"
                    ...
                    <div class="border-t pt-4">
                        <h4 class="text-sm font-semibold mb-3">{{ __('Agregar Nuevo Plan') }}</h4>
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Cantidad Cuotas') }} *</label>
                                <input type="number"
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Interés %') }}</label>
                                <input type="number"
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Coeficiente') }}</label>
                                <input type="number"
