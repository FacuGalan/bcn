...
<div class="space-y-2">
    @forelse($escalas as $index => $escala)
        <div class="bg-white dark:bg-gray-700 rounded-lg p-3 border {{ $colorClasses['border'] }}">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-medium text-gray-600 dark:text-gray-300">Escala {{ $index + 1 }}</span>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                ...
                <div>
                    <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Hasta (opcional)</label>
                    <input type="number" wire:model="escalas.{{ $index }}.cantidad_hasta" min="1"
                           placeholder="Sin lim."
                           class="w-full text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-center">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Lleva</label>
                ...
                <div>
                    <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Bonifica</label>
            ...
            <div class="flex items-center gap-3 justify-center pt-2 border-t dark:border-gray-600 flex-wrap">
                <label class="flex items-center gap-1.5 cursor-pointer">
                    ...
                    <span class="text-xs text-gray-700 dark:text-gray-300">Gratis</span>
                </label>
                <label class="flex items-center gap-1.5 cursor-pointer">
                    <input type="radio" wire:model.live="escalas.{{ $index }}.beneficio_tipo" value="descuento"
                           class="{{ $colorClasses['radio'] }}">
                    <span class="text-xs text-gray-700 dark:text-gray-300">Descuento</span>
                </label>
                @if(($escala['beneficio_tipo'] ?? 'gratis') === 'descuento')
                ...
                <div class="mt-2 text-center">
                    <span class="px-2 py-0.5 {{ $colorClasses['bg'] }} {{ $colorClasses['text'] }} rounded text-xs font-medium">
                        @if($beneficioTipoEscala === 'gratis')
                            {{ $bonifica }} {{ $bonifica == 1 ? 'gratis' : 'gratis' }}
                        @else
                            1 con {{ $porcentaje }}% dto
                        @endif
                    </span>
                </div>
            @endif
    ...
    @empty
        <div class="bg-white dark:bg-gray-700 rounded-lg p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">No hay escalas configuradas</p>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Agrega escalas para ofrecer diferentes promociones segun cantidad</p>
        </div>
    @endforelse
    ...
    @if(count($escalas) > 0)
        <div class="mt-2 p-2 {{ $colorClasses['bg'] }} rounded text-xs {{ $colorClasses['text'] }}">
            <strong>Tip:</strong> Las escalas se evaluan en orden. Configura desde menores a mayores cantidades.
        </div>
    @endif
</div>
