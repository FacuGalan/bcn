<div class="py-6 sm:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex-1">
                    <div class="flex items-center justify-between gap-3 sm:block">
                        ...
                        <a
                            ...
                            :title="__('Volver a Formas de Pago')"
                        >
                    ...
                    <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-300">{{ __('Configura qué formas de pago están disponibles en cada sucursal y sus ajustes') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4 sm:mb-6 p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row sm:items-end gap-4">
                    ...
                    <div class="flex-1 sm:max-w-xs">
                        <label for="sucursal" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Sucursal') }}</label>
                        <select
                            id="sucursal"
            ...
            <div class="sm:hidden space-y-3">
                                    ...
                                    @if($fp['factura_fiscal_efectivo'])
                                        <span class="inline-flex items-center px-2 py-0.5 rounded {{ $fp['tiene_factura_fiscal_especifica'] ? 'bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-400' : 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-400' }}">
                                            ...
                                            @if($fp['tiene_factura_fiscal_especifica'])
                                                <svg class="w-3 h-3 ml-1" fill="currentColor" viewBox="0 0 20 20" title="{{ __('Específico para esta sucursal') }}">
                                                    <path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd" />
                                                </svg>
                                            @endif
                                        </span>
                                    @endif
                                    @if($puntosActivo && $fp['multiplicador_efectivo'] != 1)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded {{ $fp['tiene_multiplicador_especifico'] ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' }}">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                            x{{ $fp['multiplicador_efectivo'] }} {{ __('puntos') }}
                                            @if($fp['tiene_multiplicador_especifico'])
                                                <svg class="w-3 h-3 ml-1" fill="currentColor" viewBox="0 0 20 20" title="{{ __('Específico para esta sucursal') }}">
                                                    <path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd" />
                                                </svg>
                                            @endif
                        ...
                        @else
                            <div class="mt-3 text-xs text-gray-500 dark:text-gray-400 text-center">
                                <span class="inline-flex items-center px-2 py-1 rounded bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400">
                                    ...
                                    {{ __('Ajustes y cuotas se configuran en el desglose') }}
                                </span>
                            </div>
                        @endif
                    </div>
                @empty
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center text-gray-500 dark:text-gray-400">
                        <p class="text-sm">{{ __('No hay formas de pago activas en el sistema') }}</p>
                    </div>
                @endforelse
            </div>
                            ...
                            @forelse($formasPago as $fp)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150 {{ !$fp['activo_sucursal'] ? 'opacity-60' : '' }}">
                                        ...
                                        @if($fp['es_mixta'])
                                            ...
                                            @if(count($fp['conceptos_permitidos']) > 0)
                                                <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                    {{ implode(', ', array_map(fn($c) => __($c), $fp['conceptos_permitidos'])) }}
                                                </div>
                                            @endif
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <span class="text-xs text-gray-400 dark:text-gray-500">{{ __('Sin ajuste') }}</span>
                                        @endif
                                        @if(!$fp['es_mixta'] && $fp['tiene_ajuste_especifico'] && $fp['ajuste_general'] != 0)
                                            <div class="text-xs text-gray-400 dark:text-gray-500 mt-1">
                                                {{ __('General') }}: {{ $fp['ajuste_general'] > 0 ? '+' : '' }}{{ $fp['ajuste_general'] }}%
                                            </div>
                                        @endif
                                        @if($puntosActivo && !$fp['es_mixta'] && $fp['multiplicador_efectivo'] != 1)
                                            <div class="mt-1">
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $fp['tiene_multiplicador_especifico'] ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' }}">
                                                    <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                                    x{{ $fp['multiplicador_efectivo'] }} {{ __('puntos') }}
                                                    @if($fp['tiene_multiplicador_especifico'])
                                                        <svg class="w-3 h-3 ml-1" fill="currentColor" viewBox="0 0 20 20" title="{{ __('Específico') }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        >
                                            <span class="sr-only">{{ $fp['activo_sucursal'] ? __('Desactivar') : __('Activar') }}</span>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        {{ __('No hay formas de pago activas en el sistema') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        @else
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center text-gray-500 dark:text-gray-400">
                <p class="text-sm">{{ __('Selecciona una sucursal para configurar las formas de pago') }}</p>
            </div>
        @endif
                        ...
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-700">
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                                    {{ __('Configurar Forma de Pago por Sucursal') }}
                                </h3>
                            ...
                            <div class="mt-6 space-y-6">
                                ...
                                <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-4">
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Ajuste (Recargo/Descuento)') }}</h4>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <label for="usar_ajuste_general" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                                            {{ __('Usar ajuste general de la forma de pago') }}
                                        </label>
                                    </div>
                                    ...
                                    <div class="{{ $usar_ajuste_general ? 'opacity-50' : '' }}">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                            {{ __('Ajuste específico para esta sucursal (%)') }}
                                        </label>
                                        <input
                                            type="number"
                                            step="0.01"
                                        ...
                                        />
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                            {{ __('Valor positivo = recargo, valor negativo = descuento') }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-4">
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Facturación Fiscal') }}</h4>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <label for="usar_factura_fiscal_general" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                                            {{ __('Usar configuración general de la forma de pago') }}
                                        </label>
                                    </div>
                                    ...
                                    <div class="{{ $usar_factura_fiscal_general ? 'opacity-50' : '' }}">
                                        <div class="flex items-center">
                                            ...
                                            <label for="factura_fiscal_sucursal" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                                                {{ __('Generar factura fiscal automáticamente') }}
                                            </label>
                                        </div>
                                        <p class="mt-1 ml-6 text-xs text-gray-500 dark:text-gray-400">
                                            {{ __('Si está activado, las ventas con esta forma de pago generarán factura fiscal por defecto') }}
                                        </p>
                                    </div>
                                </div>

                                <!-- Sección: Multiplicador de Puntos -->
                                @if($puntosActivo)
                                    <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-4">
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Multiplicador de Puntos') }}</h4>

                                        <!-- Checkbox usar multiplicador general -->
                                        <div class="flex items-center">
                                            <input
                                                type="checkbox"
                                                id="usar_multiplicador_general"
                                                wire:model.live="usar_multiplicador_general"
                                                class="rounded border-gray-300 dark:border-gray-600 text-bcn-primary focus:ring-bcn-primary"
                                            />
                                            <label for="usar_multiplicador_general" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                                                {{ __('Usar multiplicador general de la forma de pago') }}
                                            </label>
                                        </div>

                                        <!-- Campo de multiplicador específico -->
                                        <div class="{{ $usar_multiplicador_general ? 'opacity-50' : '' }}">
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                                {{ __('Multiplicador específico para esta sucursal') }}
                                            </label>
                                            <input
                                                type="number"
                                                step="0.01"
                                                min="0"
                                                max="99.99"
                                                wire:model="multiplicador_puntos_sucursal"
                                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white shadow-sm focus:border-bcn-primary focus:ring-bcn-primary"
                                                placeholder="1.00"
                                                {{ $usar_multiplicador_general ? 'disabled' : '' }}
                                            />
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-700">
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                                    {{ __('Configurar Cuotas por Sucursal') }}
                                </h3>
                            ...
                            <div class="mt-6">
                                            ...
                                            <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                                <div class="flex items-center justify-between mb-3">
                                                    <div class="flex items-center gap-3">
                                                        <span class="text-sm font-medium text-gray-900 dark:text-white">
                                                            {{ $config['cantidad_cuotas'] }} {{ $config['cantidad_cuotas'] == 1 ? __('cuota') : __('cuotas') }}
                                                        </span>
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-200 text-gray-700 dark:text-gray-300">
                                                            {{ __('General') }}: {{ $config['recargo_general'] }}%
                                                        </span>
                                                    </div>

                                                    <!-- Toggle activo -->
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $config['activo'] ? __('Activa') : __('Inactiva') }}</span>
                                                        <button
                                                            type="button"
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    ...
                                                    <div class="{{ $config['usar_recargo_general'] ? 'opacity-50' : '' }}">
                                                        <div class="flex items-center gap-2">
                                                            <label class="text-xs text-gray-700 dark:text-gray-300">{{ __('Recargo sucursal') }}:</label>
                                                            <input
                                                                type="number"
                                                                step="0.01"
                                                                min="0"
                                ...
                                @else
                                    <p class="text-sm text-gray-500 dark:text-gray-400 text-center py-4">
                                        {{ __('No hay planes de cuotas configurados para esta forma de pago') }}
                                    </p>
                                @endif
                            </div>
                        </div>
