    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-start gap-3 sm:gap-4">
                <div class="flex-1">
                    <div class="flex items-center justify-between gap-3 sm:block">
                        <h2 class="text-xl sm:text-2xl font-bold text-bcn-secondary dark:text-white flex items-center h-10 sm:h-auto">Formas de Pago</h2>
                    ...
                    <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-300">Administra las formas de pago y planes de cuotas</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4 sm:mb-6 p-4 sm:p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <!-- Búsqueda -->
                <div>
                    <label for="busqueda" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Buscar</label>
                    <input
                        type="text"
                        id="busqueda"
                        wire:model.live.debounce.300ms="busqueda"
                        placeholder="Nombre, concepto o descripción..."
                ...
                <!-- Filtro de estado -->
                <div>
                    <label for="filtroActivo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Estado</label>
                    <select
                        id="filtroActivo"
        ...
        <div class="sm:hidden space-y-3">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                ...
                                @else
                                    {{ $formaPago->conceptoPago?->nombre ?? ucfirst(str_replace('_', ' ', $formaPago->concepto)) }}
                                @endif
                            </p>
            ...
            @empty
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center text-gray-500 dark:text-gray-400">
                    ...
                    <p class="mt-2 text-sm">No se encontraron formas de pago</p>
                </div>
            @endforelse
                        ...
                        @forelse($formasPago as $formaPago)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                        ...
                                        @if($formaPago->conceptosPermitidos->count() > 0)
                                            <div class="mt-1 flex flex-wrap gap-1">
                                                @foreach($formaPago->conceptosPermitidos->take(3) as $concepto)
                                                    <span class="text-xs text-gray-500">{{ $concepto->nombre }}</span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    >
                                        <span class="sr-only">{{ $formaPago->activo ? 'Desactivar' : 'Activar' }} forma de pago</span>
                                        <span
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">No se encontraron formas de pago</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div class="sm:col-span-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                        <label class="flex items-center cursor-pointer">
                                            ...
                                            />
                                            <span class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">Es forma de pago mixta</span>
                                        </label>
                                        <p class="mt-1 ml-6 text-xs text-gray-500 dark:text-gray-400">
                                            Una forma de pago mixta permite combinar múltiples conceptos de pago (efectivo + tarjeta + transferencia, etc.)
                                        </p>
                                    </div>
                                    ...
                                    @if(!$es_mixta)
                                        ...
                                        <div class="flex items-center gap-6">
                                            <div class="flex items-center">
                                                <input
                                                    ...
                                                    class="rounded border-gray-300 dark:border-gray-600 text-bcn-primary focus:ring-bcn-primary"
                                                />
                                                <label for="permite_cuotas" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">Permite cuotas</label>
                                            </div>
                                            <div class="flex items-center">
                                                <input
                                                    type="checkbox"
                                                    ...
                                                    class="rounded border-gray-300 dark:border-gray-600 text-bcn-primary focus:ring-bcn-primary"
                                                />
                                                <label for="factura_fiscal" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">Factura fiscal</label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="sm:col-span-2">
                                            ...
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                Valor positivo = recargo, valor negativo = descuento. Ej: 5 = +5% recargo, -10 = 10% descuento
                                            </p>
                                    ...
                                    @else
                                        ...
                                        <div class="sm:col-span-2">
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                                                Conceptos de pago permitidos *
                                            </label>
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                                                Seleccione los conceptos que podrán usarse con esta forma de pago mixta (mínimo 2).
                                            </p>
                                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                @foreach($conceptosPago as $concepto)
                                                    <label class="flex items-center p-3 bg-white dark:bg-gray-700 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors {{ in_array($concepto->id, $conceptos_permitidos) ? 'border-bcn-primary bg-bcn-light dark:bg-gray-600' : 'border-gray-200 dark:border-gray-600' }}">
                                                        ...
                                                        />
                                                        <div class="ml-2">
                                                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $concepto->nombre }}</span>
                                                            @if($concepto->permite_cuotas)
                                                                <span class="block text-xs text-blue-600">Permite cuotas</span>
                                                            @endif
                                                            @if($concepto->permite_vuelto)
                                                                <span class="block text-xs text-green-600">Permite vuelto</span>
                                                            @endif
                                                        </div>
                                                    </label>
                                                @endforeach
                                        ...
                                        <div class="sm:col-span-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                            <div class="flex items-start">
                                                ...
                                                <div class="text-xs text-amber-700">
                                                    <p class="font-medium">Nota sobre formas de pago mixtas:</p>
                                                    <ul class="mt-1 list-disc list-inside space-y-1">
                                                        <li>No tienen ajuste (recargo/descuento) propio</li>
                                                        <li>Los ajustes se aplican por cada forma de pago usada en el desglose</li>
                                                        <li>Las cuotas se configuran en las formas de pago individuales</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="sm:col-span-2 flex items-center">
                                        <input
                                            ...
                                            class="rounded border-gray-300 dark:border-gray-600 text-bcn-primary focus:ring-bcn-primary"
                                        />
                                        <label for="activo" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">Forma de pago activa</label>
                                    </div>
                                    ...
                                    <div class="sm:col-span-2 border-t border-gray-200 dark:border-gray-700 pt-4 mt-2">
                                        ...
                                        @if($sucursales->count() == 0)
                                            <p class="text-sm text-gray-500 dark:text-gray-400 italic">No hay sucursales configuradas</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-6">
                                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Planes Configurados</h4>
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        @if($cuota['recargo_porcentaje'] > 0)
                                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">
                                                                +{{ $cuota['recargo_porcentaje'] }}% recargo
                                                            </span>
                                                        @else
                                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                                                                Sin recargo
                                ...
                                @else
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 italic">No hay planes de cuotas configurados</p>
                                @endif

                                <!-- Agregar nuevo plan de cuotas -->
                                <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                                    <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Agregar Nuevo Plan</h4>
