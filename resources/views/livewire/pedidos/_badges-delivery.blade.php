...
<div class="space-y-1 {{ $class ?? '' }}">
    ...
    <div class="flex flex-wrap items-center gap-1">
        ...
        {{-- Encargo (RF-T16): pedido programado para día/hora futuros --}}
        @if($pedido->programado_para)
            <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-fuchsia-100 text-fuchsia-800 dark:bg-fuchsia-900/50 dark:text-fuchsia-200"
                title="{{ __('Encargo para') }} {{ $pedido->programado_para->format('d/m H:i') }}">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                {{ __('Encargo') }} · {{ $pedido->programado_para->isToday() ? $pedido->programado_para->format('H:i') : $pedido->programado_para->format('d/m H:i') }}
            </span>
        @endif
