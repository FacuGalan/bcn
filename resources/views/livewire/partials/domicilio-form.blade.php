    ...
    @if($conGeo)
        ...
        @if($this->mapsHabilitado())
            {{-- Picker de Google Maps (flujo invertido: provincia→localidad acotan el mapa).
                 Carga PEREZOSA: el SDK y el mapa solo se cargan al tocar "Abrir mapa"
                 (abrir() en domicilio-mapa.js). Sin click no hay llamada a la API. --}}
            <div class="sm:col-span-2" x-cloak
                ...
                <div x-show="!abierto" class="mt-1 space-y-1">
                    ...
                    @if(!is_null($domLatitud) && !is_null($domLongitud) && $domLatitud !== '' && $domLongitud !== '')
                        <p class="text-[11px] text-gray-500 dark:text-gray-400">{{ __('Coordenadas') }}: <span class="tabular-nums">{{ number_format((float) $domLatitud, 6) }}, {{ number_format((float) $domLongitud, 6) }}</span></p>
                    @endif
