...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Gestión de Cajas') }}</h1>
        <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ __('Apertura, cierre y movimientos de caja') }}</p>
    </div>
            ...
            @foreach($cajas as $caja)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                    <div class="px-6 py-4 {{ $caja->estaAbierta() ? 'bg-green-50 dark:bg-green-900/20 border-b-4 border-green-500' : 'bg-gray-50 dark:bg-gray-700 border-b-4 border-gray-300 dark:border-gray-600' }}">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full {{ $caja->estaAbierta() ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                {{ $caja->estaAbierta() ? __('Abierta') : __('Cerrada') }}
                            </span>
                        </div>
                        ...
                        @if($caja->estaAbierta())
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-300">{{ __('Saldo Actual') }}:</span>
                                    <span class="text-2xl font-bold text-gray-900 dark:text-white">$@precio($caja->saldo_actual)</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-300">{{ __('Saldo Inicial') }}:</span>
                                    <span class="font-medium dark:text-white">$@precio($caja->saldo_inicial)</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-300">{{ __('Ingresos') }}:</span>
                                    <span class="font-medium text-green-600">$@precio($caja->obtenerTotalIngresos())</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-300">{{ __('Egresos') }}:</span>
                                ...
                                <div class="pt-2 border-t dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
                                    {{ __('Abierta') }}: {{ $caja->fecha_apertura->format('d/m/Y H:i') }}
                                </div>
                            </div>
                        @else
                            <div class="text-center py-4">
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Caja cerrada') }}</p>
                                @if($caja->fecha_cierre)
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ __('Último cierre') }}: {{ $caja->fecha_cierre->format('d/m/Y H:i') }}</p>
                                @endif
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="px-6 py-3 bg-gray-50 dark:bg-gray-700 border-t dark:border-gray-600 flex flex-col gap-2">
                        @if($caja->estaAbierta())
                            <button wire:click="abrirModalMovimiento({{ $caja->id }})" class="w-full px-3 py-2 border border-transparent rounded text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">{{ __('Registrar Movimiento') }}</button>
                            <button wire:click="abrirModalCierre({{ $caja->id }})" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">{{ __('Cerrar Caja') }}</button>
                        @else
