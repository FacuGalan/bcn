    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">Gestión de compras y proveedores</p>
            </div>
            <button
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Comprobante</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Forma Pago</th>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                                <p class="text-gray-900 dark:text-white font-medium mb-1">No hay compras registradas</p>
                                <p class="text-gray-500 dark:text-gray-400">Comienza creando tu primera compra</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div class="lg:col-span-2 space-y-4">
                                <div>
                                    ...
                                    <input wire:model.live.debounce.300ms="buscarArticulo" type="text" class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" placeholder="Buscar por código o nombre...">
                                </div>

                                <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-2 border-b dark:border-gray-600"><h4 class="text-sm font-medium dark:text-white">Carrito ({{ count($carrito) }} items)</h4></div>

                                    @if(empty($carrito))
                                        <div class="p-8 text-center text-gray-500 dark:text-gray-400"><p class="text-sm">El carrito está vacío</p></div>
                                    @else
                                        <div class="max-h-96 overflow-y-auto">
                                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0">
                                                    <tr>
                                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Artículo</th>
                                                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Cant.</th>
                                                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Precio s/IVA</th>
                            ...
                            <div class="space-y-4">
                                ...
                                @if($formaPago !== 'cta_cte')
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Caja <span class="text-red-500">*</span></label>
                                ...
                                <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-gray-600 dark:text-gray-300">Subtotal (sin IVA):</span>
                                        <span class="font-medium dark:text-white">$@precio($subtotal)</span>
                                    </div>
                                    <div class="flex justify-between text-sm">
                                        <span class="text-gray-600 dark:text-gray-300">IVA (Crédito Fiscal):</span>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <button wire:click="cancelarCompraModal" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="inline-block w-full max-w-3xl my-8 bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                    <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b dark:border-gray-600">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-medium dark:text-white">Detalle de Compra #{{ $compraDetalle->numero_comprobante }}</h3>
                    ...
                    <div class="px-6 py-4 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Proveedor</label><p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $compraDetalle->proveedor->nombre }}</p></div>
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fecha</label><p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $compraDetalle->fecha->format('d/m/Y') }}</p></div>
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Forma de Pago</label><p class="mt-1 text-sm text-gray-900 dark:text-white">{{ ucfirst(str_replace('_', ' ', $compraDetalle->forma_pago)) }}</p></div>
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Estado</label><span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium {{ $compraDetalle->estado === 'completada' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">{{ ucfirst($compraDetalle->estado) }}</span></div>
                        </div>
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
                            <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-300">Subtotal:</span><span class="font-medium dark:text-white">$@precio($compraDetalle->subtotal)</span></div>
                            <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-300">IVA:</span><span class="font-medium dark:text-white">$@precio($compraDetalle->total_iva)</span></div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-6 py-3 border-t dark:border-gray-600 flex justify-end">
                        <button wire:click="cerrarDetalle" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">Cerrar</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="inline-block w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Saldo Pendiente</label><p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white">$@precio($compraPago->saldo_pendiente)</p></div>
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Monto a Pagar</label><input wire:model="montoPago" type="number" step="0.01" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"></div>
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Caja</label><select wire:model="cajaPago" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"><option value="">Seleccione una caja</option>@foreach($cajas as $caja)@if($caja->estaAbierta())<option value="{{ $caja->id }}">{{ $caja->nombre }}</option>@endif @endforeach</select></div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-700 px-6 py-3 border-t dark:border-gray-600 flex justify-end gap-3">
                        <button wire:click="cancelarModalPago" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">Cancelar</button>
