<div>
    @if($mostrarModal)
        <x-bcn-modal
            :title="__('Perfil fiscal de :proveedor', ['proveedor' => $proveedorNombre])"
        ...
        >
            <x-slot:body>
                {{-- Proveedor sobre el que se está trabajando --}}
                <div class="mb-4 px-3 py-2 rounded-md bg-bcn-primary/10 dark:bg-bcn-primary/20">
                    <p class="text-sm text-gray-700 dark:text-gray-200">
                        <span class="font-semibold">{{ $proveedorNombre }}</span>
                        @if($proveedorCuit)
                            <span class="text-gray-500 dark:text-gray-400">· {{ __('CUIT') }} <span class="font-mono">{{ $proveedorCuit }}</span></span>
                        @endif
                    </p>
                </div>

                <p class="mb-4 text-xs text-gray-500 dark:text-gray-400">
                    {{ __('Se precargan como renglones de percepción al elegir el proveedor en una compra (el monto exacto sale de la factura física)') }}
                </p>
                ...
                {{-- Lista de percepciones habituales --}}
                @if(count($filas) > 0)
                    <div class="space-y-3">
                        @foreach($filas as $i => $fila)
                            <div wire:key="fila-{{ $fila['impuesto_id'] }}"
                                ...
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <div class="w-28 shrink-0">
                                        ...
                                        <input type="number" step="0.0001" min="0" max="100" wire:model="filas.{{ $i }}.alicuota"
                                            class="mt-1 block w-full text-right rounded-md border-gray-300 shadow-sm focus:border-bcn-primary focus:ring-bcn-primary sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        @error('filas.'.$i.'.alicuota') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <button type="button" wire:click="quitarImpuesto({{ $fila['impuesto_id'] }})"
                    ...
                    <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                        {{ __('Los cambios se aplican al presionar Guardar.') }}
                    </p>
                @else
                    <div class="text-center py-8">
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Este proveedor no tiene percepciones habituales configuradas.') }}
                        </p>
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">
                            {{ __('Buscá la percepción arriba (ej: IIBB Buenos Aires) y agregala; después cargá la alícuota con la que este proveedor suele percibirte.') }}
                        </p>
                    </div>
                @endif
            </x-slot:body>
