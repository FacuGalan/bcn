<div class="relative" x-data="{ open: @entangle('mostrarDropdown') }">
    @if($cajasDisponibles && $cajasDisponibles->count() > 1)
        ...
        <button
            @click="open = !open"
            type="button"
            class="w-full md:w-auto flex items-center justify-center md:justify-start gap-1.5 px-2 py-1 text-xs font-medium whitespace-nowrap text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-indigo-500"
        >
            ...
            <span x-show="$wire.showLabels || ($store.awareness?.caja ?? false)" x-cloak>
            ...
            @if($cajaActual)
                ...
                @if($estadoOp === 'operativa')
                    <span x-show="$wire.showLabels || ($store.awareness?.caja ?? false)" x-cloak class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-green-800 bg-green-100 dark:text-green-200 dark:bg-green-900/50 rounded-full" title="Operativa">
                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                    </span>
                @elseif($estadoOp === 'pausada')
                    <span x-show="$wire.showLabels || ($store.awareness?.caja ?? false)" x-cloak class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-amber-800 bg-amber-100 dark:text-amber-200 dark:bg-amber-900/50 rounded-full" title="Pausada">
                ...
                @else
                    <span x-show="$wire.showLabels || ($store.awareness?.caja ?? false)" x-cloak class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-gray-600 bg-gray-100 dark:text-gray-400 dark:bg-gray-700 rounded-full" title="Sin turno">
    ...
    @elseif($cajaActual)
        ...
        <div class="w-full md:w-auto flex items-center justify-center md:justify-start gap-1.5 px-2 py-1 text-xs font-medium whitespace-nowrap text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-900 rounded-md">
            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
            </svg>
            <span x-show="$wire.showLabels || ($store.awareness?.caja ?? false)" x-cloak>{{ $cajaActual->nombre }}</span>
